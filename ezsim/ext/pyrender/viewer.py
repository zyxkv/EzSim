...
class Viewer(pyglet.window.Window):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Note: context.scene is ezsim.engine.scene.Scene
        # Note: context._scene is ezsim.ext.pyrender.scene.Scene
    ...
    def render_offscreen(self, camera_node, render_target, rgb=True, depth=False, seg=False, normal=False):
        ...
        if rgb and seg:
            ezsim.raise_exception("RGB and segmentation map cannot be rendered in the same forward pass.")
    ...
    def draw_offscreen(self):
        ...
        self._renderer.jit.update_buffer(self.ezsim_context.buffer)
        self.ezsim_context.buffer.clear()
    ...
    def on_key_press(self, symbol: int, modifiers: int) -> EVENT_HANDLE_STATE:
        ...
        # W toggles world frame
            ...
            if not self.ezsim_context.world_frame_shown:
                self.ezsim_context.on_world_frame()
                self._message_text = "World Frame On"
            else:
                self.ezsim_context.off_world_frame()
                self._message_text = "World Frame Off"

        # L toggles link frame
        elif symbol == pyglet.window.key.L:
            if not self.ezsim_context.link_frame_shown:
                self.ezsim_context.on_link_frame()
                self._message_text = "Link Frame On"
            else:
                self.ezsim_context.off_link_frame()
                self._message_text = "Link Frame Off"

        # C toggles camera frustum
        elif symbol == pyglet.window.key.C:
            if not self.ezsim_context.camera_frustum_shown:
                self.ezsim_context.on_camera_frustum()
                self._message_text = "Camera Frustrum On"
            else:
                self.ezsim_context.off_camera_frustum()
        ...
        #     if self.ezsim_context.rigid_shown == 'visual':
        #         self.ezsim_context.on_rigid('collision')
        #         self._message_text = "Geom Type: 'collision'"
        #     elif self.ezsim_context.rigid_shown == 'collision':
        #         self.ezsim_context.on_rigid('sdf')
        #         self._message_text = "Geom Type: 'sdf'"
        #     else:
        #         self.ezsim_context.on_rigid('visual')
    ...
    def _save_image(self):
        ...
        if filename is not None:
            ...
            img_arr = self._renderer.jit.read_color_buf(*self._viewport_size, rgba=False)
