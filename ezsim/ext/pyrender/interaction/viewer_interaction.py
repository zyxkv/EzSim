...
if TYPE_CHECKING:
    ...
    from ezsim.ext.pyrender.node import Node
...
class ViewerInteraction(ViewerInteractionBase):
    ...
    def raycast_against_entity_obb(self, entity: RigidEntity, ray: Ray) -> RayHit:
        if isinstance(entity.morph, ezsim.morphs.Box):
            obb: OBB = self._get_box_obb(entity)
            ray_hit = obb.raycast(ray)
            if ray_hit.is_hit:
                ray_hit.geom = entity.geoms[0]
            return ray_hit
        elif isinstance(entity.morph, ezsim.morphs.Plane):
            # ignore plane
    ...
    def _get_box_obb(self, box_entity: RigidEntity) -> OBB:
        box: ezsim.morphs.Box = box_entity.morph
    ...
    def _draw_entity_unrotated_obb(self, geom: 'RigidGeom') -> None:
        obb: OBB | None = None
        if isinstance(geom.entity.morph, ezsim.morphs.Box):
        ...
        # if isinstance(entity.morph, ezsim.morphs.Plane):
        #     plane: ezsim.morphs.Plane = entity.morph
        #     pass
        # if isinstance(entity.morph, ezsim.morphs.Box):
        #     box: ezsim.morphs.Box = entity.morph
