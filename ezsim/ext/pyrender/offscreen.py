...
class OffscreenRenderer(object):
    ...
    def make_uncurrent(self):
        ...
        if not self._has_valid_context:
            ezsim.raise_exception("The method was called before making a context current.")
        ...
        if rgb or depth or seg:
            if self._platform.supports_framebuffers():
                ...
                if flags & RenderFlags.ENV_SEPARATE:
                    ezsim.raise_exception("'env_separate_rigid=True' not supported on this platform.")
                # if normal:
                #     ezsim.raise_exception("'normal=True' not supported on this platform.")
