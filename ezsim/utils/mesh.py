...
def get_hashkey(*args):
    ...
    for arg in args:
        hasher.update(arg)
    hasher.update(ezsim.__version__.encode())
...
def compute_sdf_data(mesh, res):
    ...
    voxels = voxels.reshape((res, res, res)).astype(ezsim.np_float, copy=False)

    T_mesh_to_sdf = np.eye(4, dtype=ezsim.np_float)
...
def surface_uvs_to_trimesh_visual(surface, uvs=None, n_verts=None):
    ...
    if isinstance(texture, ezsim.textures.ImageTexture):
        if uvs is not None:
            uvs = uvs.copy()
            uvs[:, 1] = 1.0 - uvs[:, 1]
    ...
    elif isinstance(texture, ezsim.textures.ColorTexture):
        if n_verts is None:
            ezsim.raise_exception("n_verts is required for color texture.")
        visual = trimesh.visual.ColorVisuals(vertex_colors=np.tile(np.array(texture.color), [n_verts, 1]))

    else:
        ezsim.raise_exception("Cannot get texture when generating trimesh visual.")
...
def convex_decompose(mesh, coacd_options):
    ...
    if os.path.exists(cvx_path):
        ezsim.logger.debug("Convex decomposition file (.cvx) found in cache.")
        try:
            with open(cvx_path, "rb") as file:
                mesh_parts = pkl.load(file)
            is_cached_loaded = True
        except (EOFError, ModuleNotFoundError, pkl.UnpicklingError):
            ezsim.logger.info("Ignoring corrupted cache.")

    if not is_cached_loaded:
        with ezsim.logger.timer("Running convex decomposition."):
...
def postprocess_collision_geoms(
    ...
    for g_info in g_infos:
        mesh = g_info["mesh"]
        tmesh = mesh.trimesh
        if g_info["type"] != ezsim.GEOM_TYPE.MESH:
            continue
        if tmesh.is_winding_consistent and not tmesh.is_watertight:
            ...
            if abs(tmesh_repaired.volume) < ezsim.EPS:
                continue
    ...
    if must_decompose and len(g_infos) > 1:
        is_merged = all(g_info["type"] == ezsim.GEOM_TYPE.MESH for g_info in g_infos)
        for name in ("contype", "conaffinity", "friction", "sol_params"):
            ...
            if not (np.isnan(diffs).all(axis=0) | (np.abs(diffs) < ezsim.EPS).all(axis=0)).all():
                is_merged = False
        ...
        if is_merged:
            ...
            for g_info in g_infos:
                ...
                tmesh.apply_transform(ezsim.utils.geom.trans_quat_to_T(pos, quat))
                tmeshes.append(tmesh)
            tmesh = trimesh.util.concatenate(tmeshes)
            mesh = ezsim.Mesh.from_trimesh(mesh=tmesh, surface=ezsim.surfaces.Collision(), metadata={"merged": True})
    ...
    if must_decompose:
        ...
        for g_info in g_infos:
            ...
            if not tmesh.is_winding_consistent:
                volume_err = float("inf")
            elif abs(tmesh.volume) < ezsim.EPS:
                volume_err = 0.0
            else:
                cmesh = trimesh.convex.convex_hull(tmesh)
                volume_err = cmesh.volume / abs(tmesh.volume) - 1.0
            if volume_err > decompose_error_threshold:  # Note that 'inf' is not larger than 'inf'
                ...
                meshes = [
                    ezsim.Mesh.from_trimesh(
                        tmesh, surface=ezsim.surfaces.Collision(), metadata={**mesh.metadata, "decomposed": True}
                    )
...
def parse_mesh_trimesh(path, group_by_material, scale, surface):
    ...
    for _, mesh in trimesh.load(path, force="scene", group_material=group_by_material, process=False).geometry.items():
        meshes.append(ezsim.Mesh.from_trimesh(mesh=mesh, scale=scale, surface=surface, metadata={"mesh_path": path}))
    return meshes


def trimesh_to_mesh(mesh, scale, surface):
    return ezsim.Mesh.from_trimesh(mesh=mesh, scale=scale, surface=surface)
...
def create_texture(image, factor, encoding):
    if image is not None:
        return ezsim.textures.ImageTexture(image_array=image, image_color=factor, encoding=encoding)
    elif factor is not None:
        return ezsim.textures.ColorTexture(color=factor)
    else:
        return None
...
def apply_transform(transform, positions, normals=None):
    ...
    if normals is not None:
        ...
        if np.abs(3.0 - np.trace(rot_mat)) > ezsim.EPS**2:  # has rotation or scaling
            transformed_normals = normals @ rot_mat
            scale = np.linalg.norm(rot_mat, axis=1, keepdims=True)
            if np.any(np.abs(scale - 1.0) > ezsim.EPS):  # has scale
...
def create_box(extents=None, color=(1.0, 1.0, 1.0, 1.0), bounds=None, wireframe=False, wireframe_radius=0.002):
    if bounds is not None:
        ...
        ezsim.raise_exception("Neither `extents` nor `bounds` is provided.")
...
def generate_tetgen_config_from_morph(morph):
    if not isinstance(morph, ezsim.options.morphs.TetGenMixin):
        raise TypeError(
...
def check_exr_compression(exr_path):
    ...
    if exr_header["compression"].v > Imath.Compression.PIZ_COMPRESSION:
        ...
        if os.path.exists(new_exr_path):
            ezsim.logger.info(f"Assets of fixed compression detected and used: {new_exr_path}.")
        else:
            ezsim.logger.warning(
