...
class BVHTree:
    ...
    def __init__(self, rigid_bodies):
        ...
        struct_narrow_candidate = ti.types.struct(
            ...
            v2=ezsim.ti_int,
            v3=ezsim.ti_int,
            ctype=ezsim.ti_int,  # 0: vertex face; 1: edge edge
            x0=ezsim.ti_vec3,
            x1=ezsim.ti_vec3,
            x2=ezsim.ti_vec3,
            x3=ezsim.ti_vec3,
            ...
            # accd
            toi=ezsim.ti_float,
            is_col=ezsim.ti_int,  # 0: no collision; 1: collision; -1: unkown
            max_disp_mag=ezsim.ti_float,
            distance=ezsim.ti_float,
            # dvd
        ...
        self.con_jac = ti.field(dtype=ezsim.ti_float, shape=(self.len_constraints, self.solver.n_dofs))
        self.con_diag = ti.field(dtype=ezsim.ti_float, shape=(self.len_constraints))
        self.con_aref = ti.field(dtype=ezsim.ti_float, shape=(self.len_constraints))

        self.constraint_A = ti.field(dtype=ezsim.ti_float, shape=(self.len_constraints, self.len_constraints))
        self.constraint_b = ti.field(dtype=ezsim.ti_float, shape=(self.len_constraints))

        self.x = ti.field(dtype=ezsim.ti_float, shape=(self.len_constraints))
        self.x1 = ti.field(dtype=ezsim.ti_float, shape=(self.len_constraints))
        self.xgrad = ti.field(dtype=ezsim.ti_float, shape=(self.len_constraints))

        self.Ax_b = ti.field(dtype=ezsim.ti_float, shape=(self.len_constraints))

        # 0 candidate, 1 impact, 3 constraints
        self.cd_counter = ti.field(ezsim.ti_int, shape=(3))
        ...
        self.tri_centers = ti.Vector.field(3, dtype=ezsim.ti_float, shape=(self.n_f))

        # my_id, fstart, n, parent_id
        self.que = ti.field(dtype=ezsim.ti_int, shape=(2 * self.n_f, 4))
        # self.face_buffer[self.n_f] records the current node idx during building tree
        self.face_buffer = ti.field(dtype=ezsim.ti_int, shape=(self.n_f + self.n_links))
    ...
    def process_structure(self):
        ...
        node_idx = np.arange(len(depth), dtype=ezsim.np_int)
        self.depth_node_idx = []
        ...
        self.nodes_leaf_to_root = ti.field(dtype=ezsim.ti_int, shape=nodes_leaf_to_root.shape)
    ...
    def generate_collision_shape_pairs(self):
        ...
        self.collision_pairs = ti.Vector.field(2, dtype=ezsim.ti_int, shape=(len(pairs)))
        self.collision_pairs.from_numpy(np.array(pairs, dtype=ezsim.np_int))
        print(f"generate_collision_shape_pairs done, {len(pairs)} pairs")
        print("pairs", pairs)

        # node a, node b
        self.cd_node_que = ti.field(
            dtype=ezsim.ti_int, shape=(self.solver.n_geoms_max, self.solver.n_geoms_max, self.len_cd_que, 2)
        )
