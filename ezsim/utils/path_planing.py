...
class PathPlanner(ABC):
    def __init__(self, entity):
        ...
        self.PENETRATION_EPS = 1e-5 if ezsim.ti_float == ti.f32 else 0.0

        for joint in entity.joints:
            if joint.type == ezsim.JOINT_TYPE.FREE:
                ezsim.raise_exception("planning for the ezsim.JOINT_TYPE.FREE is not supported (yet)")
            elif joint.type == ezsim.JOINT_TYPE.SPHERICAL:
                ezsim.raise_exception("planning for the ezsim.JOINT_TYPE.SPHERICAL is not supported (yet)")
    ...
    def shortcut_path(
        ...
        for i in range(iterations):
            ind = torch.multinomial(path_mask.T, 2).sort().values.to(ezsim.tc_int)  # B, 2
...
class RRT(PathPlanner):
    def _init_rrt_fields(self, goal_bias=0.05, max_nodes=2000, pos_tol=5e-3, max_step_size=0.1):
        ...
        if not self._is_rrt_init:
            ...
            self._rrt_start_configuration = ti.field(
                dtype=ezsim.ti_float, shape=self._solver._batch_shape(self._entity.n_qs)
            )
            self._rrt_goal_configuration = ti.field(
                dtype=ezsim.ti_float, shape=self._solver._batch_shape(self._entity.n_qs)
            )
            self.struct_rrt_node_info = ti.types.struct(
                configuration=ti.types.vector(self._entity.n_qs, ezsim.ti_float),
                parent_idx=ezsim.ti_int,
            )
            self._rrt_node_info = self.struct_rrt_node_info.field(shape=self._solver._batch_shape(self._rrt_max_nodes))
            self._rrt_tree_size = ti.field(dtype=ezsim.ti_int, shape=self._solver._batch_shape())
            self._rrt_is_active = ti.field(dtype=ezsim.ti_bool, shape=self._solver._batch_shape())
            self._rrt_goal_reached_node_idx = ti.field(dtype=ezsim.ti_int, shape=self._solver._batch_shape())
        ...
        for i_b_ in range(envs_idx.shape[0]):
            ...
            if self._rrt_is_active[i_b]:
                random_sample = ti.Vector(
                    [
                        q_limit_lower[i_q] + ti.random(dtype=ezsim.ti_float) * (q_limit_upper[i_q] - q_limit_lower[i_q])
                        for i_q in range(self._entity.n_qs)
                    ]
                )
                ...
                nearest_neighbor_dist = ezsim.ti_float(1e30)
                ...
                steer_result = ti.Vector.zero(ezsim.ti_float, self._entity.n_qs)
                for i_q in range(self._entity.n_qs):
    ...
    def plan(
        ...
        envs_idx_local = torch.arange(len(envs_idx), device=ezsim.device)
        ...
        ezsim.logger.debug("Start RRT planning...")
        ...
        is_invalid = self._rrt_is_active.to_torch(device=ezsim.device).bool()
        ts = self._rrt_tree_size.to_torch(device=ezsim.device)
        g_n = self._rrt_goal_reached_node_idx.to_torch(device=ezsim.device)  # B

        node_info = self._rrt_node_info.to_torch(device=ezsim.device)
        ...
        if is_invalid.all():
            ...
            sol = torch.zeros((num_waypoints, len(envs_idx), sol.shape[-1]), dtype=ezsim.tc_float, device=ezsim.device)
        ...
        if is_invalid.any():
            ezsim.logger.info(f"RRT planning failed in {int(is_invalid.sum())} environments")
        return sol, is_invalid
...
class RRTConnect(PathPlanner):
    ...
    def plan(
        ...
        ezsim.logger.debug("Start RRTConnect planning...")
        time_start = time.time()
        forward_pass = True
        for _ in range(self._rrt_max_nodes):
            ...
            ezsim.logger.info(f"RRTConnect planning exceeded maximum number of nodes ({self._rrt_max_nodes}).")

        ezsim.logger.debug(f"RRTConnect planning time: {time.time() - time_start}")
        is_invalid = self._rrt_is_active.to_torch(device=ezsim.device).bool()
        ts = self._rrt_tree_size.to_torch(device=ezsim.device)
        g_n = self._rrt_goal_reached_node_idx.to_torch(device=ezsim.device)  # B
        ...
        c_n = self._rrt_goal_reached_node_idx.to_torch(device=ezsim.device)  # B
        res = []
        for _ in range(ts.max() // 2):
        ...
        if is_invalid.all():
            if self._solver.n_envs > 0:
                ...
                self._entity.set_qpos(qpos_cur, zero_velocity=False)
            return torch.zeros(num_waypoints, len(envs_idx), sol.shape[-1], device=ezsim.device), is_invalid
        ...
        if is_invalid.any():
            ezsim.logger.info(f"RRTConnect planning failed in {int(is_invalid.sum())} environments")
...
def align_waypoints_length(path: torch.Tensor, mask: torch.Tensor, num_points: int) -> torch.Tensor:
    ...
    res = torch.zeros(
        (num_points, t_path.shape[0], t_path.shape[1]), dtype=ezsim.tc_float, device=ezsim.device
    )  # [num_points, B, Dof]
...
def rrt_valid_mask(tensor: torch.Tensor) -> torch.Tensor:
    ...
    tensor: torch.Tensor
        path tensor in [N, B]
    """
    mask = (tensor > 0.0).to(ezsim.tc_float)  # N, B
    mask_float = mask.T.unsqueeze(1)  # B 1, N
    kernel = torch.ones((1, 1, 3), device=tensor.device, dtype=ezsim.tc_float)
