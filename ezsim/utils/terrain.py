...
from ezsim.ext.isaacgym import terrain_utils as isaacgym_terrain_utils
from ezsim.options.morphs import Terrain
...
def parse_terrain(morph: Terrain, surface):
    ...
    if morph.from_stored is not None:
        ...
        if morph.height_field is not None:
            ...
            heightfield = np.zeros(
                np.array(morph.n_subterrains) * np.array([subterrain_rows, subterrain_cols]), dtype=ezsim.np_float
            )
            ...
            for i in range(morph.n_subterrains[0]):
                for j in range(morph.n_subterrains[1]):
                    ...
                    if subterrain_type == "flat_terrain":
                        subterrain_height_field = np.zeros((subterrain_rows, subterrain_cols), dtype=ezsim.np_float)
                        ...
                        ezsim.raise_exception(f"Unsupported subterrain type: {subterrain_type}")
    ...
    vmesh = ezsim.Mesh.from_trimesh(mesh=tmesh, surface=surface, metadata={})
...
def fractal_terrain(terrain, levels=8, scale=1.0):
    ...
    height = np.zeros((width, length), dtype=ezsim.np_float)
