...
def ti_R_to_xyz(R):
    ...
    """
    xyz = ti.Vector.zero(ezsim.ti_float, 3)
...
def ti_rotvec_to_R(rotvec):
    R = ti.Matrix.identity(ezsim.ti_float, 3)
...
def ti_rotvec_to_quat(rotvec):
    quat = ti.Vector.zero(ezsim.ti_float, 4)
...
def ti_quat_to_xyz(quat):
    ...
    """
    roll = ezsim.ti_float(0.0)
    pitch = ezsim.ti_float(0.0)
    yaw = ezsim.ti_float(0.0)

    quat_norm_sqr = quat.norm_sqr()
    if quat_norm_sqr > ezsim.EPS:
        s = 2.0 / quat_norm_sqr
    ...
    return ti.Vector([roll, pitch, yaw], dt=ezsim.ti_float)


@ti.func
def ti_quat_to_rotvec(quat):
    q_w, q_x, q_y, q_z = quat
    rotvec = ti.Vector([q_x, q_y, q_z], dt=ezsim.ti_float)
...
def ti_trans_quat_to_T(trans, quat):
    T = ti.Matrix.identity(ezsim.ti_float, 4)
...
def ti_inv_quat(quat):
    return ti.Vector([quat[0], -quat[1], -quat[2], -quat[3]], dt=ezsim.ti_float)
...
def ti_quat_mul(u, v):
    ...
    return ti.Vector([w, x, y, z], dt=ezsim.ti_float)
...
@ti.func
def ti_normalize(v):
    return v / (v.norm(ezsim.EPS))


@ti.func
def ti_identity_quat():
    return ti.Vector([1.0, 0.0, 0.0, 0.0], dt=ezsim.ti_float)


@ti.func
def ti_vec3(val):
    return ti.Vector([val, val, val], dt=ezsim.ti_float)


@ti.func
def ti_nowhere():
    # let's inject a bit of humor here
    return ti.Vector([-2333333, -6666666, -5201314], dt=ezsim.ti_float)
...
def ti_orthogonals(a):
    ...
    """
    b = ti.Vector.zero(ezsim.ti_float, 3)
    if ti.abs(a[1]) < 0.5:
        b[0] = -a[0] * a[1]
        b[1] = 1.0 - a[1] ** 2
        b[2] = -a[2] * a[1]
...
def inv_quat(quat):
    if isinstance(quat, torch.Tensor):
        ...
        ezsim.raise_exception(f"the input must be either torch.Tensor or np.ndarray. got: {type(quat)=}")
    return _quat
...
def inv_T(T):
    if isinstance(T, torch.Tensor):
        ...
        ezsim.raise_exception(f"the input must be torch.Tensor or np.ndarray. got: {type(T)=}")
...
def normalize(x, eps: float = 1e-12):
    if isinstance(x, torch.Tensor):
        ...
        ezsim.raise_exception(f"the input must be either torch.Tensor or np.ndarray. got: {type(x)=}")
...
def rot6d_to_R(d6):
    ...
    if isinstance(d6, torch.Tensor):
        ...
        ezsim.raise_exception(f"the input must be either torch.Tensor or np.ndarray. got: {type(d6)=}")
...
def R_to_rot6d(R):
    ...
    if isinstance(R, torch.Tensor):
        ...
        ezsim.raise_exception(f"the input must be either torch.Tensor or np.ndarray. got: {type(R)=}")
...
def xyz_to_quat(xyz, rpy=False, degrees=False):
    if isinstance(xyz, torch.Tensor):
        ...
        ezsim.raise_exception(f"the input must be either torch.Tensor or np.ndarray. got: {type(xyz)=}")
...
def trans_R_to_T(trans=None, R=None, *, out=None):
    ...
    if is_torch:
        ...
        ezsim.raise_exception(f"both of the inputs must be torch.Tensor or np.ndarray. got: {type(trans)=} and {type(R)=}")
    if B:
...
def transform_quat_by_quat(v, u):
    ...
    if all(isinstance(e, torch.Tensor) for e in (u, v)):
        ...
        ezsim.raise_exception(f"The inputs must all be torch.Tensor or np.ndarray. got: {type(v)=} and {type(quat)=}")
...
def transform_by_R(pos, R):
    ...
    if all(isinstance(e, torch.Tensor) for e in (pos, R) if e is not None):
        ...
        ezsim.raise_exception(f"both of the inputs must be torch.Tensor or np.ndarray. got: {type(pos)=} and {type(R)=}")
...
def _tc_z_up_to_R(z, up=None, out=None):
    ...
    z /= z_norm.clamp(min=ezsim.EPS)
    # Handle zero norm cases
    zero_mask = z_norm[..., 0] < ezsim.EPS
    if zero_mask.any():
    ...
    x /= x_norm.clamp(min=ezsim.EPS)

    # Handle zero x norm cases
    zero_x_mask = x_norm[..., 0] < ezsim.EPS
...
def T_to_pos_lookat_up(T): 
    # T supposed to be batch Transformation matrices
    pos = T[..., :3, 3]
    lookat = T[..., :3, 3] - T[..., :3, 2]
    up = T[..., :3, 1]
...
def quat_to_rotvec(quat: np.ndarray, out: np.ndarray | None = None) -> np.ndarray:
    ...
    inv_sinc = angle / np.maximum(s2, ezsim.EPS)
...
def rotvec_to_quat(rotvec: np.ndarray, out: np.ndarray | None = None) -> np.ndarray:
    ...
    axis = rotvec / np.maximum(angle, ezsim.EPS)
...
def z_to_R(v_a: np.ndarray, out: np.ndarray | None = None) -> np.ndarray:
    ...
    for i in np.ndindex(B):
        ...
        v_x, v_y, v_z = v_a[i]
        cos_theta[i] = v_z
        if abs(cos_theta[i]) < 1.0 - ezsim.EPS:
            axis_i[0] = -v_y
            axis_i[1] = v_x
            axis_i[2] = 0.0
        else:
...
def z_to_quat(v_a: np.ndarray, out: np.ndarray | None = None) -> np.ndarray:
    ...
    for i in np.ndindex(v_a.shape[:-1]):
        ...
        if v_a_norm[i] < ezsim.EPS:
            quat[:] = 1.0, 0.0, 0.0, 0.0
        elif v_z > -1.0 + ezsim.EPS:
            s = np.sqrt(2.0 * (1.0 + v_z))
            s_inv = 1.0 / s
            quat[:] = 0.5 * s, -v_y * s_inv, v_x * s_inv, 0.0
        else:
            eps_thr = np.sqrt(ezsim.EPS)
            eps_x = abs(v_x) < ezsim.EPS
            eps_y = abs(v_y) < ezsim.EPS
            if not eps_y:
                ratio = v_x / v_y
                if eps_x:
...
def zero_pos():
    return np.zeros(3, dtype=ezsim.np_float)


def identity_quat():
    return np.array([1.0, 0.0, 0.0, 0.0], dtype=ezsim.np_float)


def tc_zero_pos():
    return torch.zeros(3, dtype=ezsim.tc_float, device=ezsim.device)


def tc_identity_quat():
    return torch.tensor([1.0, 0.0, 0.0, 0.0], dtype=ezsim.tc_float, device=ezsim.device)


def nowhere():
    # let's inject a bit of humor here
    return np.array([2333333, 6666666, 5201314], dtype=ezsim.np_float)
...
def default_dofs_kp(n=6):
    return np.full((n,), fill_value=100.0, dtype=ezsim.np_float)


def default_dofs_kv(n=6):
    return np.full((n,), fill_value=10.0, dtype=ezsim.np_float)
...
class SpatialHasher:
    ...
    def build(self, n_batch):
        ...
        self.slot_size = ti.field(ezsim.ti_int, shape=(self.n_slots, self._B))
        # element index offset in each slot
        self.slot_start = ti.field(ezsim.ti_int, shape=(self.n_slots, self._B))
        self.cur_cnt = ti.field(ezsim.ti_int, shape=self._B)
    ...
    def pos_to_grid(self, pos):
        return ti.floor(pos / self.cell_size, ezsim.ti_int)
