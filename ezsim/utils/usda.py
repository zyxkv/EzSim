    ...
    if shader_id == "UsdPreviewSurface":
        ...
        if texture is not None:
            ...
            if output_name == "r":
                ...
                ezsim.raise_exception(f"Invalid output channel for UsdUVTexture: {output_name}.")
        else:
            texture_image = None
...
def parse_usd_material(material, surface):
    ...
    for surface_output in surface_outputs:
        ...
        #         break
        #     ezsim.logger.warning(f"Fail to parse Shader {surface_shader.GetPath()} with ID {shader_id}.")
        #     continue
    ...
    if require_bake:
        ...
        ezsim.logger.debug(f"Material require baking:\n{candidates_str}")
...
def replace_asset_symlinks(stage):
    ...
    for asset_path in map(Path, asset_paths):
        ...
        if real_path.is_file():
            ezsim.logger.warning(f"Replacing symlink {asset_path} with real file {real_path}.")
...
def decompress_usdz(usdz_path):
    ...
    if not root_file.lower().endswith(ezsim.options.morphs.USD_FORMATS[:-1]):
        ezsim.raise_exception(f"Invalid usdz root file: {root_file}")
    ...
    if not os.path.exists(root_path):
        ...
        ezsim.logger.warning(f"USDZ file {usdz_path} decompressed to {root_path}.")
    else:
        ezsim.logger.info(f"Decompressed assets detected and used: {root_path}.")
    return root_path

def parse_mesh_usd(path, group_by_material, scale, surface, bake_cache=True):
    if path.lower().endswith(ezsim.options.morphs.USD_FORMATS[-1]):
    ...
    if bake_cache and os.path.exists(baked_path):
        ...
        ezsim.logger.info(f"Baked assets detected and used: {path}")
    ...
    if baked_materials:
        ...
        if device.type == "cpu":
            try:
                device, *_ = ezsim.utils.get_device(ezsim.cuda)
            except ezsim.EzSimException as e:
                ezsim.raise_exception_from("USD baking requires CUDA GPU.", e)
        ...
        commands = [
            ...
            *map(str, baked_materials.values()),
            ...
            str(device.index),
            "--log_level",
            logging.getLevelName(ezsim.logger.level).lower(),
        ]
        ezsim.logger.debug(f"Execute: {' '.join(commands)}")
        ...
        try:
            ...
            if result.stdout:
                ezsim.logger.debug(result.stdout)
            if result.stderr:
                ezsim.logger.warning(result.stderr)
        except (subprocess.CalledProcessError, OSError) as e:
            ezsim.logger.warning(f"Baking process failed: {e} (Note that USD baking may only support Python 3.10 now.)")

        if os.path.exists(baked_path):
            ezsim.logger.warning(f"USD materials baked to file {baked_path}")
    ...
    for i, prim in enumerate(stage.Traverse()):
        if prim.IsA(UsdGeom.Mesh):
            ...
            if normal_attr.HasValue():
                ...
                if normals.shape[0] != points.shape[0]:
                    if normals.shape[0] == faces.shape[0]:  # face varying meshes, adjacent faces do not share vertices
                        ...
                        ezsim.raise_exception(f"Size of normals mismatch for mesh {mesh_id} in usd file {path}.")
                ...
                if uv_var.IsDefined() and uv_var.HasValue():
                    ...
                    if uvs.shape[0] != points.shape[0]:
                        if uvs.shape[0] == faces.shape[0]:
                            ...
                            ezsim.raise_exception(f"Size of uvs mismatch for mesh {mesh_id} in usd file {path}.")
                    uvs[:, 1] = 1.0 - uvs[:, 1]
            ...
            if np.max(faces_vertex_counts) > 3:
                ...
                ezsim.logger.warning(f"Mesh {mesh_usd} has non-triangle faces.")
            else:
