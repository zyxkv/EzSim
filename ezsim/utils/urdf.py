...
def parse_urdf(morph, surface):
    ...
    for link, l_info, link_g_infos in zip(robot.links, l_infos, links_g_infos):
        ...
        for geom in (*link.collisions, *link.visuals):
            ...
            if isinstance(geom.geometry.geometry, urdfpy.Mesh):
                ...
                # One asset (.obj) can contain multiple meshes. Each mesh is one RigidGeom in ezsim.
                for tmesh in geom.geometry.meshes:
                    ...
                    mesh = ezsim.Mesh.from_trimesh(
                        tmesh,
                        scale=scale,
                        surface=ezsim.surfaces.Collision() if geom_is_col else surface,
                        metadata={
                ...
                # Each geometry primitive is one RigidGeom in ezsim.
                if isinstance(geom.geometry.geometry, urdfpy.Box):
                    ...
                    geom_type = ezsim.GEOM_TYPE.CYLINDER
                    geom_data = None
    ...
    for joint in robot.joints:
        ...
        if joint.joint_type == "fixed":
            ...
            j_info["type"] = ezsim.JOINT_TYPE.FIXED
            j_info["n_qs"] = 0
            j_info["n_dofs"] = 0
            ...
            j_info["type"] = ezsim.JOINT_TYPE.REVOLUTE
            j_info["n_qs"] = 1
            j_info["n_dofs"] = 1
            ...
            j_info["type"] = ezsim.JOINT_TYPE.PRISMATIC
            j_info["n_qs"] = 1
            j_info["n_dofs"] = 1
            j_info["init_qpos"] = np.zeros(1)
            ...
            ezsim.raise_exception(f"Unsupported URDF joint type: {joint.joint_type}")
