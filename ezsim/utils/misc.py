...
import ezsim
from ezsim.constants import backend as ezsim_backend
...
def raise_exception(msg="Something went wrong.") -> NoReturn:
    raise ezsim.EzSimException(msg)


def raise_exception_from(msg="Something went wrong.", cause=None) -> NoReturn:
    raise ezsim.EzSimException(msg) from cause
...
def assert_unbuilt(method):
    ...
    def wrapper(self, *args, **kwargs):
        if self.is_built:
            ezsim.raise_exception("Scene is already built.")
...
def assert_built(method):
    ...
    def wrapper(self, *args, **kwargs):
        if not self.is_built:
            ezsim.raise_exception(f"{type(self).__name__} is not built yet.")
        return method(self, *args, **kwargs)
...
def get_device(backend: ezsim_backend, device_idx: Optional[int] = None):
    if backend == ezsim_backend.cuda:
        if not torch.cuda.is_available():
            ezsim.raise_exception("torch cuda not available")
    ...
    elif backend == ezsim_backend.metal:
        if not torch.backends.mps.is_available():
            ezsim.raise_exception("metal device not available")

        # on mac, cpu and gpu are in the same device
        _, device_name, total_mem, _ = get_device(ezsim_backend.cpu)
        device = torch.device("mps", device_idx)

    elif backend == ezsim_backend.vulkan:
        if torch.cuda.is_available():
            device, device_name, total_mem, _ = get_device(ezsim_backend.cuda)
            ...
            logger = getattr(ezsim, "logger", None) or LOGGER
            logger.warning("No Intel XPU device available. Falling back to CPU for torch device.")
            device, device_name, total_mem, _ = get_device(ezsim_backend.cpu)

    elif backend == ezsim_backend.gpu:
        if torch.cuda.is_available():
            return get_device(ezsim_backend.cuda)
        elif get_platform() == "macOS":
            return get_device(ezsim_backend.metal)
        else:
            return get_device(ezsim_backend.vulkan)
...
def get_src_dir():
    return os.path.dirname(ezsim.__file__)


def get_gen_log_dir():
    current_time = datetime.datetime.now()
    unique_id = current_time.strftime("%Y%m%d_%H%M%S_%f")
    return os.path.join(os.path.dirname(ezsim.__file__), "gen", "logs", unique_id)
...
def get_cache_dir():
    ...
    if cache_dir is not None:
        return cache_dir
    return os.path.join(os.path.expanduser("~"), ".cache", "ezsim")
...
def clean_cache_files():
    folder = ezsim.utils.misc.get_cache_dir()
    try:
...
def assert_ezsim_tensor(x):
    if not isinstance(x, ezsim.Tensor):
        ezsim.raise_exception("Only accepts ezsim.Tensor.")


def to_ezsim_tensor(x):
    if isinstance(x, ezsim.Tensor):
        return x

    elif isinstance(x, list):
        return ezsim.from_numpy(np.array(x))

    elif isinstance(x, np.ndarray):
        return ezsim.from_numpy(x)

    elif isinstance(x, torch.Tensor):
        return ezsim.Tensor(x)

    else:
        ezsim.raise_exception("Only accepts ezsim.Tensor, torch.Tensor, np.ndarray or List.")
...
def ti_field_to_torch(
    ...
    *,
    ...
    if not unsafe:
        ...
        if is_1D_batch:
            if transpose and row_mask is not None:
                ezsim.raise_exception("Cannot specify row mask for fields with 1D batch and `transpose=True`.")
            elif not transpose and col_mask is not None:
                ezsim.raise_exception("Cannot specify column mask for fields with 1D batch and `transpose=False`.")
        for i, mask in enumerate((col_mask if transpose else row_mask,) if is_1D_batch else (row_mask, col_mask)):
            if mask is None or isinstance(mask, slice):
                ...
                if not mask.ndim <= 1:
                    ezsim.raise_exception(f"Expecting 1D tensor for masks.")
            ...
            if is_out_of_bounds:
                ezsim.raise_exception("Masks are out-of-range.")
    ...
    if is_row_mask_tensor:
        _row_mask = torch.as_tensor(row_mask, dtype=ezsim.tc_int, device=ezsim.device)
    ...
    if is_col_mask_tensor:
        _col_mask = torch.as_tensor(col_mask, dtype=ezsim.tc_int, device=ezsim.device)
    ...
    is_metal = ezsim.device.type == "mps"
    ...
    if isinstance(field, ti.lang.ScalarField):
        if is_metal:
            ...
            out = torch.zeros(size=field_shape, dtype=tc_dtype, device=ezsim.device)
        ...
        if is_metal:
            ...
            out = torch.empty(field_shape + shape_ext, dtype=tc_dtype, device=ezsim.device)
        _matrix_to_ext_arr_fast(field, out, as_vector)
    if is_metal:
        out = out.to(ezsim.device)
    ti.sync()
        ...
        if not unsafe and is_out_of_bounds is None:
            for i, mask in enumerate((col_mask if transpose else row_mask,) if is_1D_batch else (row_mask, col_mask)):
                ...
                if not (0 <= mask[0] <= mask[-1] < field_shape[i]):
                    ezsim.raise_exception_from("Masks are out-of-range.", e)
