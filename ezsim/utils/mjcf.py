...
def build_model(xml, discard_visual, default_armature=None, merge_fixed_links=False, links_to_keep=()):
    if isinstance(xml, (str, Path)):
        ...
        try:
            ...
            ezsim.raise_exception_from(f"'{xml}' is not a valid XML file path or string.")
        ...
        raise ezsim.raise_exception(f"'{xml}' is not a valid MJCF file.")
...
def parse_xml(morph, surface):
    ...
    if isinstance(morph, (ezsim.morphs.URDF, ezsim.morphs.Drone)):
    ...
    # ezsim.logger.warning(f"(MJCF) Approximating tendon by joint actuator for `{j_info['name']}`")
    # if mj.ntendon:
    #     ezsim.logger.warning("(MJCF) Tendon not supported")
...
def parse_link(mj, i_l, scale):
    ...
    for i_j in range(jnt_adr, jnt_adr + max(jnt_num, 1)):
        ...
        if mj_type is None:
            ezsim_type = ezsim.JOINT_TYPE.FIXED
            n_qs, n_dofs = 0, 0
        elif mj_type == mujoco.mjtJoint.mjJNT_FREE:
            ezsim_type = ezsim.JOINT_TYPE.FREE
            n_qs, n_dofs = 7, 6
        elif mj_type == mujoco.mjtJoint.mjJNT_HINGE:
            ezsim_type = ezsim.JOINT_TYPE.REVOLUTE
            n_qs, n_dofs = 1, 1
        elif mj_type == mujoco.mjtJoint.mjJNT_SLIDE:
            ezsim_type = ezsim.JOINT_TYPE.PRISMATIC
            n_qs, n_dofs = 1, 1
        elif mj_type == mujoco.mjtJoint.mjJNT_BALL:
            ezsim_type = ezsim.JOINT_TYPE.SPHERICAL
            n_qs, n_dofs = 4, 3
        else:
            ezsim.raise_exception(f"Unsupported MJCF joint type: {mj_type}")
        j_info["type"], j_info["n_qs"], j_info["n_dofs"] = ezsim_type, n_qs, n_dofs
        ...
        #     ezsim.logger.warning("(MJCF) Friction loss at DoF-level not supported.")
        ...
        if ezsim_type == ezsim.JOINT_TYPE.FIXED:
        ...
        elif ezsim_type == ezsim.JOINT_TYPE.FREE:
            if mj_stiffness > 0.0:
                raise ezsim.raise_exception("(MJCF) Joint stiffness not supported for free joints")
        ...
        elif ezsim_type == ezsim.JOINT_TYPE.SPHERICAL:
            if mj_is_limited:
                ezsim.logger.warning("(MJCF) Joint limit ignored for ball joints")
            ...
            if ezsim_type == ezsim.JOINT_TYPE.REVOLUTE:
            ...
            else:  # ezsim_type == ezsim.JOINT_TYPE.PRISMATIC:
        ...
        # if (mj.dof_frictionloss[mj_dof_offset : (mj_dof_offset + n_dofs)] > 0.0).any():
        #     ezsim.logger.warning("(MJCF) Joint Coulomb friction not supported.")

        # Parsing actuator parameters
        j_info["dofs_kp"] = np.zeros((n_dofs,), dtype=ezsim.np_float)
        j_info["dofs_kv"] = np.zeros((n_dofs,), dtype=ezsim.np_float)
        ...
        try:
            ...
            ezsim.logger.warning(f"(MJCF) Failed to parse actuator for joint `{j_info['name']}`.")

        if i_a >= 0:
            if mj.actuator_dyntype[i_a] != mujoco.mjtDyn.mjDYN_NONE:
                ezsim.logger.warning(f"(MJCF) Actuator internal dynamics not supported")
            gaintype = mujoco.mjtGain(mj.actuator_gaintype[i_a])
            if gaintype != mujoco.mjtGain.mjGAIN_FIXED:
                ezsim.logger.warning(f"(MJCF) Actuator control gain of type '{gaintype}' not supported")
            ...
            if biastype not in (mujoco.mjtBias.mjBIAS_NONE, mujoco.mjtBias.mjBIAS_AFFINE):
                ezsim.logger.warning(f"(MJCF) Actuator control bias of type '{biastype}' not supported")
            if n_dofs > 1 and not (mj.actuator_gear[i_a, :n_dofs] == 1.0).all():
                ezsim.logger.warning("(MJCF) Actuator transmission gear is only supported of 1DoF joints")
        ...
        elif ezsim_type not in (ezsim.JOINT_TYPE.FIXED, ezsim.JOINT_TYPE.FREE):
            ezsim.logger.debug(f"(MJCF) No actuator found for joint `{j_info['name']}`")
...
def parse_geom(mj, i_g, scale, surface, xml_path):
    ...
    if mj_geom.type == mujoco.mjtGeom.mjGEOM_PLANE:
        ...
        ezsim_type = ezsim.GEOM_TYPE.PLANE
        ...
        ezsim_type = ezsim.GEOM_TYPE.SPHERE
        ...
        ezsim_type = ezsim.GEOM_TYPE.ELLIPSOID
        geom_data = geom_size
        ...
        ezsim_type = ezsim.GEOM_TYPE.CAPSULE
        ...
        ezsim_type = ezsim.GEOM_TYPE.MESH
        geom_data = None
        ...
        ezsim.logger.warning(f"Unsupported MJCF geom type '{mj_geom.type}'.")
        return None

    mesh = ezsim.Mesh.from_trimesh(
        tmesh, scale=scale, surface=ezsim.surfaces.Collision() if is_col else surface, metadata=metadata
    )
