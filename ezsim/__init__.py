...
from .constants import EZSIM_ARCH, TI_ARCH
from .constants import backend as ezsim_backend
...
def init(
    ...
    if backend is None:
        if debug:
            backend = ezsim_backend.cpu
        else:
            backend = ezsim_backend.gpu
    ...
    taichi_kwargs = {}
    if ezsim.logger.level == _logging.CRITICAL:
        taichi_kwargs.update(log_level=ti.CRITICAL)
    elif ezsim.logger.level == _logging.ERROR:
        taichi_kwargs.update(log_level=ti.ERROR)
    elif ezsim.logger.level == _logging.WARNING:
        taichi_kwargs.update(log_level=ti.WARN)
    elif ezsim.logger.level == _logging.INFO:
        taichi_kwargs.update(log_level=ti.INFO)
    elif ezsim.logger.level == _logging.DEBUG:
        taichi_kwargs.update(log_level=ti.INFO)
    if debug:
        if backend == ezsim_backend.cpu:
        ...
        logger.info("Beware running EzSim in debug mode dramatically reduces runtime speed.")
    ...
    with redirect_stdout(_ti_outputs):
        ti.init(
            ...
            debug=debug and backend == ezsim.cpu and (os.environ.get("TI_DEBUG") != "0"),
    ...
    if backend != ezsim.cpu and ti_arch in (ti._lib.core.Arch.arm64, ti._lib.core.Arch.x64):
        device, device_name, total_mem, backend = get_device(ezsim.cpu)
    ...
    logger.info(
        f"♾️  EzSim initialized. 🔖 version: ~~<{__version__}>~~, 🌱 seed: ~~<{seed}>~~, 📏 precision: '~~<{precision}>~~', 🐛 debug: ~~<{debug}>~~, 🎨 theme: '~~<{theme}>~~'."
    )
...
def destroy():
    ...
    ezsim.init() needs to be called again to reinitialize the system after destroy.
    """
    ...
    # Do not consider EzSim as initialized at this point
    _initialized = False
    ...
    global logger
    if logger:
        logger.info("🌌 Exiting EzSim and caching compiled kernels...")
...
def _display_greeting(INFO_length):
    ...
    bar_width = wave_width * 2 + 7
    wave = ("  " * wave_width)[:wave_width]
    global logger
    logger.info(f"~<╭{'─'*(bar_width)}╮>~")
    logger.info(f"~<│{wave}>~ ~~~~<EzSim>~~~~ ~<{wave}│>~")
    logger.info(f"~<╰{'─'*(bar_width)}╯>~")
...
class EzSimException(Exception):
...
for name, member in ezsim_backend.__members__.items():
    globals()[name] = member
