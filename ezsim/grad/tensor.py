...
import ezsim
from ezsim.utils.repr import brief


class Tensor(torch.Tensor):
    """
    This is the ezsim customization of torch's Tensor datatype. This allows our customizations, a few safety checks and also more elegant end-to-end gradient flow.
    """
    ...
    def __torch_function__(cls, func, types, args=(), kwargs=None):
        ...
        # NOTE: This is a temporary hack. Due to some unknown reason, the unbind operation is super slow for ezsim.Tensor when requires_grad is True. This unbind operations seems to be the last internally called operation inside pytorch when we perform tensor operations. However, magically, disabling it (returning None) doesn't seem to affect anything, but helps bypass the time spent on it. Need to look into this further.
    ...
    def backward(self, *args, **kwargs):
        ...
        # if it's a tensor derived from a ezsim Scene, we let the gradient keep flowing
    ...
    def _backward_from_ti(self, ti_kernel, *args):
        temp_grad = ezsim.zeros_like(self, requires_grad=False)
    ...
    def assert_contiguous(self):
        if not self.is_contiguous():
            ezsim.raise_exception("Tensor not contiguogs.")
    ...
    def __repr__(self):
        return (
            super().__repr__().replace("Tensor", "ezsim.tensor")[:-1]
