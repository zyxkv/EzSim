...
from ezsim.engine.force_fields import ForceField
from ezsim.engine.materials.base import Material
from ezsim.engine.entities import Emitter
from ezsim.engine.states.solvers import SimState
from ezsim.engine.simulator import Simulator
from ezsim.options import (
    AvatarOptions,
    BaseCouplerOptions,
...
class Scene(RBC):
    """
    A ``ezsim.Scene`` object wraps all components in a simulation environment, including a simulator (containing multiple physics solvers), entities, and a visualizer (controlling both the viewer and all the cameras).
    ...
    coupler_options : ezsim.options.CouplerOptions
        The options configuring the `coupler` between different solvers.
    tool_options : ezsim.options.ToolOptions
    ...
    avatar_options : ezsim.options.AvatarOptions
    ...
    def __init__(
        ...
        ezsim.logger.info(f"Scene ~~~<{self._uid}>~~~ created.")
    ...
    def _validate_options(
        ...
        if not isinstance(coupler_options, BaseCouplerOptions):
            ezsim.raise_exception("`coupler_options` should be an instance of `BaseCouplerOptions`.")

        if not isinstance(tool_options, ToolOptions):
            ezsim.raise_exception("`tool_options` should be an instance of `ToolOptions`.")

        if not isinstance(rigid_options, RigidOptions):
            ezsim.raise_exception("`rigid_options` should be an instance of `RigidOptions`.")

        if not isinstance(avatar_options, AvatarOptions):
            ezsim.raise_exception("`avatar_options` should be an instance of `AvatarOptions`.")

        if not isinstance(mpm_options, MPMOptions):
            ezsim.raise_exception("`mpm_options` should be an instance of `MPMOptions`.")

        if not isinstance(sph_options, SPHOptions):
            ezsim.raise_exception("`sph_options` should be an instance of `SPHOptions`.")

        if not isinstance(fem_options, FEMOptions):
            ezsim.raise_exception("`fem_options` should be an instance of `FEMOptions`.")

        if not isinstance(sf_options, SFOptions):
            ezsim.raise_exception("`sf_options` should be an instance of `SFOptions`.")

        if not isinstance(pbd_options, PBDOptions):
            ezsim.raise_exception("`pbd_options` should be an instance of `PBDOptions`.")

        if not isinstance(vis_options, VisOptions):
            ezsim.raise_exception("`vis_options` should be an instance of `VisOptions`.")

        if not isinstance(viewer_options, ViewerOptions):
            ezsim.raise_exception("`viewer_options` should be an instance of `ViewerOptions`.")
    ...
    def add_entity(
        ...
        material : ezsim.materials.Material | None, optional
            The material of the entity. If None, use ``ezsim.materials.Rigid()``.
        surface : ezsim.surfaces.Surface | None, optional
            The surface of the entity. If None, use ``ezsim.surfaces.Default()``.
        ...
        if isinstance(material, ezsim.materials.Rigid):
            ...
            if isinstance(morph, ezsim.morphs.Primitive):
                material._sdf_max_res = 32

        # some morph should not smooth surface normal
        if isinstance(morph, (ezsim.morphs.Box, ezsim.morphs.Cylinder, ezsim.morphs.Terrain)):
            surface.smooth = False

        if isinstance(morph, (ezsim.morphs.URDF, ezsim.morphs.MJCF, ezsim.morphs.Terrain)):
            if not isinstance(material, (ezsim.materials.Rigid, ezsim.materials.Avatar, ezsim.materials.Hybrid)):
                ezsim.raise_exception(f"Unsupported material for morph: {material} and {morph}.")

        if surface.double_sided is None:
            if isinstance(material, ezsim.materials.PBD.Cloth):
        ...
        if isinstance(material, (ezsim.materials.Rigid, ezsim.materials.Avatar, ezsim.materials.Tool)):
            ...
            material,
            (
                ezsim.materials.PBD.Liquid,
                ezsim.materials.PBD.Particle,
                ezsim.materials.MPM.Liquid,
                ezsim.materials.MPM.Sand,
                ezsim.materials.MPM.Snow,
                ezsim.materials.SPH.Liquid,
            ),
        ):
            if surface.vis_mode is None:
        ...
        elif isinstance(material, (ezsim.materials.SF.Smoke)):
            if surface.vis_mode is None:
                surface.vis_mode = "particle"
        ...
        elif isinstance(material, (ezsim.materials.PBD.Base, ezsim.materials.MPM.Base, ezsim.materials.SPH.Base)):
            if surface.vis_mode is None:
                surface.vis_mode = "visual"
        ...
        elif isinstance(material, (ezsim.materials.FEM.Base)):
            if surface.vis_mode is None:
                surface.vis_mode = "visual"
        ...
        elif isinstance(material, (ezsim.materials.Hybrid)):  # determine the visual of the outer soft part
            if surface.vis_mode is None:
                surface.vis_mode = "particle"
        ...
        if isinstance(morph, ezsim.morphs.FileMorph):
            # Rigid entities will convexify geom by default
            if morph.convexify is None:
                morph.convexify = isinstance(material, (ezsim.materials.Rigid, ezsim.materials.Avatar))
    ...
    def link_entities(
        ...
        """
        if not isinstance(parent_entity, ezsim.engine.entities.RigidEntity):
            ezsim.raise_exception("Currently only rigid entities are supported for merging.")
        if not isinstance(child_entity, ezsim.engine.entities.RigidEntity):
            ezsim.raise_exception("Currently only rigid entities are supported for merging.")
    ...
    def add_emitter(
        ...
        Parameters
        ----------
        material : ezsim.materials.Material
            The material of the fluid to be emitted. Must be an instance of `ezsim.materials.MPM.Base` or `ezsim.materials.SPH.Base`.
        max_particles : int
            The maximum number of particles that can be emitted by the emitter. Particles will be recycled once this limit is reached.
        surface : ezsim.surfaces.Surface | None, optional
            The surface of the emitter. If None, use ``ezsim.surfaces.Default(color=(0.6, 0.8, 1.0, 1.0))``.

        Returns
        -------
        emitter : ezsim.Emitter
            The created emitter object.

        """
        if self.requires_grad:
            ezsim.raise_exception("Emitter is not supported in differentiable mode.")

        if not isinstance(
            material, (ezsim.materials.MPM.Base, ezsim.materials.SPH.Base, ezsim.materials.PBD.Particle, ezsim.materials.PBD.Liquid)
        ):
            ezsim.raise_exception(
                "Non-supported material for emitter. Supported materials are: `ezsim.materials.MPM.Base`, `ezsim.materials.SPH.Base`, `ezsim.materials.PBD.Particle`, `ezsim.materials.PBD.Liquid`."
            )

        if surface is None:
            surface = ezsim.surfaces.Default(color=(0.6, 0.8, 1.0, 1.0))

        emitter = Emitter(max_particles)
        entity = self.add_entity(
            morph=ezsim.morphs.Nowhere(n_particles=max_particles),
            material=material,
            surface=surface,
        )
    ...
    def add_force_field(self, force_field: ForceField):
        ...
        Parameters
        ----------
        force_field : ezsim.force_fields.ForceField
    ...
    def build(
        ...
        """
        with ezsim.logger.timer(f"Building scene ~~~<{self._uid}>~~~..."):
        ...
        if compile_kernels:
            with ezsim.logger.timer("Compiling simulation kernels..."):
                self._sim.step()
                self._reset()

        # visualizer
        with ezsim.logger.timer("Building visualizer..."):
        ...
        ezsim.global_scene_list.add(self)
    ...
    def _parallelize(
        ...
        self._envs_idx = torch.arange(self._B, dtype=ezsim.tc_int, device=ezsim.device)
        ...
        if not isinstance(env_spacing, (list, tuple)) or len(env_spacing) != 2:
            ezsim.raise_exception("`env_spacing` should be a tuple of length 2.")
        ...
        self.envs_offset = np.stack((offset_x, offset_y, offset_z), axis=-1, dtype=ezsim.np_float)
        ...
        """
        if ezsim.backend == ezsim.cpu:
            self._para_level = ezsim.PARA_LEVEL.NEVER
        elif self.n_envs == 0:
            self._para_level = ezsim.PARA_LEVEL.PARTIAL
        else:
            self._para_level = ezsim.PARA_LEVEL.ALL
    ...
    def reset(self, state: SimState | None = None, envs_idx=None):
        ...
        """
        ezsim.logger.debug(f"Resetting Scene ~~~<{self._uid}>~~~.")
    ...
    def step(self, update_visualizer=True, refresh_visualizer=True):
        ...
        if not self._forward_ready:
            ezsim.raise_exception("Forward simulation not allowed after backward pass. Please reset scene state.")
    ...
    def draw_debug_line(self, start, end, radius=0.002, color=(1.0, 0.0, 0.0, 0.5)):
        ...
        Returns
        -------
        node : ezsim.ext.pyrender.mesh.Mesh
    ...
    def draw_debug_path(self, qposs, entity, link_idx=-1, density=0.3, frame_scaling=1.0):
        ...
        entity : ezsim.engine.entities.RigidEntity
    ...
    def _backward(self):
        ...
        if not self._backward_ready:
            ezsim.raise_exception("Multiple backward calls not allowed.")
    ...
    def _sanitize_envs_idx(self, envs_idx, *, unsafe=False):
        ...
        if self.n_envs == 0:
            ezsim.raise_exception("`envs_idx` is not supported for non-parallelized scene.")
        ...
        _envs_idx = torch.as_tensor(envs_idx, dtype=ezsim.tc_int, device=ezsim.device).contiguous()
        if _envs_idx is not envs_idx:
            ezsim.logger.debug(ALLOCATE_TENSOR_WARNING)
        _envs_idx = torch.atleast_1d(_envs_idx)

        if _envs_idx.ndim != 1:
            ezsim.raise_exception("Expecting a 1D tensor for `envs_idx`.")

        if (_envs_idx < 0).any() or (_envs_idx >= self.n_envs).any():
            ezsim.raise_exception("`envs_idx` exceeds valid range.")
