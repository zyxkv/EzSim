...
class SAPCoupler(RBC):
    ...
    def compute_inertia_elastic_gradient_alpha(self, i_step: ti.i32):
        ...
        for i_b, i_v in ti.ndrange(self._B, self.fem_solver.n_vertices):
            if not self.batch_linesearch_active[i_b]:
                continue
            self.linesearch_state.dell_dalpha[i_b] += dp[i_b, i_v].dot(v[i_b, i_v] - v_star[i_step + 1, i_v, i_b]) #FIXME:? why ib,iv -> iv,ib
    ...
    def compute_inertia_elastic_energy_alpha(self, i_step: ti.i32, energy: ti.template()):
        ...
        for i_b, i_v in ti.ndrange(self._B, self.fem_solver.n_vertices):
            if not self.batch_linesearch_active[i_b]:
                continue
            energy[i_b] += alpha[i_b] * dp[i_b, i_v].dot(v[i_b, i_v] - v_star[i_step + 1, i_v, i_b]) #FIXME:? why ib,iv -> iv,ib
...
class FEMSelfTetContact(BaseContact):
    ...
    def compute_Jx(self, i_p, x):
        ...
        Jx = ti.Vector(
            [Jx.dot(pairs[i_p].tangent0), Jx.dot(pairs[i_p].tangent1), Jx.dot(pairs[i_p].normal)], dt=ezsim.ti_float
        )
        return Jx
