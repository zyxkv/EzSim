...
import ezsim
from ezsim.options.solvers import SAPCouplerOptions
from ezsim.repr_base import RBC
from ezsim.engine.bvh import AABB, LBVH, FEMSurfaceTetLBVH
from ezsim.utils.element import mesh_to_elements, split_all_surface_tets
import ezsim.utils.geom as gu
from ezsim.constants import IntEnum
from ezsim.engine.solvers.rigid.rigid_solver_decomp import func_update_all_verts
...
def tet_barycentric(p, tet_vertices):
    ...
    return ti.Vector([b0, b1, b2, b3], dt=ezsim.ti_float)
...
class SAPCoupler(RBC):
    ...
    def _init_hydroelastic_fem_fields_and_info(self):
        self.fem_pressure = ti.field(ezsim.ti_float, shape=(self.fem_solver.n_vertices))
        ...
        self.fem_pressure_gradient = ti.field(ezsim.ti_vec3, shape=(self.fem_solver._B, self.fem_solver.n_elements))

        # Lookup table for marching tetrahedra edges
        self.MarchingTetsEdgeTable = ti.field(ezsim.ti_ivec4, shape=len(MARCHING_TETS_EDGE_TABLE))
        self.MarchingTetsEdgeTable.from_numpy(np.array(MARCHING_TETS_EDGE_TABLE, dtype=np.int32))

        self.TetEdges = ti.field(ezsim.ti_ivec2, shape=len(TET_EDGES))
    ...
    def _init_sap_fields(self):
        self.batch_active = ti.field(dtype=ezsim.ti_bool, shape=self.sim._B, needs_grad=False)
        # self.v = ti.field(ezsim.ti_vec3, shape=(self.fem_solver._B, self.fem_solver.n_vertices))
        # self.v_diff = ti.field(ezsim.ti_vec3, shape=(self.fem_solver._B, self.fem_solver.n_vertices))
        # self.gradient = ti.field(ezsim.ti_vec3, shape=(self.fem_solver._B, self.fem_solver.n_vertices))

        sap_state = ti.types.struct(
            gradient_norm=ezsim.ti_float,  # norm of the gradient
            momentum_norm=ezsim.ti_float,  # norm of the momentum
            impulse_norm=ezsim.ti_float,  # norm of the impulse
        )

        self.sap_state = sap_state.field(shape=self.sim._B, needs_grad=False, layout=ti.Layout.SOA)

        # sap_state_v = ti.types.struct(
        #     impulse=ezsim.ti_vec3,  # impulse vector
        # )
    ...
    def _init_fem_fields(self):
        fem_state_v = ti.types.struct(
            v=ezsim.ti_vec3,  # vertex velocity
            v_diff=ezsim.ti_vec3,  # difference between current and previous velocity
            gradient=ezsim.ti_vec3,  # gradient vector
             impulse=ezsim.ti_vec3,  # impulse vector
        )
        ...
        pcg_fem_state_v = ti.types.struct(
            diag3x3=ezsim.ti_mat3,  # diagonal 3-by-3 block of the hessian
            prec=ezsim.ti_mat3,  # preconditioner
            x=ezsim.ti_vec3,  # solution vector
            r=ezsim.ti_vec3,  # residual vector
            z=ezsim.ti_vec3,  # preconditioned residual vector
            p=ezsim.ti_vec3,  # search direction vector
            Ap=ezsim.ti_vec3,  # matrix-vector product
        )
        ...
        linesearch_fem_state_v = ti.types.struct(
            x_prev=ezsim.ti_vec3,  # solution vector
            dp=ezsim.ti_vec3,  # A @ dv
        )
    ...
    def _init_rigid_fields(self):
        rigid_state_dof = ti.types.struct(
            v=ezsim.ti_float,  # vertex velocity
            v_diff=ezsim.ti_float,  # difference between current and previous velocity
            mass_v_diff=ezsim.ti_float,  # mass weighted difference between current and previous velocity
            gradient=ezsim.ti_float,  # gradient vector
            impulse=ezsim.ti_float,  # impulse vector
        )
        ...
        pcg_rigid_state_dof = ti.types.struct(
            x=ezsim.ti_float,  # solution vector
            r=ezsim.ti_float,  # residual vector
            z=ezsim.ti_float,  # preconditioned residual vector
            p=ezsim.ti_float,  # search direction vector
            Ap=ezsim.ti_float,  # matrix-vector product
        )
        ...
        linesearch_rigid_state_dof = ti.types.struct(
            x_prev=ezsim.ti_float,  # solution vector
            dp=ezsim.ti_float,  # A @ dv
        )
    ...
    def _init_pcg_fields(self):
        self.batch_pcg_active = ti.field(dtype=ezsim.ti_bool, shape=self.sim._B, needs_grad=False)

        pcg_state = ti.types.struct(
            rTr=ezsim.ti_float,
            rTz=ezsim.ti_float,
            rTr_new=ezsim.ti_float,
            rTz_new=ezsim.ti_float,
            pTAp=ezsim.ti_float,
            alpha=ezsim.ti_float,
            beta=ezsim.ti_float,
        )
    ...
    def _init_linesearch_fields(self):
        self.batch_linesearch_active = ti.field(dtype=ezsim.ti_bool, shape=self.sim._B, needs_grad=False)

        linesearch_state = ti.types.struct(
            prev_energy=ezsim.ti_float,
            energy=ezsim.ti_float,
            step_size=ezsim.ti_float,
            m=ezsim.ti_float,
            dell_dalpha=ezsim.ti_float,  # first derivative of the total energy w.r.t. alpha
            d2ellA_dalpha2=ezsim.ti_float,  # second derivative of the dynamic energy w.r.t. alpha
            d2ell_dalpha2=ezsim.ti_float,  # second derivative of the total energy w.r.t. alpha
            dell_scale=ezsim.ti_float,  # scale factor for the first derivative
            alpha_min=ezsim.ti_float,  # minimum stepsize value
            alpha_max=ezsim.ti_float,  # maximum stepsize value
            alpha_tol=ezsim.ti_float,  # stepsize tolerance for convergence
            f_lower=ezsim.ti_float,  # minimum f value
            f_upper=ezsim.ti_float,  # maximum f value
            f=ezsim.ti_float,  # f value
            df=ezsim.ti_float,  # f gradient
            minus_dalpha=ezsim.ti_float,  # negative stepsize
            minus_dalpha_prev=ezsim.ti_float,  # previous negative stepsize
        )
    ...
    def couple_grad(self, i_step):
        ezsim.raise_exception("couple_grad is not available for SAPCoupler. Please use LegacyCoupler instead.")
    ...
    def fem_compute_pressure_gradient(self, i_step: ti.i32):
        for i_b, i_e in ti.ndrange(self.fem_solver._B, self.fem_solver.n_elements):
            ...
            for i in ti.static(range(4)):
                ...
                if ti.abs(signed_volume) > ezsim.EPS:
    ...
    def compute_fem_matrix_vector_product(self, src, dst, active):
        ...
        for i_b, i_e in ti.ndrange(self.fem_solver._B, self.fem_solver.n_elements):
            ...
            S = ti.Matrix.zero(ezsim.ti_float, 4, 3)
            S[:3, :] = B
            S[3, :] = -B[0, :] - B[1, :] - B[2, :]
            i_vs = self.fem_solver.elements_i[i_e].el2v
            ...
            if ti.static(self.fem_solver._enable_vertex_constraints):
                for i in ti.static(range(4)):
                    if self.fem_solver.vertex_constraints.is_constrained[i_vs[i], i_b]:
                        S[i, :] = ti.Vector.zero(ezsim.ti_float, 3)
...
class BaseContactHandler(RBC):
    ...
    def __init__(
        ...
        self.n_contact_pairs = ti.field(ezsim.ti_int, shape=())
        self.sap_contact_info_type = ti.types.struct(
            k=ezsim.ti_float,  # contact stiffness
            phi0=ezsim.ti_float,  # initial signed distance
            Rn=ezsim.ti_float,  # Regularization for normal
            Rt=ezsim.ti_float,  # Regularization for tangential
            Rn_inv=ezsim.ti_float,  # Inverse of Rn
            Rt_inv=ezsim.ti_float,  # Inverse of Rt
            vn_hat=ezsim.ti_float,  # Stablization for normal velocity
            mu=ezsim.ti_float,  # friction coefficient
            mu_hat=ezsim.ti_float,  # friction coefficient regularized
            mu_factor=ezsim.ti_float,  # friction coefficient factor, 1/(1+mu_tilde**2)
            energy=ezsim.ti_float,  # energy
            gamma=ezsim.ti_vec3,  # contact impulse
            G=ezsim.ti_mat3,  # Hessian matrix
            dvc=ezsim.ti_vec3,  # velocity change at contact point, for exact line search
        )
    ...
    def compute_contact_gamma_G(self, sap_info, i_p, vc):
        ...
        if contact_mode == ContactMode.STICK:
            ...
            Pperp = ti.Matrix.identity(ezsim.ti_float, 2) - P
...
class FEMFloorTetContactHandler(FEMContactHandler):
    ...
    def __init__(
        ...
        self.contact_candidate_type = ti.types.struct(
            batch_idx=ezsim.ti_int,  # batch index
            geom_idx=ezsim.ti_int,  # index of the FEM element
            intersection_code=ezsim.ti_int,  # intersection code for the element
            distance=ezsim.ti_vec4,  # distance vector for the element
        )
        self.n_contact_candidates = ti.field(ezsim.ti_int, shape=())
        ...
        self.contact_pair_type = ti.types.struct(
            batch_idx=ezsim.ti_int,  # batch index
            geom_idx=ezsim.ti_int,  # index of the FEM element
            barycentric=ezsim.ti_vec4,  # barycentric coordinates of the contact point
            contact_pos=ezsim.ti_vec3,  # contact position
    ...
    def detection(self, f: ti.i32):
        ...
        for i_b, i_e in ti.ndrange(self.fem_solver._B, self.fem_solver.n_elements):
            ...
            distance = ti.Vector.zero(ezsim.ti_float, 4)
            for i in ti.static(range(4)):
        ...
        for i_c in range(self.n_contact_candidates[None]):
            ...
            tet_vertices = ti.Matrix.zero(ezsim.ti_float, 3, 4)  # 4 vertices
            tet_pressures = ti.Vector.zero(ezsim.ti_float, 4)  # pressures at the vertices
            for i in ti.static(range(4)):
                i_v = self.fem_solver.elements_i[i_e].el2v[i]
                tet_vertices[:, i] = self.fem_solver.elements_v[f, i_v, i_b].pos
                tet_pressures[i] = self.coupler.fem_pressure[i_v]

            polygon_vertices = ti.Matrix.zero(ezsim.ti_float, 3, 4)  # 3 or 4 vertices
            total_area = ezsim.EPS  # avoid division by zero
            total_area_weighted_centroid = ti.Vector.zero(ezsim.ti_float, 3)
            for i in ti.static(range(4)):
            ...
            if total_area < ezsim.EPS or rigid_g < ezsim.EPS:
                continue
    ...
    def compute_Jx(self, i_p, x):
        ...
        Jx = ti.Vector.zero(ezsim.ti_float, 3)
        for i in ti.static(range(4)):
    ...
    def compute_delassus(self, i_p):
        i_b = self.contact_pairs[i_p].batch_idx
        i_g = self.contact_pairs[i_p].geom_idx
        W = ti.Matrix.zero(ezsim.ti_float, 3, 3)
        # W = sum (JA^-1J^T)
...
class FEMSelfTetContactHandler(FEMContactHandler):
    ...
    def __init__(
        ...
        self.contact_candidate_type = ti.types.struct(
            batch_idx=ezsim.ti_int,  # batch index
            geom_idx0=ezsim.ti_int,  # index of the FEM element0
            intersection_code0=ezsim.ti_int,  # intersection code for element0
            geom_idx1=ezsim.ti_int,  # index of the FEM element1
            normal=ezsim.ti_vec3,  # contact plane normal
            x=ezsim.ti_vec3,  # a point on the contact plane
            distance0=ezsim.ti_vec4,  # distance vector for element0
        )
        self.n_contact_candidates = ti.field(ezsim.ti_int, shape=())
        ...
        self.contact_pair_type = ti.types.struct(
            batch_idx=ezsim.ti_int,  # batch index
            normal=ezsim.ti_vec3,  # contact plane normal
            tangent0=ezsim.ti_vec3,  # contact plane tangent0
            tangent1=ezsim.ti_vec3,  # contact plane tangent1
            geom_idx0=ezsim.ti_int,  # index of the FEM element0
            geom_idx1=ezsim.ti_int,  # index of the FEM element1
            barycentric0=ezsim.ti_vec4,  # barycentric coordinates of the contact point in tet 0
            barycentric1=ezsim.ti_vec4,  # barycentric coordinates of the contact point in tet 1
            contact_pos=ezsim.ti_vec3,  # contact position
            sap_info=self.sap_contact_info_type,  # contact info
        )
        self.max_contact_pairs = self.fem_solver.n_surface_elements * self.fem_solver._B
    ...
    def compute_candidates(self, f: ti.i32):
        ...
        for i_r in ti.ndrange(self.coupler.fem_surface_tet_bvh.query_result_count[None]):
            ...
            g0_norm = g0.norm()
            g1_norm = g1.norm()
            if g0_norm < ezsim.EPS or g1_norm < ezsim.EPS:
                continue
            ...
            distance0 = ti.Vector.zero(ezsim.ti_float, 4)
            intersection_code1 = ti.int32(0)
            distance1 = ti.Vector.zero(ezsim.ti_float, 4)
            for i in ti.static(range(4)):
    ...
    def compute_pairs(self, i_step: ti.i32):
        ...
        overflow = False
        sap_info = ti.static(self.contact_pairs.sap_info)
        normal_signs = ti.Vector([1.0, -1.0, 1.0, -1.0], dt=ezsim.ti_float)  # make normal point outward
        self.n_contact_pairs[None] = 0
        for i_c in range(self.n_contact_candidates[None]):
            ...
            tet_vertices0 = ti.Matrix.zero(ezsim.ti_float, 3, 4)  # 4 vertices of tet 0
            tet_pressures0 = ti.Vector.zero(ezsim.ti_float, 4)  # pressures at the vertices of tet 0
            tet_vertices1 = ti.Matrix.zero(ezsim.ti_float, 3, 4)  # 4 vertices of tet 1
            for i in ti.static(range(4)):
            ...
            polygon_vertices = ti.Matrix.zero(ezsim.ti_float, 3, 8)  # maximum 8 vertices
            polygon_n_vertices = ezsim.ti_int(0)
            clipped_vertices = ti.Matrix.zero(ezsim.ti_float, 3, 8)  # maximum 8 vertices
            clipped_n_vertices = ezsim.ti_int(0)
            for i in range(4):
            ...
            for face in range(4):
                ...
                distances = ti.Vector.zero(ezsim.ti_float, 8)
...
class FEMFloorVertContactHandler(FEMContactHandler):
    ...
    def __init__(
        ...
        self.contact_pair_type = ti.types.struct(
            batch_idx=ezsim.ti_int,  # batch index
            geom_idx=ezsim.ti_int,  # index of the vertex
            contact_pos=ezsim.ti_vec3,  # contact position
            sap_info=self.sap_contact_info_type,  # contact info
        )
        self.max_contact_pairs = self.fem_solver.n_surface_elements * self.fem_solver._B
...
class RigidFloorVertContactHandler(RigidContactHandler):
    def __init__(
        ...
        self.contact_pair_type = ti.types.struct(
            batch_idx=ezsim.ti_int,  # batch index
            geom_idx=ezsim.ti_int,  # index of the vertex
            link_idx=ezsim.ti_int,  # index of the link
            contact_pos=ezsim.ti_vec3,  # contact position
            sap_info=self.sap_contact_info_type,  # contact info
        )
        self.max_contact_pairs = self.rigid_solver.n_free_verts * self.sim._B
        self.contact_pairs = self.contact_pair_type.field(shape=(self.max_contact_pairs,))
        self.Jt = ti.field(ezsim.ti_vec3, shape=(self.max_contact_pairs, self.rigid_solver.n_dofs))
        self.M_inv_Jt = ti.field(ezsim.ti_vec3, shape=(self.max_contact_pairs, self.rigid_solver.n_dofs))
        self.W = ti.field(ezsim.ti_mat3, shape=(self.max_contact_pairs,))
...
class RigidFemTetContactHanlder(RigidFEMContactHandler):
    ...
    def __init__(
        ...
        self.contact_candidate_type = ti.types.struct(
            batch_idx=ezsim.ti_int,  # batch index
            geom_idx0=ezsim.ti_int,  # index of the FEM element
            geom_idx1=ezsim.ti_int,  # index of the Rigid Triangle
            vert_idx1=ezsim.ti_ivec3,  # vertex indices of the rigid triangle
            normal=ezsim.ti_vec3,  # contact plane normal
            x=ezsim.ti_vec3,  # a point on the contact plane
        )
        self.n_contact_candidates = ti.field(ezsim.ti_int, shape=())
        ...
        self.contact_pair_type = ti.types.struct(
            ...
            geom_idx1=ezsim.ti_int,  # index of the Rigid triangle
            vert_idx1=ezsim.ti_ivec3,  # vertex indices of the rigid triangle
            barycentric0=ezsim.ti_vec4,  # barycentric coordinates of the contact point in tet
            barycentric1=ezsim.ti_vec3,  # barycentric coordinates of the contact point in tri
            link_idx=ezsim.ti_int,  # index of the link
            contact_pos=ezsim.ti_vec3,  # contact position
            sap_info=self.sap_contact_info_type,  # contact info
        )
    ...
    def compute_candidates(self, f: ti.i32):
        ...
        for i_r in ti.ndrange(self.coupler.rigid_tri_bvh.query_result_count[None]):
            ...
            if i_c < self.max_contact_candidates:
                ...
                self.contact_candidates[i_c].vert_idx1 = ezsim.ti_ivec3(i_v0, i_v1, i_v2)
            else:
                overflow = True
        return overflow
    ...
    def compute_pairs(self, f: ti.i32):
        ...
        for i_c in range(self.n_contact_candidates[None]):
            ...
            tri_vertices = ti.Matrix.zero(ezsim.ti_float, 3, 3)  # 3 vertices of the triangle
            tet_vertices = ti.Matrix.zero(ezsim.ti_float, 3, 4)  # 4 vertices of tet 0
            tet_pressures = ti.Vector.zero(ezsim.ti_float, 4)  # pressures at the vertices of tet 0
            ...
            polygon_vertices = ti.Matrix.zero(ezsim.ti_float, 3, 7)  # maximum 7 vertices
            polygon_n_vertices = 3
            for i in ti.static(range(3)):
                polygon_vertices[:, i] = tri_vertices[:, i]
            clipped_vertices = ti.Matrix.zero(ezsim.ti_float, 3, 7)  # maximum 7 vertices
            clipped_n_vertices = 0
            distances = ti.Vector.zero(ezsim.ti_float, 7)
            for face in range(4):
                clipped_n_vertices = 0
