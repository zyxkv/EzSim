...
import ezsim
from ezsim.options.solvers import LegacyCouplerOptions
...
class LegacyCoupler(RBC):
    ...
    def build(self) -> None:
        ...
        if self._rigid_mpm and self.mpm_solver.enable_CPIC:
            ...
            self.cpic_flag = ti.field(ezsim.ti_int, shape=(self.mpm_solver.n_particles, 3, 3, 3, self.mpm_solver._B))
        ...
        if self._rigid_pbd:
            self.pbd_rigid_normal_reordered = ti.Vector.field(
                3, dtype=ezsim.ti_float, shape=(self.pbd_solver.n_particles, self.pbd_solver._B, self.rigid_solver.n_geoms)
            )
    ...
    def _func_collide_with_rigid_geom(self, pos_world, vel, mass, geom_idx, batch_idx):
        signed_dist = self.rigid_solver.sdf.sdf_func_world(
    ...
    def _func_collide_in_rigid_geom(self, pos_world, vel, mass, normal_rigid, influence, geom_idx, batch_idx):
        ...
        if rvel_normal_magnitude < 0:  # colliding
            ...
            rvel_tan_norm = rvel_tan.norm(ezsim.EPS)
    ...
    def mpm_grid_op(self, f: ti.i32, t: ti.f32):
        ...
        for ii, jj, kk, i_b in ti.ndrange(*self.mpm_solver.grid_res, self.mpm_solver._B):
            I = (ii, jj, kk)
            if self.mpm_solver.grid[f, I, i_b].mass > ezsim.EPS:
                #################### MPM grid op ####################
    ...
    def fem_surface_force(self, f: ti.i32):
        ...
        for i_s, i_b in ti.ndrange(self.fem_solver.n_surfaces, self.fem_solver._B):
            if self.fem_solver.surface[i_s].active:
                ...
                surface_normal = surface_normal / surface_normal.norm(ezsim.EPS)
                ...
                if ti.static(self._fem_mpm):
                    for j in ti.static(range(3)):
                        ...
                        mass_mpm = 0.0
                        mpm_base = ti.floor(pos * self.mpm_solver.inv_dx - 0.5).cast(ezsim.ti_int)
                        mpm_fx = pos * self.mpm_solver.inv_dx - mpm_base.cast(ezsim.ti_float)
                        ...
                        for mpm_offset in ti.static(ti.grouped(self.mpm_solver.stencil_range())):
                            ...
                            mpm_weight = ti.cast(1.0, ezsim.ti_float)
