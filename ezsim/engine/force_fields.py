...
class ForceField(RBC):
    ...
    def __init__(self):
        self._active = ti.field(ezsim.ti_bool, shape=())
...
class Constant(ForceField):
    ...
    def __init__(self, direction=(1, 0, 0), strength=1.0):
        ...
        self._acc_ti = ti.Vector(self._direction * self._strength, dt=ezsim.ti_float)
...
class Wind(ForceField):
    ...
    def __init__(self, direction=(1, 0, 0), strength=1.0, radius=1, center=(0, 0, 0)):
        ...
        self._direction_ti = ti.Vector(self._direction, dt=ezsim.ti_float)
        self._center_ti = ti.Vector(self._center, dt=ezsim.ti_float)
        self._acc_ti = ti.Vector(self._direction * self._strength, dt=ezsim.ti_float)
...
class Point(ForceField):
    ...
    def __init__(self, strength=1.0, position=(0, 0, 0), falloff_pow=0.0, flow=1.0):
        ...
        self._position_ti = ti.Vector(self._position, dt=ezsim.ti_float)

    @ti.func
    def _get_acc(self, pos, vel, t, i):
        relative_pos = pos - self._position_ti
        radius = relative_pos.norm(ezsim.EPS)
...
class Vortex(ForceField):
    ...
    def _get_acc(self, pos, vel, t, i):
        ...
        perpendicular = ti.Vector([-relative_pos[1], relative_pos[0], 0.0], dt=ezsim.ti_float)
        radial = -ti.Vector([relative_pos[0], relative_pos[1], 0.0], dt=ezsim.ti_float)

        falloff = ezsim.ti_float(0.0)
...
class PerlinNoiseField:
    ...
    def _noise(self, x, y, z):
        ...
        X = ezsim.ti_int(ti.floor(x)) & (self._wrap_size - 1)
        Y = ezsim.ti_int(ti.floor(y)) & (self._wrap_size - 1)
        Z = ezsim.ti_int(ti.floor(z)) & (self._wrap_size - 1)
