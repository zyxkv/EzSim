...
import ezsim
import ezsim.utils.element as eu
import ezsim.utils.geom as gu
import ezsim.utils.mesh as mu
from ezsim.engine.entities.rigid_entity import RigidLink
from ezsim.engine.couplers import SAPCoupler
from ezsim.engine.states.cache import QueriedStates
from ezsim.engine.states.entities import FEMEntityState
from ezsim.utils.misc import ALLOCATE_TENSOR_WARNING, to_ezsim_tensor, tensor_to_array
...
class FEMEntity(Entity):
    ...
    def __init__(self, scene, solver, material, morph, surface, idx, v_start=0, el_start=0, s_start=0):
        ...
        tri2el = np.repeat(np.arange(self.elems.shape[0], dtype=ezsim.np_int)[:, np.newaxis], 4, axis=1)
    ...
    def set_position(self, pos):
        ...
        """
        self._assert_active()
        ezsim.logger.warning("Manally setting element positions. This is not recommended and could break gradient flow.")

        pos = to_ezsim_tensor(pos)
        ...
        if pos.ndim == 1:
            ...
            if pos.shape == (self._sim._B, self.n_vertices, 3):
                ...
                is_valid = True
        if not is_valid:
            ezsim.raise_exception("Tensor shape not supported.")
    ...
    def set_velocity(self, vel):
        ...
        """
        self._assert_active()
        ezsim.logger.warning("Manally setting element velocities. This is not recommended and could break gradient flow.")

        vel = to_ezsim_tensor(vel)
    ...
    def set_actuation(self, actu):
        ...
        if actu.ndim == 0:
            ...
            if actu.shape == (n_groups,):
                ...
                ezsim.raise_exception("Cannot set per-element actuation.")
        elif actu.ndim == 2:
    ...
    def set_muscle(self, muscle_group=None, muscle_direction=None):
        ...
        if muscle_group is not None:
            ...
            muscle_group = to_ezsim_tensor(muscle_group)
        ...
        if muscle_direction is not None:
            muscle_direction = to_ezsim_tensor(muscle_direction)
            assert muscle_direction.shape == (self.n_elements, 3)
            assert ((1.0 - muscle_direction.norm(dim=-1)).abs() < ezsim.EPS).all()
    ...
    def deactivate(self):
        ezsim.logger.info(f"{self.__class__.__name__} <{self.id}> deactivated.")
        self._tgt["act"] = ezsim.INACTIVE
        self.active = False

    def activate(self):
        ezsim.logger.info(f"{self.__class__.__name__} <{self.id}> activated.")
        self._tgt["act"] = ezsim.ACTIVE
        self.active = True
    ...
    def instantiate(self, verts, elems):
        ...
        """
        verts = verts.astype(ezsim.np_float, copy=False)
        elems = elems.astype(ezsim.np_int, copy=False)

        # rotate
        R = gu.quat_to_R(np.array(self.morph.quat, dtype=ezsim.np_float))
        verts_COM = verts.mean(axis=0)
        init_positions = (verts - verts_COM) @ R.T + verts_COM

        if not init_positions.shape[0] > 0:
            ezsim.raise_exception(f"Entity has zero vertices.")

        self.init_positions = ezsim.tensor(init_positions)
        self.init_positions_COM_offset = self.init_positions - ezsim.tensor(verts_COM)
    ...
    def sample(self):
        ...
        if isinstance(self.morph, ezsim.options.morphs.Sphere):
            verts, elems = eu.sphere_to_elements(
                pos=self._morph.pos,
                radius=self._morph.radius,
                tet_cfg=self.tet_cfg,
            )
        elif isinstance(self.morph, ezsim.options.morphs.Box):
            verts, elems = eu.box_to_elements(
                pos=self._morph.pos,
                size=self._morph.size,
                tet_cfg=self.tet_cfg,
            )
        elif isinstance(self.morph, ezsim.options.morphs.Cylinder):
            verts, elems = eu.cylinder_to_elements()
        elif isinstance(self.morph, ezsim.options.morphs.Mesh):
            ...
            ezsim.raise_exception(f"Unsupported morph: {self.morph}.")
    ...
    def _add_to_solver(self, in_backward=False):
        ...
        elems_np = self.elems.astype(ezsim.np_int, copy=False)
        verts_numpy = tensor_to_array(self.init_positions, dtype=ezsim.np_float)
    ...
    def compute_pressure_field(self):
        ...
        signed_distance = signed_distance.astype(ezsim.np_float, copy=False)

        unsigned_distance = np.abs(signed_distance)
        max_distance = np.max(unsigned_distance)
        if max_distance < ezsim.EPS:
    ...
    def process_input(self, in_backward=False):
        ...
        if self._tgt["act"] is not None:
            assert self._tgt["act"] in [ezsim.ACTIVE, ezsim.INACTIVE]
    ...
    def _assert_active(self):
        if not self.active:
            ezsim.raise_exception(f"{self.__class__.__name__} is inactive. Call `entity.activate()` first.")
    ...
    def set_active(self, f, active):
        ...
        active : int
            Activity flag (ezsim.ACTIVE or ezsim.INACTIVE).
        """
    ...
    def _sanitize_input_tensor(self, tensor, dtype, unbatched_ndim=1):
        _tensor = torch.as_tensor(tensor, dtype=dtype, device=ezsim.device)
        ...
        if _tensor.ndim < unbatched_ndim + 1:
            ...
            if len(_tensor) != self._sim._B:
                ezsim.raise_exception("Input tensor batch size must match the number of environments.")

        if _tensor.ndim != unbatched_ndim + 1:
            ezsim.raise_exception(f"Input tensor ndim is {_tensor.ndim}, should be {unbatched_ndim + 1}.")

        return _tensor

    def _sanitize_input_verts_idx(self, verts_idx_local):
        verts_idx = self._sanitize_input_tensor(verts_idx_local, dtype=ezsim.tc_int, unbatched_ndim=1) + self._v_start
        assert ((verts_idx >= 0) & (verts_idx < self._solver.n_vertices)).all(), "Vertex indices out of bounds."
        return verts_idx

    def _sanitize_input_poss(self, poss):
        poss = self._sanitize_input_tensor(poss, dtype=ezsim.tc_float, unbatched_ndim=2)
    ...
    def set_vertex_constraints(
        ...
        if self._solver._use_implicit_solver:
            if not self._solver._enable_vertex_constraints:
                ezsim.logger.warning("Ignoring vertex constraint; FEM implicit solver needs to enable vertex constraints.")
                return
        ...
        if target_poss is None:
            target_poss = torch.zeros(
                (verts_idx.shape[0], verts_idx.shape[1], 3), dtype=ezsim.tc_float, device=ezsim.device, requires_grad=False
            )
        ...
        if link is None:
            link_init_pos = torch.zeros((self._sim._B, 3), dtype=ezsim.tc_float, device=ezsim.device)
            link_init_quat = torch.zeros((self._sim._B, 4), dtype=ezsim.tc_float, device=ezsim.device)
            link_idx = -1
        else:
            assert isinstance(link, RigidLink), "Only RigidLink is supported for vertex constraints."
            link_init_pos = self._sanitize_input_tensor(link.get_pos(), dtype=ezsim.tc_float)
            link_init_quat = self._sanitize_input_tensor(link.get_quat(), dtype=ezsim.tc_float)
            link_idx = link.idx
    ...
    def update_constraint_targets(self, verts_idx, target_poss, envs_idx=None):
        ...
        if not self._solver._constraints_initialized:
            ezsim.logger.warning("Ignoring update_constraint_targets; constraints have not been initialized.")
            return
    ...
    def remove_vertex_constraints(self, verts_idx=None, envs_idx=None):
        ...
        if not self._solver._constraints_initialized:
            ezsim.logger.warning("Ignoring remove_vertex_constraints; constraints have not been initialized.")
            return
    ...
    def get_el2v(self):
        ...
        el2v = ezsim.zeros((self.n_elements, 4), dtype=int, requires_grad=False, scene=self.scene)
