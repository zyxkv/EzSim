...
class Emitter(RBC):
    ...
    def set_entity(self, entity):
        ...
        ezsim.logger.info(f"~<{self._repr_briefer()}>~ created using ~<{entity._repr_briefer()}.")
    ...
    def emit(
        ...
        if droplet_shape in ["circle", "sphere", "square"]:
            ...
            ezsim.raise_exception(f"Unsupported nozzle shape: {droplet_shape}.")

        direction = np.asarray(direction, dtype=ezsim.np_float)
        if np.linalg.norm(direction) < ezsim.EPS:
            ezsim.raise_exception("Zero-length direction.")
        else:
        ...
        if droplet_length > 0.0:
            ...
            positions = gu.transform_by_trans_R(
                positions.astype(ezsim.np_float, copy=False),
                np.asarray(pos, dtype=ezsim.np_float),
                gu.z_up_to_R(direction) @ gu.axis_angle_to_R(np.array([0.0, 0.0, 1.0], dtype=ezsim.np_float), theta),
            )

            positions = np.tile(positions[np.newaxis], (self._sim._B, 1, 1))

            if not self._solver.boundary.is_inside(positions):
                ezsim.raise_exception("Emitted particles are outside the boundary.")
            ...
            ezsim.logger.debug(f"Emitted {n_particles} particles. Next particle index: {self._next_particle}.")

        else:
            ezsim.logger.debug("Droplet length is too short for current step. Skipping to next step.")
    ...
    def emit_omni(self, source_radius=0.1, pos=(0.5, 0.5, 1.0), speed=1.0, particle_size=None):
        ...
        pos = np.asarray(pos, dtype=ezsim.np_float)
        ...
        positions[dists < ezsim.EPS] = ezsim.EPS
        vels = (speed / (dists + ezsim.EPS)) * positions_
