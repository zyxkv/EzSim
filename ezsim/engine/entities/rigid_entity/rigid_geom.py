...
class RigidGeom(RBC):
    ...
    def _preprocess(self):
        ...
        if os.path.exists(self._gsd_path):
            ezsim.logger.debug(f"Preprocessed file (`.gsd`) found in cache for geom idx {self._idx}.")
            try:
        ...
        if not is_cached_loaded:
            with ezsim.logger.timer(f"Preprocessing geom idx ~~<{self._idx}>~~."):
                ######## sdf ########
                ...
                sdf_cell_size = ezsim.EPS + np.clip(
                    self._material.sdf_cell_size,
                    grid_size.max() / (self._material.sdf_max_res - 1),
                    grid_size.min() / max(self._material.sdf_min_res - 1, 2),
                )
                sdf_res = np.ceil(grid_size / sdf_cell_size).astype(ezsim.np_int) + 1
                ...
                ######## sdf gradient ########
                if self.type == ezsim.GEOM_TYPE.TERRAIN:  # terrain uses finite difference for sdf gradient computation
                    # dummy
                    sdf_grad_delta = 0.0
    ...
    def _compute_sd(self, query_points):
        ...
        return signed_distance.astype(ezsim.np_float, copy=False)
    ...
    def _compute_closest_verts(self, query_points):
        ...
        verts_ids = verts_ids[
            np.arange(len(query_points), dtype=ezsim.np_int),
    ...
    def get_sdf_trimesh(self, color=[1.0, 1.0, 0.6, 1.0]):
        ...
        if self.sdf_val.min() >= 0:
            ezsim.logger.warning("SDF is positive everywhere. Returning empty mesh.")
    ...
    def visualize_sdf(
        ...
        if self._solver.scene.viewer is None:
            ezsim.raise_exception("Viewer is not available.")

        if self._solver.n_envs > 0:
            ezsim.raise_exception("Batched scene does not support sdf visualization.")
    ...
    def set_friction(self, friction):
        ...
        """
        if friction < 0:
            ezsim.raise_exception("`friction` must be non-negative.")
        self._friction = friction
    ...
    def get_pos(self):
        ...
        """
        tensor = torch.empty(self._solver._batch_shape(3, True), dtype=ezsim.tc_float, device=ezsim.device)
    ...
    def get_quat(self):
        ...
        """
        tensor = torch.empty(self._solver._batch_shape(4, True), dtype=ezsim.tc_float, device=ezsim.device)
