...
import ezsim
from ezsim.engine.states.entities import MPMEntityState
...
class MPMEntity(ParticleEntity):
    ...
    def __init__(
        ...
        if isinstance(material, (ezsim.materials.MPM.Liquid, ezsim.materials.MPM.Sand, ezsim.materials.MPM.Snow)):
            need_skinning = False
        else:
            need_skinning = True
    ...
    def get_muscle_group(self):
        ...
        """
        muscle_group = ezsim.zeros((self._n_particles,), dtype=int, requires_grad=False, scene=self._scene)
    ...
    def set_active(self, f, active):
        ...
        f : int
            The current substep index.
        active : int
            Value indicating whether particles are active (ezsim.ACTIVE) or inactive (ezsim.INACTIVE).
        """
    ...
    def get_free(self):
        ...
        free = ezsim.zeros((self._n_particles,), dtype=ezsim.tc_bool, requires_grad=False, scene=self._scene)
    ...
    def get_particles(self):
        ...
        """
        pos = np.empty((self._sim._B, self.n_particles, 3), dtype=ezsim.np_float)
