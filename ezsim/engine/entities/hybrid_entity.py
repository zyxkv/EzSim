...
from ezsim.ext.urdfpy.urdf import URDF
from ezsim.utils.hybrid import (
    check_graph,
...
class HybridEntity(Entity):
    ...
    surface : ezsim.surfaces.Surface
    ...
    def __init__(
        ...
        surface_rigid = ezsim.surfaces.Smooth(roughness=0.4)  # HACK hardcoded

        assert isinstance(mat_soft, ezsim.materials.MPM.Base)  # TODO: need FEM and PBD

        if isinstance(morph, ezsim.morphs.URDF):
            # set up rigid part
            morph.fixed = material.fixed  # NOTE: use hybrid material to determine this
            if material.use_default_coupling:
                ezsim.logger.info("Use default coupling in hybrid. Overwrite `needs_coup` in rigid material to True")
                mat_rigid._needs_coup = True
            else:
                ezsim.logger.info("Use default coupling in hybrid. Overwrite `needs_coup` in rigid material to False")
        ...
        elif isinstance(morph, ezsim.morphs.Mesh):
        ...
        if not material.use_default_coupling:
            ...
            if muscle_group is not None:
                muscle_group = muscle_group.astype(ezsim.np_int, copy=False)
            ...
            if isinstance(mat_soft, ezsim.materials.MPM.Base):
                part_soft_info = ti.types.struct(
                    ...
                    geom_idx=ezsim.ti_int,
                    trans_local_to_global=ezsim.ti_vec3,
                    quat_local_to_global=ezsim.ti_vec4,
                ).field(shape=(mat_soft.n_groups,), needs_grad=False, layout=ti.Layout.SOA)
                part_soft_info.link_idx.from_numpy(np.asarray(link_idcs, dtype=ezsim.np_int))
                part_soft_info.geom_idx.from_numpy(np.asarray(geom_idcs, dtype=ezsim.np_int))
                part_soft_info.trans_local_to_global.from_numpy(np.asarray(trans_local_to_global, dtype=ezsim.np_float))
                part_soft_info.quat_local_to_global.from_numpy(np.asarray(quat_local_to_global, dtype=ezsim.np_float))

                part_soft_init_positions = ti.field(dtype=ezsim.ti_vec3, shape=(part_soft.init_particles.shape[0],))
                part_soft_init_positions.from_torch(ezsim.Tensor(part_soft.init_particles))
    ...
    def build(self):
        ...
        if not self.material.use_default_coupling and self._muscle_group_cache is not None:
            self._part_soft.set_muscle(
                muscle_group=ezsim.tensor(self._muscle_group_cache),
...
def augment_link_world_coords(part_rigid):
    ...
    for i, i_l_global in enumerate(ordered_links_idx):
        ...
        if (link.joints and link.joints[0].type == ezsim.JOINT_TYPE.FREE) or (
            i_p == -1 and (not link.joints or link.joints[0].type == ezsim.JOINT_TYPE.FIXED)
        ):
...
def default_func_instantiate_rigid_from_soft(
    ...
    pos_rigid = morph.pos + offset.astype(ezsim.np_float, copy=False)
    quat_rigid = morph.quat
    scale_rigid = morph.scale
    morph_rigid = ezsim.morphs.URDF(
        file=urdf,
        pos=pos_rigid,
        quat=quat_rigid,
        scale=scale_rigid,
