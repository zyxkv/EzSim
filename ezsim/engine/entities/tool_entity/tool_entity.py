...
from ezsim.engine.states.entities import ToolEntityState
...
class ToolEntity(Entity):
    ...
    def __init__(
        ...
        self._init_pos = np.array(morph.pos, dtype=ezsim.np_float)
        self._init_quat = np.array(morph.quat, dtype=ezsim.np_float)
        ...
        self.latest_pos = ti.Vector.field(3, dtype=ezsim.ti_float, shape=(1))
        ...
        if self._sim.requires_grad:
            if ckpt_name not in self._ckpt:
                self._ckpt[ckpt_name] = {
                    "pos": torch.zeros((self._sim._B, 3), dtype=ezsim.tc_float),
                    "quat": torch.zeros((self._sim._B, 4), dtype=ezsim.tc_float),
                    "vel": torch.zeros((self._sim._B, 3), dtype=ezsim.tc_float),
                    "ang": torch.zeros((self._sim._B, 3), dtype=ezsim.tc_float),
                    "_tgt_buffer": dict(),
                }
    ...
    def build(self):
        self.pos = ti.Vector.field(3, ezsim.ti_float, needs_grad=True)  # positon
        self.quat = ti.Vector.field(4, ezsim.ti_float, needs_grad=True)  # quaternion wxyz
        self.vel = ti.Vector.field(3, ezsim.ti_float, needs_grad=True)  # velocity
        self.ang = ti.Vector.field(3, ezsim.ti_float, needs_grad=True)  # angular velocity
