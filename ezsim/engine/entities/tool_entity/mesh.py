...
class Mesh:
    ...
    def load_file(self):
        self.process_mesh()
        self.raw_vertices = np.asarray(self.mesh.vertices, dtype=ezsim.np_float, order="C")
        self.raw_vertex_normals = np.asarray(self.mesh.vertex_normals, dtype=ezsim.np_float, order="C")
        self.faces_np = np.asarray(self.mesh.faces, dtype=ezsim.np_int, order="C").reshape((-1))

        # apply initial transforms (scale then quat then pos)
        scale = np.array(self.scale, dtype=ezsim.np_float)
    ...
    def process_mesh(self):
        # clean up mesh
        ezsim.logger.debug(f"Processing mesh: {self.raw_file}.")
        ...
        if self.collision:
            ...
            self.sdf_voxels_np = sdf_data["voxels"].astype(ezsim.np_float, order="C", copy=False)
            self.sdf_res = self.sdf_voxels_np.shape[0]
            self.T_mesh_to_sdf_np = sdf_data["T_mesh_to_sdf"].astype(ezsim.np_float, order="C", copy=False)

    def init_fields(self):
        # init ti fields
        self.init_vertices = ti.Vector.field(3, dtype=ezsim.ti_float, shape=(self.n_vertices))
        self.init_vertex_normals = ti.Vector.field(3, dtype=ezsim.ti_float, shape=(self.n_vertices))
        self.faces = ti.field(dtype=ezsim.ti_int, shape=(self.n_faces))
        ...
        if self.collision:
            self.sdf_voxels = ti.field(dtype=ezsim.ti_float, shape=self.sdf_voxels_np.shape)
            self.T_mesh_to_sdf = ti.Matrix.field(4, 4, dtype=ezsim.ti_float, shape=())

            self.sdf_voxels.from_numpy(self.sdf_voxels_np)
            self.T_mesh_to_sdf.from_numpy(self.T_mesh_to_sdf_np)

        self.vertices = ti.Vector.field(3, dtype=ezsim.ti_float, shape=(self.n_vertices))
        self.vertex_normals = ti.Vector.field(3, dtype=ezsim.ti_float, shape=(self.n_vertices))
    ...
    def sdf_(self, pos_voxels):
        ...
        base = ti.floor(pos_voxels, ezsim.ti_int)
    ...
    def normal_(self, pos_voxels):
        ...
        normal_vec = ti.Vector([0, 0, 0], dt=ezsim.ti_float)
    ...
    def collide(self, f, pos_world, vel_mat, i_b):
        if ti.static(self.collision):
            ...
            influence = ti.min(ti.exp(-signed_dist / max(ezsim.EPS, self.material.coup_softness)), 1)
            if signed_dist <= 0.0 or influence > 0.1:
                ...
                if normal_component < 0.00:
                    ...
                    rel_v_t_norm = rel_v_t.norm(ezsim.EPS)
                    ...
                    #FIXME: this formular can be simplified since flag=1.0 systematically
                    flag = ti.cast(normal_component < 0.0, ezsim.ti_float)
    ...
    def pbd_collide(self, f, pos_world, thickness, dt):
        ...
        if ti.static(self.collision):
            ...
            vec_world_norm = vec_world.norm(ezsim.EPS)
