...
from ezsim.utils.misc import to_ezsim_tensor
...
class ParticleEntity(Entity):
    ...
    def __init__(
        ...
        # visual mesh
        if isinstance(self._morph, ezsim.options.morphs.MeshSet):
            self._vmesh = ezsim.Mesh.from_morph_surface(self.morph, self.surface)
            self._surface = self._vmesh[0].surface

        elif isinstance(self._morph, (ezsim.options.morphs.Primitive, ezsim.options.morphs.Mesh)):
            self._vmesh = ezsim.Mesh.from_morph_surface(self.morph, self.surface)
            if isinstance(self._vmesh, list):
                if len(self._vmesh) > 1:
                    ezsim.raise_exception("Mesh file with multiple sub-meshes are not supported.")
                else:
    ...
    def _add_to_solver(self):
        if isinstance(self._morph, ezsim.options.morphs.Nowhere):
    ...
    def _add_vverts_to_solver(self):
        ...
        def _kernel_add_vverts_to_solver(
            ...
            P_invs: ti.types.ndarray(dtype=ti.types.matrix(self.solver._n_vvert_supports - 1, 3, ezsim.ti_float)),
        ...
        support_idxs = np.clip(support_idxs.astype(ezsim.np_int, copy=False), 0, len(self._particles) - 1)
        all_ps = self._particles[support_idxs]
        Ps = np.stack(
            [all_ps[:, i, :] for i in range(self.solver._n_vvert_supports - 1)], axis=2, dtype=ezsim.np_float
    ...
    def sample(self):
        ...
        if isinstance(self._morph, ezsim.options.morphs.MeshSet):
            ...
            particles = particles.astype(ezsim.np_float, order="C", copy=False)
        elif isinstance(self._morph, ezsim.options.morphs.Nowhere):
            particles = pu.nowhere_particles(self._morph.n_particles)
        else:
            ezsim.raise_exception(f"Unsupported morph: {self._morph}.")

        if not isinstance(self._morph, ezsim.options.morphs.MeshSet) and particles.shape[0] == 0:
            ezsim.raise_exception("Entity has zero particles.")

        if isinstance(self._morph, ezsim.options.morphs.Nowhere):
            origin = gu.nowhere()
            self._vverts = np.array([], dtype=ezsim.np_float)
            self._vfaces = np.array([], dtype=ezsim.np_float)
        elif isinstance(self._morph, ezsim.options.morphs.MeshSet):
            for i in range(len(self._morph.files)):
                pos_i = np.asarray(self._morph.poss[i], dtype=ezsim.np_float)
                euler_i = np.asarray(self._morph.eulers[i], dtype=ezsim.np_float)
                quat_i = ezsim.utils.geom.xyz_to_quat(euler_i, rpy=True, degrees=True)
            ...
            self.mesh_set_group_ids = np.concatenate(
                [np.full((len(v),), fill_value=i, dtype=ezsim.np_int) for i, v in enumerate(particles)]
            )
            particles = np.concatenate(particles, dtype=ezsim.np_float)
            ...
            if self._need_skinning:
                self._vverts = np.asarray(self._vmesh.verts, dtype=ezsim.np_float)
                self._vfaces = np.asarray(self._vmesh.faces, dtype=ezsim.np_float)
            else:
                self._vverts = np.array([], dtype=ezsim.np_float)
                self._vfaces = np.array([], dtype=ezsim.np_float)
            origin = np.mean(self._morph.poss, dtype=ezsim.np_float)
        else:
            # transform vmesh
            # pos, quat = map(np.asarray, (self._morph.pos, self._morph.quat))
            pos = np.asarray(self._morph.pos, dtype=ezsim.np_float)
            quat = np.asarray(self._morph.quat, dtype=ezsim.np_float)
            self._vmesh.apply_transform(gu.trans_quat_to_T(pos, quat))
            # transform particles
            particles = gu.transform_by_trans_quat(
                particles,
                np.asarray(self._morph.pos, dtype=ezsim.np_float),
                np.asarray(self._morph.quat, dtype=ezsim.np_float),
            )
            ...
            if self._need_skinning:
                ...
                self._vverts = np.array([], dtype=ezsim.np_float)
                self._vfaces = np.array([], dtype=ezsim.np_float)
            origin = np.array(self._morph.pos, dtype=ezsim.np_float)

        self._particles = particles
        self._init_particles_offset = ezsim.tensor(self._particles) - ezsim.tensor(origin)
        self._n_particles = len(self._particles)

        ezsim.logger.info(f"Sampled ~~<{self._n_particles:,}>~~ particles.")
    ...
    def get_mass(self):
        ...
        """
        mass = np.zeros((1,),dtype=ezsim.np_float)
    ...
    def deactivate(self):
        ...
        """
        ezsim.logger.info(f"{self.__class__.__name__} <{self._uid}> deactivated.")
    ...
    def activate(self):
        ...
        """
        ezsim.logger.info(f"{self.__class__.__name__} <{self._uid}> activated.")
    ...
    def get_particles(self):
        ...
        """
        particles = np.empty((self._sim._B, self.n_particles, 3), dtype=ezsim.np_float)
