...
class PBDTetEntity(ParticleEntity):
    ...
    def _kernel_add_particles_edges_to_solver(
        ...
        for i_p_, i_b in ti.ndrange(self.n_particles, self._sim._B):
            ...
            self.solver.particles[i_p, i_b].vel = ti.Vector.zero(ezsim.ti_float, 3)
            self.solver.particles[i_p, i_b].dpos = ti.Vector.zero(ezsim.ti_float, 3)
            self.solver.particles[i_p, i_b].free = True

            self.solver.particles_ng[i_p, i_b].active = ti.cast(active,ezsim.ti_bool)
...
class PBD2DEntity(PBDTetEntity):
    ...
    def sample(self):
        ...
        if self._vmesh.area < 1e-6:
            ezsim.raise_exception("Input mesh has zero surface area.")
        self._mass = self._vmesh.area * self.material.rho

        self._particles = np.asarray(self._mesh.verts, dtype=ezsim.np_float)
        self._edges = np.asarray(self._mesh.get_unique_edges(), dtype=ezsim.np_int)
        ...
        self._inner_edges = np.stack([inner_edges[:, 0], inner_edges[:, 1], v3, v4], axis=1, dtype=ezsim.np_int)
...
class PBD3DEntity(PBDTetEntity):
    ...
    def sample(self):
        ...
        if self._vmesh.volume < 1e-6:
            ezsim.raise_exception("Input mesh has zero volume.")
        ...
        self._particles = particles.astype(ezsim.np_float, copy=False)
        self._elems = elems.astype(ezsim.np_int, copy=False)
