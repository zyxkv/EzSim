...
class CubeBoundary:
    def __init__(self, lower, upper, restitution=0.0):
        ...
        self.upper = np.array(upper, dtype=ezsim.np_float)
        self.lower = np.array(lower, dtype=ezsim.np_float)
        assert (self.upper >= self.lower).all()

        self.upper_ti = ti.Vector(upper, dt=ezsim.ti_float)
        self.lower_ti = ti.Vector(lower, dt=ezsim.ti_float)
