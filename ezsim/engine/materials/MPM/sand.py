...
class Sand(Base):
    ...
    def sand_projection(self, S, Jp):
        S_out = ti.Matrix.zero(ezsim.ti_float, 3, 3)
        epsilon = ti.Vector.zero(ezsim.ti_float, 3)
        for i in ti.static(range(3)):
            epsilon[i] = ti.log(max(abs(S[i, i]), 1e-4))
            S_out[i, i] = 1
        tr = epsilon.sum() + Jp
        epsilon_hat = epsilon - tr / 3
        epsilon_hat_norm = epsilon_hat.norm(ezsim.EPS)

        Jp_new = ezsim.ti_float(0.0)
        if tr >= 0.0:
            Jp_new = tr
        else:
            Jp_new = 0.0
    ...
    def update_stress(self, U, S, V, F_tmp, F_new, J, Jp, actu, m_dir):
        log_S_sum = ezsim.ti_float(0.0)
        center = ti.Matrix.zero(ezsim.ti_float, 3, 3)
        for i in ti.static(range(3)):
