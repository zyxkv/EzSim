...
class Rigid(Material):
    ...
    def __init__(
        ...
        if coup_friction < 0:
            ezsim.raise_exception("`coup_friction` must be non-negative.")

        if coup_softness < 0:
            ezsim.raise_exception("`coup_softness` must be non-negative.")

        if coup_restitution < 0 or coup_restitution > 1:
            ezsim.raise_exception("`coup_restitution` must be in the range [0, 1].")

        if coup_restitution != 0:
            ezsim.logger.warning("Non-zero `coup_restitution` could lead to instability. Use with caution.")

        if sdf_min_res < 16:
            ezsim.raise_exception("`sdf_min_res` must be at least 16.")

        if sdf_min_res > sdf_max_res:
            ezsim.raise_exception("`sdf_min_res` must be smaller than or equal to `sdf_max_res`.")
