...
class Elastic(Base):
    ...
    def __init__(
        ...
        if model == "linear":
            ...
            if model == "stable_neohooken":
                ezsim.logger.warning("The 'stable_neohooken' model is deprecated. Use 'stable_neohookean' instead.")
            ...
            ezsim.raise_exception(f"Unrecognized constitutive model: {model}")

        self._model = model

    def build_linear_corotated(self, fem_solver):
        self.R = ti.field(dtype=ezsim.ti_mat3, shape=(fem_solver._B, fem_solver.n_elements))
    ...
    def update_stress_linear(self, mu, lam, J, F, actu, m_dir):
        I = ti.Matrix.identity(dt=ezsim.ti_float, n=3)
