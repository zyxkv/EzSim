...
from ezsim.engine.entities import (
    PBD2DEntity,
    PBD3DEntity,
    PBDFreeParticleEntity,
    PBDParticleEntity,
)
from ezsim.engine.states.solvers import PBDSolverState
from ezsim.utils.geom import SpatialHasher
...
class PBDSolver(Solver):
    ...
    def init_particle_fields(self):
        ...
        struct_particle_info = ti.types.struct(
            mass=ezsim.ti_float,
            pos_rest=ezsim.ti_vec3,
            rho_rest=ezsim.ti_float,
            mat_type=ezsim.ti_int,
            mu_s=ezsim.ti_float,
            mu_k=ezsim.ti_float,
            air_resistance=ezsim.ti_float,
            density_relaxation=ezsim.ti_float,
            viscosity_relaxation=ezsim.ti_float,
        )
        # particles state (dynamic)
        struct_particle_state = ti.types.struct(
            free=ezsim.ti_bool,  # if not free, the particle is not affected by internal forces and solely controlled by external user until released
            pos=ezsim.ti_vec3,  # position
            ipos=ezsim.ti_vec3,  # initial position
            dpos=ezsim.ti_vec3,  # delta position
    ...
    def init_edge_fields(self):
        ...
        struct_edge_info = ti.types.struct(
            len_rest=ezsim.ti_float,
            stretch_compliance=ezsim.ti_float,
            stretch_relaxation=ezsim.ti_float,
            v1=ezsim.ti_int,
            v2=ezsim.ti_int,
        )
        ...
        struct_inner_edge_info = ti.types.struct(
            len_rest=ezsim.ti_float,
            bending_compliance=ezsim.ti_float,
            bending_relaxation=ezsim.ti_float,
            v1=ezsim.ti_int,
            v2=ezsim.ti_int,
            v3=ezsim.ti_int,
            v4=ezsim.ti_int,
    ...
    def init_elem_fields(self):
        struct_elem_info = ti.types.struct(
            vol_rest=ezsim.ti_float,
            volume_compliance=ezsim.ti_float,
            volume_relaxation=ezsim.ti_float,
            v1=ezsim.ti_int,
            v2=ezsim.ti_int,
            v3=ezsim.ti_int,
            v4=ezsim.ti_int,
    ...
    def add_entity(self, idx, material, morph, surface):
        ...
        elif isinstance(material, ezsim.materials.PBD.Elastic):
            entity = PBD3DEntity(
                scene=self.scene,
                solver=self,
                material=material,
        ...
        elif isinstance(material, ezsim.materials.PBD.Liquid):
        ...
        elif isinstance(material, ezsim.materials.PBD.Particle):
    ...
    def spiky(self, dist):
        # dist is a VECTOR
        result = ti.Vector.zero(ezsim.ti_float, 3)
        d = dist.norm() / self.dist_scale
        if 0 < d < self.h:
            m = (self.h - d) * (self.h - d)
    ...
    def _kernel_apply_external_force(self, f: ti.i32, t: ti.f32):
        for i_p, i_b in ti.ndrange(self._n_particles, self._B):
            if self.particles[i_p, i_b].free:
                ...
                acc = ti.Vector.zero(ezsim.ti_float, 3)
    ...
    def _kernel_solve_stretch(self, f: ti.i32):
        for _ in ti.static(range(self._max_stretch_solver_iterations)):
            for i_e, i_b in ti.ndrange(self._n_edges, self._B):
                ...
                dp = -C / (w1 + w2 + alpha) * n / n.norm(ezsim.EPS) * self.edges_info[i_e].stretch_relaxation
    ...
    #         dp = -C / (w1 + w2 + alpha) * n / n.norm(ezsim.EPS) * self.inner_edges_info[i_ie].bending_relaxation
    ...
    def _func_solve_collision(self, i, j, i_b):
        ...
        cur_dist = (self.particles_reordered[i, i_b].pos - self.particles_reordered[j, i_b].pos).norm(ezsim.EPS)
        rest_dist = (
        ...
        if cur_dist < target_dist and rest_dist > target_dist:
            ...
            if dpos.norm() < mu_s * d:
                ...
                self.particles_reordered[i, i_b].dpos += (
                    wi / (wi + wj) * dpos * ti.min(1.0, mu_k * d / dpos.norm(ezsim.EPS))
                )
        ...
        for _ in ti.static(range(self._max_density_solver_iterations)):
            ...
            for i_p, i_b in ti.ndrange(self._n_particles, self._B):
                if self.particles_info_reordered[i_p, i_b].mat_type == self.MATS.LIQUID:
                    ...
                    lower_sum = 0.0
                    rho = 0.0
                    spiky_i = ti.Vector.zero(ezsim.ti_float, 3)
                    for offset in ti.grouped(ti.ndrange((-1, 2), (-1, 2), (-1, 2))):
                        slot_idx = self.sh.grid_to_slot(base + offset)
                        for j in range(
                            self.sh.slot_start[slot_idx, i_b],
        ...
        for _ in ti.static(range(self._max_viscosity_solver_iterations)):
            for i_p, i_b in ti.ndrange(self._n_particles, self._B):
                if self.particles_info_reordered[i_p, i_b].mat_type == self.MATS.LIQUID:
                    pos_i = self.particles_reordered[i_p, i_b].pos
                    base = self.sh.pos_to_grid(pos_i)
                    xsph_sum = ti.Vector.zero(ezsim.ti_float, 3)
                    omega_sum = ti.Vector.zero(ezsim.ti_float, 3)
                    # -For Gradient Approx.-
                    dx_sum = ti.Vector.zero(ezsim.ti_float, 3)
                    dy_sum = ti.Vector.zero(ezsim.ti_float, 3)
                    dz_sum = ti.Vector.zero(ezsim.ti_float, 3)
                    n_dx_sum = ti.Vector.zero(ezsim.ti_float, 3)
                    n_dy_sum = ti.Vector.zero(ezsim.ti_float, 3)
                    n_dz_sum = ti.Vector.zero(ezsim.ti_float, 3)
                    dx = ti.Vector([self.g_del, 0.0, 0.0], dt=ezsim.ti_float)
                    dy = ti.Vector([0.0, self.g_del, 0.0], dt=ezsim.ti_float)
                    dz = ti.Vector([0.0, 0.0, self.g_del], dt=ezsim.ti_float)
    ...
    def _kernel_get_frame(
        ...
        for i_p, i_b in ti.ndrange(self._n_particles, self._B):
            ...
            free[i_b, i_p] = ti.cast(self.particles[i_p, i_b].free, ezsim.ti_bool)
