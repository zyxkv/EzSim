...
import ezsim
from ezsim.options.solvers import MPMOptions
import ezsim.utils.geom as gu
from ezsim.engine.boundaries import CubeBoundary
from ezsim.engine.entities import MPMEntity
from ezsim.engine.states.solvers import MPMSolverState
import ezsim.utils.sdf_decomp as sdf_decomp

from .base_solver import Solver

if TYPE_CHECKING:
    from ezsim.engine.scene import Scene
    from ezsim.engine.solvers.base_solver import Solver
...
class MPMSolver(Solver):
    ...
    def __init__(self, scene: "Scene", sim: "Simulator", options: "MPMOptions"):
        ...
        self._lower_bound_cell = np.round(self._grid_density * self._lower_bound).astype(ezsim.np_int)
        self._upper_bound_cell = np.round(self._grid_density * self._upper_bound).astype(ezsim.np_int)
        ...
        if self._use_sparse_grid:
            ezsim.raise_exception("Sparse grid is not supported after adding batch dimension.")
            self._grid_res = (np.ceil(self._grid_res / self._leaf_block_size) * self._leaf_block_size).astype(ezsim.np_int)

            if sim.requires_grad:
                ezsim.raise_exception("Sparse grid is not supported in differentiable mode.")
    ...
    def init_particle_fields(self):
        ...
        struct_particle_state = ti.types.struct(
            pos=ezsim.ti_vec3,  # position
            vel=ezsim.ti_vec3,  # velocity
            C=ezsim.ti_mat3,  # affine velocity field
            F=ezsim.ti_mat3,  # deformation gradient
            F_tmp=ezsim.ti_mat3,  # temp deformation gradient
            U=ezsim.ti_mat3,  # SVD
            V=ezsim.ti_mat3,  # SVD
            S=ezsim.ti_mat3,  # SVD
            actu=ezsim.ti_float,  # actuation
            Jp=ezsim.ti_float,  # volume ratio
        )

        # dynamic particle state without gradient
        struct_particle_state_ng = ti.types.struct(
            active=ezsim.ti_bool,
        )

        # static particle info
        struct_particle_info = ti.types.struct(
            mat_idx=ezsim.ti_int,
            mass=ezsim.ti_float,
            default_Jp=ezsim.ti_float,
            free=ezsim.ti_bool,
            # for muscle
            muscle_group=ezsim.ti_int,
            muscle_direction=ezsim.ti_vec3,
        )
    ...
    def init_grid_fields(self):
        grid_cell_state = ti.types.struct(
            vel_in=ezsim.ti_vec3,  # input momentum/velocity
            mass=ezsim.ti_float,  # mass
            vel_out=ezsim.ti_vec3,  # output momentum/velocity
        )
    ...
    def init_vvert_fields(self):
        struct_vvert_info = ti.types.struct(
            support_idxs=ti.types.vector(self._n_vvert_supports, ezsim.ti_int),
            support_weights=ti.types.vector(self._n_vvert_supports, ezsim.ti_float),
        )
        ...
        for i_p, i_b in ti.ndrange(self._n_particles, self._B):
            if self.particles_ng[f, i_p, i_b].active:
                self.particles[f, i_p, i_b].F_tmp = (
                    ti.Matrix.identity(ezsim.ti_float, 3) + self.substep_dt * self.particles[f, i_p, i_b].C
        ...
        for i_p, i_b in ti.ndrange(self._n_particles, self._B):
            if self.particles_ng[f, i_p, i_b].active:
                self.particles[f, i_p, i_b].U, self.particles[f, i_p, i_b].S, self.particles[f, i_p, i_b].V = ti.svd(
                    self.particles[f, i_p, i_b].F_tmp, ezsim.ti_float
                )
    ...
    def backward_svd(self, grad_U, grad_S, grad_V, U, S, V):
        ...
        F = ti.Matrix.zero(ezsim.ti_float, 3, 3)
    ...
    def p2g(self, f: ti.i32):
        for i_p, i_b in ti.ndrange(self._n_particles, self._B):
            if self.particles_ng[f, i_p, i_b].active:
                ...
                F_new = ti.Matrix.zero(ezsim.ti_float, 3, 3)
                S_new = ti.Matrix.zero(ezsim.ti_float, 3, 3)
                Jp_new = ezsim.ti_float(1.0)
                ...
                stress = ti.Matrix.zero(ezsim.ti_float, 3, 3)
                for mat_idx in ti.static(self._mats_idx):
                    if self.particles_info[i_p].mat_idx == mat_idx:
                ...
                # C. project onto grid
                base = ti.floor(self.particles[f, i_p, i_b].pos * self._inv_dx - 0.5).cast(ezsim.ti_int)
                fx = self.particles[f, i_p, i_b].pos * self._inv_dx - base.cast(ezsim.ti_float)
                w = [0.5 * (1.5 - fx) ** 2, 0.75 - (fx - 1) ** 2, 0.5 * (fx - 0.5) ** 2]
                for offset in ti.static(ti.grouped(self.stencil_range())):
                    dpos = (offset.cast(ezsim.ti_float) - fx) * self._dx
                    weight = ti.cast(1.0, ezsim.ti_float)
                    ...
                    if not self.particles_info[i_p].free:  # non-free particles behave as boundary conditions
                        self.grid[f, base - self._grid_offset + offset, i_b].vel_in = ti.Vector.zero(ezsim.ti_float, 3)
    ...
    def g2p(self, f: ti.i32):
        for i_p, i_b in ti.ndrange(self._n_particles, self._B):
            if self.particles_ng[f, i_p, i_b].active:
                ...
                new_vel = ti.Vector.zero(ezsim.ti_float, 3)
                new_C = ti.Matrix.zero(ezsim.ti_float, 3, 3)
                for offset in ti.static(ti.grouped(self.stencil_range())):
                    dpos = offset.cast(ezsim.ti_float) - fx
    ...
    def reset_grid_and_grad(self, f: ti.i32):
        ...
        for i, j, k, i_b in ti.ndrange(*self._grid_res, self._B):
            self.grid[f, i, j, k, i_b].vel_in = ti.Vector.zero(ezsim.ti_float, 3)
            self.grid[f, i, j, k, i_b].mass = ezsim.ti_float(0.0)
            self.grid[f, i, j, k, i_b].vel_out = ti.Vector.zero(ezsim.ti_float, 3)

            self.grid.grad[f, i, j, k, i_b].vel_in = ti.Vector.zero(ezsim.ti_float, 3)
            self.grid.grad[f, i, j, k, i_b].mass = ezsim.ti_float(0.0)
            self.grid.grad[f, i, j, k, i_b].vel_out = ti.Vector.zero(ezsim.ti_float, 3)
    ...
    def reset_grad_till_frame(self, f: ti.i32):
        ...
        for i_f, i_p, i_b in ti.ndrange(f, self._n_particles, self._B):
            self.particles.grad[i_f, i_p, i_b].pos = ti.Vector.zero(ezsim.ti_float, 3)
            self.particles.grad[i_f, i_p, i_b].vel = ti.Vector.zero(ezsim.ti_float, 3)
            self.particles.grad[i_f, i_p, i_b].C = ti.Matrix.zero(ezsim.ti_float, 3, 3)
            self.particles.grad[i_f, i_p, i_b].F = ti.Matrix.zero(ezsim.ti_float, 3, 3)
            self.particles.grad[i_f, i_p, i_b].F_tmp = ti.Matrix.zero(ezsim.ti_float, 3, 3)
            self.particles.grad[i_f, i_p, i_b].Jp = ezsim.ti_float(0.0)
            self.particles.grad[i_f, i_p, i_b].U = ti.Matrix.zero(ezsim.ti_float, 3, 3)
            self.particles.grad[i_f, i_p, i_b].V = ti.Matrix.zero(ezsim.ti_float, 3, 3)
            self.particles.grad[i_f, i_p, i_b].S = ti.Matrix.zero(ezsim.ti_float, 3, 3)
    ...
    def save_ckpt(self, ckpt_name):
        if self._sim.requires_grad:
            if ckpt_name not in self._ckpt:
                ...
                self._ckpt[ckpt_name]["pos"] = torch.zeros((self._B, self._n_particles, 3), dtype=ezsim.tc_float)
                self._ckpt[ckpt_name]["vel"] = torch.zeros((self._B, self._n_particles, 3), dtype=ezsim.tc_float)
                self._ckpt[ckpt_name]["C"] = torch.zeros((self._B, self._n_particles, 3, 3), dtype=ezsim.tc_float)
                self._ckpt[ckpt_name]["F"] = torch.zeros((self._B, self._n_particles, 3, 3), dtype=ezsim.tc_float)
                self._ckpt[ckpt_name]["Jp"] = torch.zeros((self._B, self._n_particles), dtype=ezsim.tc_float)
                self._ckpt[ckpt_name]["active"] = torch.zeros((self._B, self._n_particles), dtype=ezsim.tc_bool)
    ...
    def _kernel_add_particles(
        ...
        for i_p in range(n_particles):
            ...
            self.particles_info[i_global].muscle_direction = ti.Vector([0.0, 0.0, 1.0], dt=ezsim.ti_float)

        for i_p, i_b in ti.ndrange(n_particles, self._B):
            i_global = i_p + particle_start
            self.particles_ng[f, i_global, i_b].active = ti.cast(active, ezsim.ti_bool)
            for j in ti.static(range(3)):
                self.particles[f, i_global, i_b].pos[j] = pos[i_p, j]
            self.particles[f, i_global, i_b].vel = ti.Vector.zero(ezsim.ti_float, 3)
            self.particles[f, i_global, i_b].F = ti.Matrix.identity(ezsim.ti_float, 3)
            self.particles[f, i_global, i_b].C = ti.Matrix.zero(ezsim.ti_float, 3, 3)
            self.particles[f, i_global, i_b].Jp = mat_default_Jp
            self.particles[f, i_global, i_b].actu = ezsim.ti_float(0.0)
    ...
    def _kernel_get_state(
        ...
        for i_p, i_b in ti.ndrange(self._n_particles, self._B):
            ...
            active[i_b, i_p] = ti.cast(self.particles_ng[f, i_p, i_b].active, ezsim.ti_bool)
    ...
    def _kernel_update_render_fields(self, f: ti.i32):
        ...
        for i_v, i_b in ti.ndrange(self._n_vverts, self._B):
            vvert_pos = ti.Vector.zero(ezsim.ti_float, 3)
