...
from ezsim.engine.entities.fem_entity import FEMEntity
from ezsim.engine.states.solvers import FEMSolverState
from ezsim.utils.misc import ti_field_to_torch
from ezsim.utils.geom import ti_transform_by_quat, ti_transform_quat_by_quat
...
class FEMSolver(Solver):
    ...
    def init_element_fields(self):
        ...
        element_info = ti.types.struct(
            el2v=ezsim.ti_ivec4,  # vertex index of an element
            mu=ezsim.ti_float,  # lame parameters (1)
            lam=ezsim.ti_float,  # lame parameters (2)
            mass_scaled=ezsim.ti_float,  # scaled element mass. The real mass is mass_scaled / self._vol_scale
            mat_idx=ezsim.ti_int,  # material model index
            B=ezsim.ti_mat3,  # inverse of the deformation gradient at rest state
            V=ezsim.ti_float,  # rest volume of the element
            V_scaled=ezsim.ti_float,  # scaled rest volume of the element
            friction_mu=ezsim.ti_float,  # friction coefficient for contact
            # for muscle
        ...
        element_state_el_energy = ti.types.struct(
            energy=ezsim.ti_float,  # energy density for the element
            gradient=ezsim.ti_mat3,  # gradient density for the element, del energy / del F
        )

        element_state_v_energy = ti.types.struct(
            inertia=ezsim.ti_vec3,  # inertia for the vertex
            force=ezsim.ti_vec3,
        )

        element_v_info = ti.types.struct(
            mass=ezsim.ti_float,  # mass of the vertex
            mass_inv=ezsim.ti_float,  # inverse mass of the vertex
            mass_over_dt2=ezsim.ti_float,  # scaled mass of the vertex over dt^2
            friction_mu=ezsim.ti_float,  # friction coefficient for contact
        )
        ...
        self.elements_el_hessian = ti.field(
            shape=self._batch_shape((3, 3, self.n_elements), first_dim=True), dtype=ezsim.ti_mat3
        )
    ...
    def init_surface_fields(self):
        ...
        surface_state = ti.types.struct(
            tri2v=ezsim.ti_ivec3,  # vertex index of a triangle
            tri2el=ezsim.ti_int,  # element index of a triangle
            active=ezsim.ti_bool,
        )

        # for rendering (this is more of a surface)
        surface_state_render_v = ti.types.struct(
            vertices=ezsim.ti_vec3,
        )

        surface_state_render_f = ti.types.struct(
            indices=ezsim.ti_int,
        )
    ...
    def _init_surface_info(self):
        self.vertices_on_surface = ti.field(dtype=ezsim.ti_bool, shape=(self.n_vertices,))
        self.elements_on_surface = ti.field(dtype=ezsim.ti_bool, shape=(self.n_elements,))
        ...
        surface_vert_mass_np = mass.diagonal().astype(ezsim.np_float, copy=False)
        self.surface_vert_mass = ti.field(
            dtype=ezsim.ti_float,
            shape=(len(surface_vertices_np),),
            needs_grad=False,
        )
    ...
    def compute_surface_vertices(self):
        for i_v in range(self.n_vertices):
            self.vertices_on_surface[i_v] = False # 0

        for i_s in range(self.n_surfaces):
            tri2v = self.surface[i_s].tri2v
            for i in ti.static(range(3)):
                self.vertices_on_surface[tri2v[i]] = True # 1
    ...
    def init_constraints(self):
        ...
        vertex_constraint_info = ti.types.struct(
            is_constrained=ezsim.ti_bool,  # boolean flag indicating if vertex is constrained
            target_pos=ezsim.ti_vec3,  # target position for the constraint
            is_soft_constraint=ezsim.ti_bool,  # use spring for soft constraints
            stiffness=ezsim.ti_float,  # spring stiffness
            damping=ezsim.ti_float,  # spring damping
            link_idx=ezsim.ti_int,  # index of the rigid link (-1 if not linked)
            link_offset_pos=ezsim.ti_vec3,  # offset position of link
            link_init_quat=ezsim.ti_vec4,  # offset rotation of link
        )
    ...
    def build(self):
        ...
        self._B = self.sim._B
        self.tet_wrong_order = ti.field(dtype=ezsim.ti_bool, shape=(), needs_grad=False)
    ...
    def _func_compute_ele_energy(self, f: ti.i32):
        ...
        for i_b, i_e in ti.ndrange(self._B, self.n_elements):
            ...
            if self._damping_beta > ezsim.EPS:
    ...
    def accumulate_vertex_force_preconditioner(self, f: ti.i32):
        ...
        for i_b, i_v in ti.ndrange(self._B, self.n_vertices):
            ...
            self.pcg_state_v[i_b, i_v].diag3x3 = ti.Matrix.zero(ezsim.ti_float, 3, 3)
            for i in ti.static(range(3)):
        ...
        for i_b, i_e in ti.ndrange(self._B, self.n_elements):
            ...
            if self._damping_beta > ezsim.EPS:
                ...
                S_H_St_x_diff = ti.Vector.zero(ezsim.ti_float, 12)
        ...
        for i_b, i_v in ti.ndrange(self._B, self.n_vertices):
            ...
            # self.pcg_state_v[i_b, i_v].prec = ti.Matrix.identity(ezsim.ti_float, 3)
    ...
    def compute_Ap(self):
        ...
        for i_b, i_e in ti.ndrange(self._B, self.n_elements):
            ...
            p9 = ti.Vector([0.0] * 9, dt=ezsim.ti_float)
            for i, j in ti.static(ti.ndrange(3, 4)):
                p9[i * 3 : i * 3 + 3] = p9[i * 3 : i * 3 + 3] + S[j, i] * self.pcg_state_v[i_b, i_vs[j]].p

            new_p9 = ti.Vector([0.0] * 9, dt=ezsim.ti_float)
    ...
    def substep_pre_coupling_grad(self, f):
        if self.is_active():
            if self._use_implicit_solver:
                ezsim.raise_exception("Gradient computation is not supported for implicit solver.")
        ...
        if self.is_active():
            if not ckpt_name in self._ckpt:
                ...
                self._ckpt[ckpt_name]["pos"] = torch.zeros(
                    self._batch_shape((self.n_vertices, 3), first_dim=True), dtype=ezsim.tc_float
                )
                self._ckpt[ckpt_name]["vel"] = torch.zeros(
                    self._batch_shape((self.n_vertices, 3), first_dim=True), dtype=ezsim.tc_float
                )
                self._ckpt[ckpt_name]["active"] = torch.zeros(
                    self._batch_shape((self.n_elements,), first_dim=True), dtype=ezsim.tc_int
                )
    ...
    def _kernel_add_elements(
        ...
        for i_v, i_b in ti.ndrange(n_verts_local, self._B):
            ...
            self.elements_v[f, i_global, i_b].vel = ti.Vector.zero(ezsim.ti_float, 3)
        ...
        for i_e in range(n_elems_local):
            ...
            self.elements_i[i_global].muscle_direction = ti.Vector([0.0, 0.0, 1.0], dt=ezsim.ti_float)
    ...
    def _kernel_set_vertex_constraints(
        ...
        for i_v_, i_b_ in ti.ndrange(verts_idx.shape[1], envs_idx.shape[0]):
            ...
            self.vertex_constraints[i_v, i_b].is_soft_constraint = ti.cast(is_soft_constraint, ezsim.ti_bool)
