...
class Collider:
    ...
    def _init_collision_fields(self) -> None:
        ...
        self._warn_msg_max_collision_pairs = (
            f"{colors.YELLOW}[EzS] [00:00:00] [WARNING] Ignoring contact pair to avoid exceeding max "
            ...
            for i_pair in range(collider_state.n_broad_pairs[i_b]):
                ...
                if (
                    ...
                    if ti.static(sys.platform == "darwin"):
                        ...
                        if not (
                            geoms_info.type[i_ga] == ezsim.GEOM_TYPE.PLANE and geoms_info.type[i_gb] == ezsim.GEOM_TYPE.BOX
                        ):
                            self_unused._func_convex_convex_contact(
                                links_state,
                                links_info,
    ...
    def _func_box_box_contact(
        ...
        size1 = (
            ti.Vector([geoms_info.data[i_ga][0], geoms_info.data[i_ga][1], geoms_info.data[i_ga][2]], dt=ezsim.ti_float)
            / 2
        )
        size2 = (
            ti.Vector([geoms_info.data[i_gb][0], geoms_info.data[i_gb][1], geoms_info.data[i_gb][2]], dt=ezsim.ti_float)
            / 2
        )
                ...
                if not is_return:
                    ...
                    for i in range(4):
                        if n < collider_static_config.box_MAXCONPAIR:
                            ...
                            if not (tmp1[2] > -ezsim.EPS and c2 > margin2):
                    ...
                    for i in range(4):
                        if n < collider_static_config.box_MAXCONPAIR:
                            ...
                            if nl == 0 and nf == 0:
                                if (x < -lx + ezsim.EPS or x > lx - ezsim.EPS) and (y < -ly + ezsim.EPS or y > ly - ezsim.EPS):
                                    continue
                            elif x < -lx + ezsim.EPS or x > lx - ezsim.EPS or y < -ly + ezsim.EPS or y > ly - ezsim.EPS:
                                continue
                            ...
                            if collider_state.box_pu[i, i_b][2] > -ezsim.EPS and c1 > margin2:
                                continue
