...
import ezsim.engine.solvers.rigid.gjk_decomp as gjk
import ezsim.engine.solvers.rigid.mpr_decomp as mpr
import ezsim.utils.sdf_decomp as sdf
import ezsim.engine.solvers.rigid.support_field_decomp as support_field
...
class Collider:
    ...
    def _init_static_config(self) -> None:
        ...
        self._collider_static_config = Collider.ColliderStaticConfig(
            has_nonconvex_nonterrain=np.logical_and(
                ...
                self._solver.geoms_info.type.to_numpy() != ezsim.GEOM_TYPE.TERRAIN,
            ).any(),
            has_terrain=(self._solver.geoms_info.type.to_numpy() == ezsim.GEOM_TYPE.TERRAIN).any(),
    ...
    def _compute_verts_connectivity(self):
        ...
        if self._solver.n_verts > 0:
            vert_neighbors = np.concatenate(vert_neighbors, dtype=ezsim.np_int)
            vert_neighbor_start = np.concatenate(vert_neighbor_start, dtype=ezsim.np_int)
            vert_n_neighbors = np.concatenate(vert_n_neighbors, dtype=ezsim.np_int)
    ...
    def _init_terrain_state(self):
        if self._collider_static_config.has_terrain:
            ...
            scale = entity.terrain_scale.astype(ezsim.np_float)
            rc = np.array(entity.terrain_hf.shape, dtype=ezsim.np_int)
            hf = entity.terrain_hf.astype(ezsim.np_float) * scale[1]
            xyz_maxmin = np.array(
    ...
    def get_contacts(self, as_tensor: bool = True, to_torch: bool = True, keep_batch_dim: bool = False):
        ...
        if to_torch:
            iout = torch.full((out_size, 4), -1, dtype=ezsim.tc_int, device=ezsim.device)
            fout = torch.zeros((out_size, 10), dtype=ezsim.tc_float, device=ezsim.device)
        else:
            iout = np.full((out_size, 4), -1, dtype=ezsim.np_int)
            fout = np.zeros((out_size, 10), dtype=ezsim.np_float)
...
def rotaxis(vecin, i0, i1, i2, f0, f1, f2):
    vecres = ti.Vector([0.0, 0.0, 0.0], dt=ezsim.ti_float)
...
def rotmatx(matin, i0, i1, i2, f0, f1, f2):
    matres = ti.Matrix.zero(ezsim.ti_float, 3, 3)
    ...
    for i_b_ in range(envs_idx.shape[0]):
        ...
        for i_ga in range(n_geoms):
            for i_gb in range(n_geoms):
                ...
                collider_state.contact_cache.normal[i_ga, i_gb, i_b] = ti.Vector.zero(ezsim.ti_float, 3)
...
def func_contact_sphere_sdf(
    ...
    is_col = False
    penetration = ezsim.ti_float(0.0)
    normal = ti.Vector.zero(ezsim.ti_float, 3)
    contact_pos = ti.Vector.zero(ezsim.ti_float, 3)
...
def func_contact_mpr_terrain(
    ...
    if not is_return:
        geoms_state.pos[i_ga, i_b], geoms_state.quat[i_ga, i_b] = gu.ti_transform_pos_quat_by_trans_quat(
            ...
            ga_quat,
            ti.Vector.zero(ezsim.ti_float, 3),
            gu.ti_inv_quat(geoms_state.quat[i_gb, i_b]),
        )

        for i_axis, i_m in ti.ndrange(3, 2):
            direction = ti.Vector.zero(ezsim.ti_float, 3)
            if i_m == 0:
        ...
        if not is_return:
            ...
            r_min = ezsim.ti_int(ti.floor((collider_state.xyz_max_min[3, i_b] - collider_info.terrain_xyz_maxmin[3]) / sh))
            r_max = ezsim.ti_int(ti.ceil((collider_state.xyz_max_min[0, i_b] - collider_info.terrain_xyz_maxmin[3]) / sh))
            c_min = ezsim.ti_int(ti.floor((collider_state.xyz_max_min[4, i_b] - collider_info.terrain_xyz_maxmin[4]) / sh))
            c_max = ezsim.ti_int(ti.ceil((collider_state.xyz_max_min[1, i_b] - collider_info.terrain_xyz_maxmin[4]) / sh))
                    ...
                    for i in range(2):
                        if n_con < ti.static(collider_static_config.n_contacts_per_pair):
                            ...
                            if nvert > 2 and (
                                ...
                                center_b = ti.Vector.zero(ezsim.ti_float, 3)
                                for i_p in ti.static(range(6)):
                                    center_b = center_b + collider_state.prism[i_p, i_b]
                                center_b = center_b / 6.0

                                geoms_state.pos[i_gb, i_b] = ti.Vector.zero(ezsim.ti_float, 3)
...
def func_narrow_phase_convex_vs_convex(
    ...
    Therefore, for a big non-batched scene, users are encouraged to simply use `ezsim.cpu` backend.
    ...
    for i_b in range(_B):
        for i_pair in range(collider_state.n_broad_pairs[i_b]):
            ...
            if (
                ...
                and not geoms_info.type[i_gb] == ezsim.GEOM_TYPE.TERRAIN
                and not (
                    static_rigid_sim_config.box_box_detection
                    and geoms_info.type[i_ga] == ezsim.GEOM_TYPE.BOX
                    and geoms_info.type[i_gb] == ezsim.GEOM_TYPE.BOX
                )
            ):
                if ti.static(sys.platform == "darwin"):
                    ...
                    if not (geoms_info.type[i_ga] == ezsim.GEOM_TYPE.PLANE and geoms_info.type[i_gb] == ezsim.GEOM_TYPE.BOX):
                        func_convex_convex_contact(
                            i_ga=i_ga,
                            i_gb=i_gb,
                            i_b=i_b,
...
def func_narrow_phase_convex_specializations(
    ...
    for i_b in range(_B):
        for i_pair in range(collider_state.n_broad_pairs[i_b]):
            ...
            if ti.static(sys.platform != "darwin"):
                if geoms_info.type[i_ga] == ezsim.GEOM_TYPE.PLANE and geoms_info.type[i_gb] == ezsim.GEOM_TYPE.BOX:
            ...
            if ti.static(static_rigid_sim_config.box_box_detection):
                if geoms_info.type[i_ga] == ezsim.GEOM_TYPE.BOX and geoms_info.type[i_gb] == ezsim.GEOM_TYPE.BOX:
    ...
    for i_b in range(_B):
        for i_pair in range(collider_state.n_broad_pairs[i_b]):
            ...
            if ti.static(collider_static_config.has_terrain):
                if geoms_info.type[i_ga] == ezsim.GEOM_TYPE.TERRAIN:
                    i_ga, i_gb = i_gb, i_ga

                if geoms_info.type[i_gb] == ezsim.GEOM_TYPE.TERRAIN:
        ...
        for i_pair in range(collider_state.n_broad_pairs[i_b]):
            ...
            if ti.static(collider_static_config.has_nonconvex_nonterrain):
                if (
                    ...
                    and geoms_info.type[i_gb] != ezsim.GEOM_TYPE.TERRAIN
                ):
                    is_col = False
                    ...
                    for i in range(2):
                        ...
                        # initial point
                        is_col_i = False
                        normal_i = ti.Vector.zero(ezsim.ti_float, 3)
                        contact_pos_i = ti.Vector.zero(ezsim.ti_float, 3)
                        if not is_col:
...
def func_plane_box_contact(
    ...
    plane_dir = ti.Vector(
        [geoms_info.data[i_ga][0], geoms_info.data[i_ga][1], geoms_info.data[i_ga][2]], dt=ezsim.ti_float
    )
...
def func_compute_tolerance(
    ...
    if geoms_info.type[i_ga] != ezsim.GEOM_TYPE.PLANE:
...
def func_contact_orthogonals(
    ...
    axis_0 = ti.Vector.zero(ezsim.ti_float, 3)
    axis_1 = ti.Vector.zero(ezsim.ti_float, 3)
    ...
    if ti.static(static_rigid_sim_config.enable_mujoco_compatibility):
        ...
        axis_idx = ezsim.ti_int(0)
        axis_angle_max = ezsim.ti_float(0.0)
        for i in ti.static(range(3)):
...
def func_convex_convex_contact(
    ...
    if geoms_info.type[i_ga] == ezsim.GEOM_TYPE.PLANE and geoms_info.type[i_gb] == ezsim.GEOM_TYPE.BOX:
        ...
        multi_contact = (
            ...
            and geoms_info.type[i_ga] != ezsim.GEOM_TYPE.SPHERE
            and geoms_info.type[i_ga] != ezsim.GEOM_TYPE.ELLIPSOID
            and geoms_info.type[i_gb] != ezsim.GEOM_TYPE.SPHERE
            and geoms_info.type[i_gb] != ezsim.GEOM_TYPE.ELLIPSOID
        )
        ...
        is_col_0 = False
        penetration_0 = ezsim.ti_float(0.0)
        normal_0 = ti.Vector.zero(ezsim.ti_float, 3)
        contact_pos_0 = ti.Vector.zero(ezsim.ti_float, 3)
        ...
        n_con = ezsim.ti_int(0)
        axis_0 = ti.Vector.zero(ezsim.ti_float, 3)
        axis_1 = ti.Vector.zero(ezsim.ti_float, 3)
        qrot = ti.Vector.zero(ezsim.ti_float, 4)
            ...
            if (multi_contact and is_col_0) or (i_detection == 0):
                    ...
                    if ti.static(
                        ...
                        for i_mpr in range(2):
                            if i_mpr == 1:
                                ...
                                is_mpr_guess_direction_available = (ti.abs(normal_ws) > ezsim.EPS).any()
                                if (i_detection == 0) and not is_col and is_mpr_guess_direction_available:
                                    normal_ws = ti.Vector.zero(ezsim.ti_float, 3)
                                    is_mpr_updated = False
                ...
                if ti.static(collider_static_config.ccd_algorithm == CCD_ALGORITHM_CODE.MPR):
                    if try_sdf:
                        is_col_a = False
                        is_col_b = False
                        normal_a = ti.Vector.zero(ezsim.ti_float, 3)
                        normal_b = ti.Vector.zero(ezsim.ti_float, 3)
                        penetration_b = ezsim.ti_float(0.0)
                        penetration_a = ezsim.ti_float(0.0)
                        contact_pos_a = ti.Vector.zero(ezsim.ti_float, 3)
                        contact_pos_b = ti.Vector.zero(ezsim.ti_float, 3)
...
def func_box_box_contact(
    ...
    cle1, cle2 = 0, 0
    in_ = 0
    tmp2 = ti.Vector.zero(ezsim.ti_float, 3)
    margin2 = margin * margin
    rotmore = ti.Matrix.zero(ezsim.ti_float, 3, 3)
    ...
    size1 = (
        ti.Vector([geoms_info.data[i_ga][0], geoms_info.data[i_ga][1], geoms_info.data[i_ga][2]], dt=ezsim.ti_float) / 2
    )
    size2 = (
        ti.Vector([geoms_info.data[i_gb][0], geoms_info.data[i_gb][1], geoms_info.data[i_gb][2]], dt=ezsim.ti_float) / 2
    )
    ...
    clnorm = ti.Vector([0.0, 0.0, 0.0], dt=ezsim.ti_float)
    for i, j in ti.static(ti.ndrange(3, 3)):
        rj0 = rott[j, 0]
        rj1 = rott[j, 1]
        rj2 = rott[j, 2]
        if i == 0:
            tmp2 = ti.Vector([0.0, -rj2, +rj1], dt=ezsim.ti_float)
        elif i == 1:
            tmp2 = ti.Vector([+rj2, 0.0, -rj0], dt=ezsim.ti_float)
        else:
            tmp2 = ti.Vector([-rj1, +rj0, 0.0], dt=ezsim.ti_float)
    ...
    if not is_return:
        if code < 12:
            ...
            r = ti.Matrix.zero(ezsim.ti_float, 3, 3)
            p = ti.Vector.zero(ezsim.ti_float, 3)
            s = ti.Vector.zero(ezsim.ti_float, 3)
            if q2:
