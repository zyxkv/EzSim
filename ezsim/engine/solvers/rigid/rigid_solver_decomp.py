...
import ezsim.engine.solvers.rigid.array_class as array_class
...
class RigidSolver(Solver):
    ...
    def _kernel_control_dofs_force(
        ...
        ti.loop_config(serialize=self._para_level < ezsim.PARA_LEVEL.PARTIAL)
        for i_d_, i_b_ in ti.ndrange(dofs_idx.shape[0], envs_idx.shape[0]):
            self.dofs_state.ctrl_mode[dofs_idx[i_d_], envs_idx[i_b_]] = ezsim.CTRL_MODE.FORCE
    ...
    def _kernel_control_dofs_velocity(
        ...
        ti.loop_config(serialize=self._para_level < ezsim.PARA_LEVEL.PARTIAL)
        for i_d_, i_b_ in ti.ndrange(dofs_idx.shape[0], envs_idx.shape[0]):
            self.dofs_state.ctrl_mode[dofs_idx[i_d_], envs_idx[i_b_]] = ezsim.CTRL_MODE.VELOCITY
    ...
    def _kernel_control_dofs_position(
        ...
        ti.loop_config(serialize=self._para_level < ezsim.PARA_LEVEL.PARTIAL)
        for i_d_, i_b_ in ti.ndrange(dofs_idx.shape[0], envs_idx.shape[0]):
            self.dofs_state.ctrl_mode[dofs_idx[i_d_], envs_idx[i_b_]] = ezsim.CTRL_MODE.POSITION
    ...
    def _kernel_get_dofs_control_force(
        ...
        for i_d_, i_b_ in ti.ndrange(dofs_idx.shape[0], envs_idx.shape[0]):
            ...
            force = ezsim.ti_float(0.0)
            if self.dofs_state.ctrl_mode[i_d, i_b] == ezsim.CTRL_MODE.FORCE:
                force = self.dofs_state.ctrl_force[i_d, i_b]
            elif self.dofs_state.ctrl_mode[i_d, i_b] == ezsim.CTRL_MODE.VELOCITY:
                force = self.dofs_info.kv[I_d] * (self.dofs_state.ctrl_vel[i_d, i_b] - self.dofs_state.vel[i_d, i_b])
            elif self.dofs_state.ctrl_mode[i_d, i_b] == ezsim.CTRL_MODE.POSITION:
                force = (
    ...
    def _kernel_add_weld_constraint(
        ...
        for i_b_ in ti.ndrange(envs_idx.shape[0]):
            ...
            if i_e == self.n_equalities_candidate:
                print(
                    f"{colors.YELLOW}[EzS] [00:00:00] [WARNING] Ignoring dynamically registered weld constraint "
                ...
                self.equalities_info.eq_type[i_e, i_b] = ezsim.ti_int(ezsim.EQUALITY_TYPE.WELD)
    ...
    def _kernel_delete_weld_constraint(
        ...
        for i_b_ in ti.ndrange(envs_idx.shape[0]):
            ...
            for i_e in range(self.n_equalities, self.constraint_solver.ti_n_equalities[i_b]):
                if (
                    self.equalities_info.eq_type[i_e, i_b] == ezsim.EQUALITY_TYPE.WELD
