...
from ezsim.options.solvers import RigidOptions
import ezsim.utils.geom as gu
from ezsim.utils import linalg as lu
from ezsim.utils.misc import ti_field_to_torch, DeprecationError, ALLOCATE_TENSOR_WARNING
from ezsim.engine.entities import AvatarEntity, DroneEntity, RigidEntity
from ezsim.engine.states.solvers import RigidSolverState
...
if TYPE_CHECKING:
    import ezsim.engine.solvers.rigid.array_class
...
def _sanitize_sol_params(
    ...
    if (timeconst < ezsim.EPS).any():
        ezsim.logger.debug(
            f"Constraint solver time constant not specified. Using default value (`{default_timeconst:0.6g}`)."
        )
    invalid_mask = (timeconst > ezsim.EPS) & (timeconst + ezsim.EPS < min_timeconst)
    if invalid_mask.any():
    ...
    timeconst[timeconst < ezsim.EPS] = default_timeconst
...
class RigidSolver(Solver):
    # override typing
    _entities: list[RigidEntity] = ezsim.List()
    ...
    class StaticRigidSimConfig:
        ...
        # integrator: ezsim.integrator = ezsim.integrator.implicitfast
        # sparse_solve: bool = False
        # solver_type: ezsim.constraint_solver = ezsim.constraint_solver.CG
        # # dynamic properties
    ...
    def add_entity(self, idx, material, morph, surface, visualize_contact) -> Entity:
        if isinstance(material, ezsim.materials.Avatar):
            EntityClass = AvatarEntity
            if visualize_contact:
                ezsim.raise_exception("AvatarEntity does not support 'visualize_contact=True'.")
        elif isinstance(morph, ezsim.morphs.Drone):
    ...
    def build(self):
        ...
        self._base_links_idx = torch.tensor(base_links_idx, dtype=ezsim.tc_int, device=ezsim.device)
        ...
        self._static_rigid_sim_config = self.StaticRigidSimConfig(
            ...
            integrator=getattr(self, "_integrator", ezsim.integrator.implicitfast),
            sparse_solve=getattr(self._options, "sparse_solve", False),
            solver_type=getattr(self._options, "constraint_solver", ezsim.constraint_solver.CG),
            # dynamic properties
    ...
    def _init_invweight(self):
        ...
        links_invweight = np.zeros((self._n_links, 2), dtype=ezsim.np_float)
        ...
        dofs_invweight = np.zeros((self._n_dofs,), dtype=ezsim.np_float)
        for i_l in range(self._n_links):
            for i_j in range(links_joint_start[i_l], links_joint_end[i_l]):
                joint_type = joints_type[i_j]
                if joint_type == ezsim.JOINT_TYPE.FIXED:
                    continue
    ...
    def _init_mass_mat(self):
        ...
        # self.mass_mat = ti.field(dtype=ezsim.ti_float, shape=self._batch_shape((self.n_dofs_, self.n_dofs_)))
        # self.mass_mat_L = ti.field(dtype=ezsim.ti_float, shape=self._batch_shape((self.n_dofs_, self.n_dofs_)))
        # self.mass_mat_D_inv = ti.field(dtype=ezsim.ti_float, shape=self._batch_shape((self.n_dofs_,)))

        # self._mass_mat_mask = ti.field(dtype=ezsim.ti_int, shape=self._batch_shape(self.n_entities_))
        self._rigid_global_info._mass_mat_mask.fill(1)

        # self.meaninertia = ti.field(dtype=ezsim.ti_float, shape=self._batch_shape())

        # tree structure information
        mass_parent_mask = np.zeros((self.n_dofs_, self.n_dofs_), dtype=ezsim.np_float)
        ...
        # self.mass_parent_mask = ti.field(dtype=ezsim.ti_float, shape=(self.n_dofs_, self.n_dofs_))
    ...
    def _init_dof_fields(self):
        ...
        #     stiffness=ezsim.ti_float,
        #     invweight=ezsim.ti_float,
        #     armature=ezsim.ti_float,
        #     damping=ezsim.ti_float,
        #     motion_ang=ezsim.ti_vec3,
        #     motion_vel=ezsim.ti_vec3,
        #     limit=ezsim.ti_vec2,
        #     dof_start=ezsim.ti_int,  # dof_start of its entity
        #     kp=ezsim.ti_float,
        #     kv=ezsim.ti_float,
        #     force_range=ezsim.ti_vec2,
        # )

        # struct_dof_state = ti.types.struct(
        #     force=ezsim.ti_float,
        #     qf_bias=ezsim.ti_float,
        #     qf_passive=ezsim.ti_float,
        #     qf_actuator=ezsim.ti_float,
        #     qf_applied=ezsim.ti_float,
        #     act_length=ezsim.ti_float,
        #     pos=ezsim.ti_float,
        #     vel=ezsim.ti_float,
        #     acc=ezsim.ti_float,
        #     acc_smooth=ezsim.ti_float,
        #     qf_smooth=ezsim.ti_float,
        #     qf_constraint=ezsim.ti_float,
        #     cdof_ang=ezsim.ti_vec3,
        #     cdof_vel=ezsim.ti_vec3,
        #     cdofvel_ang=ezsim.ti_vec3,
        #     cdofvel_vel=ezsim.ti_vec3,
        #     cdofd_ang=ezsim.ti_vec3,
        #     cdofd_vel=ezsim.ti_vec3,
        #     f_vel=ezsim.ti_vec3,
        #     f_ang=ezsim.ti_vec3,
        #     ctrl_force=ezsim.ti_float,
        #     ctrl_pos=ezsim.ti_float,
        #     ctrl_vel=ezsim.ti_float,
        #     ctrl_mode=ezsim.ti_int,
        #     hibernated=ezsim.ti_int,  # Flag for dofs that converge into a static state (hibernation)
        # )
        ...
        if has_dofs:  # handle the case where there is a link with no dofs -- otherwise may cause invalid memory
            kernel_init_dof_fields(
                dofs_motion_ang=np.concatenate([joint.dofs_motion_ang for joint in joints], dtype=ezsim.np_float),
                dofs_motion_vel=np.concatenate([joint.dofs_motion_vel for joint in joints], dtype=ezsim.np_float),
                dofs_limit=np.concatenate([joint.dofs_limit for joint in joints], dtype=ezsim.np_float),
                ...
                dofs_force_range=np.concatenate([joint.dofs_force_range for joint in joints], dtype=ezsim.np_float),
                dofs_info=self.dofs_info,
                dofs_state=self.dofs_state,
                rigid_global_info=self._rigid_global_info,
                static_rigid_sim_config=self._static_rigid_sim_config,
    ...
    def _init_link_fields(self):
        ...
        kernel_init_link_fields(
            links_parent_idx=np.array([link.parent_idx for link in links], dtype=ezsim.np_int),
            links_root_idx=np.array([link.root_idx for link in links], dtype=ezsim.np_int),
            links_q_start=np.array([link.q_start for link in links], dtype=ezsim.np_int),
            links_dof_start=np.array([link.dof_start for link in links], dtype=ezsim.np_int),
            links_joint_start=np.array([link.joint_start for link in links], dtype=ezsim.np_int),
            links_q_end=np.array([link.q_end for link in links], dtype=ezsim.np_int),
            links_dof_end=np.array([link.dof_end for link in links], dtype=ezsim.np_int),
            links_joint_end=np.array([link.joint_end for link in links], dtype=ezsim.np_int),
            links_invweight=np.array([link.invweight for link in links], dtype=ezsim.np_float),
            # links_is_fixed=np.array([link.is_fixed for link in links], dtype=ezsim.np_int),
            links_is_fixed=np.array([link.is_fixed for link in links], dtype=ezsim.np_bool),
            links_pos=np.array([link.pos for link in links], dtype=ezsim.np_float),
            links_quat=np.array([link.quat for link in links], dtype=ezsim.np_float),
            links_inertial_pos=np.array([link.inertial_pos for link in links], dtype=ezsim.np_float),
            links_inertial_quat=np.array([link.inertial_quat for link in links], dtype=ezsim.np_float),
            links_inertial_i=np.array([link.inertial_i for link in links], dtype=ezsim.np_float),
            links_inertial_mass=np.array([link.inertial_mass for link in links], dtype=ezsim.np_float),
            links_entity_idx=np.array([link._entity_idx_in_solver for link in links], dtype=ezsim.np_int),
            # taichi variables
            links_info=self.links_info,
            links_state=self.links_state,
            rigid_global_info=self._rigid_global_info,
        ...
        if joints:
            ...
            joints_sol_params = np.array([joint.sol_params for joint in joints], dtype=ezsim.np_float)
            _sanitize_sol_params(joints_sol_params, self._sol_min_timeconst, self._sol_default_timeconst)

            kernel_init_joint_fields(
                joints_type=np.array([joint.type for joint in joints], dtype=ezsim.np_int),
                joints_sol_params=joints_sol_params,
                joints_q_start=np.array([joint.q_start for joint in joints], dtype=ezsim.np_int),
                joints_dof_start=np.array([joint.dof_start for joint in joints], dtype=ezsim.np_int),
                joints_q_end=np.array([joint.q_end for joint in joints], dtype=ezsim.np_int),
                joints_dof_end=np.array([joint.dof_end for joint in joints], dtype=ezsim.np_int),
                joints_pos=np.array([joint.pos for joint in joints], dtype=ezsim.np_float),
                # taichi variables
                joints_info=self.joints_info,
                static_rigid_sim_config=self._static_rigid_sim_config,
            )
        ...
        if self.n_qs > 0:
            init_qpos = self._batch_array(self.init_qpos)
            for joint in joints:
                if joint.type in (ezsim.JOINT_TYPE.REVOLUTE, ezsim.JOINT_TYPE.PRISMATIC):
        ...
        # self._rigid_global_info.links_T = ti.Matrix.field(n=4, m=4, dtype=ezsim.ti_float, shape=self.n_links)
    ...
    def _init_vert_fields(self):
        ...
        if self.n_verts > 0:
            ...
            kernel_init_vert_fields(
                verts=np.concatenate([geom.init_verts for geom in geoms], dtype=ezsim.np_float),
                faces=np.concatenate([geom.init_faces + geom.vert_start for geom in geoms], dtype=ezsim.np_int),
                edges=np.concatenate([geom.init_edges + geom.vert_start for geom in geoms], dtype=ezsim.np_int),
                normals=np.concatenate([geom.init_normals for geom in geoms], dtype=ezsim.np_float),
                verts_geom_idx=np.concatenate([np.full(geom.n_verts, geom.idx) for geom in geoms], dtype=ezsim.np_int),
                init_center_pos=np.concatenate([geom.init_center_pos for geom in geoms], dtype=ezsim.np_float),
                verts_state_idx=np.concatenate(
                    [np.arange(geom.verts_state_start, geom.verts_state_start + geom.n_verts) for geom in geoms],
                    dtype=ezsim.np_int,
                ),
                is_free=np.concatenate([np.full(geom.n_verts, geom.is_free) for geom in geoms], dtype=ezsim.np_int),
                # taichi variables
    ...
    def _init_vvert_fields(self):
        ...
        if self.n_vverts > 0:
            ...
            kernel_init_vvert_fields(
                vverts=np.concatenate([vgeom.init_vverts for vgeom in vgeoms], dtype=ezsim.np_float),
                vfaces=np.concatenate([vgeom.init_vfaces + vgeom.vvert_start for vgeom in vgeoms], dtype=ezsim.np_int),
                vnormals=np.concatenate([vgeom.init_vnormals for vgeom in vgeoms], dtype=ezsim.np_float),
                vverts_vgeom_idx=np.concatenate(
                    [np.full(vgeom.n_vverts, vgeom.idx) for vgeom in vgeoms], dtype=ezsim.np_int
                ),
                # taichi variables
    ...
    def _init_geom_fields(self):
        ...
        if self.n_geoms > 0:
            # Make sure that the constraints parameters are valid
            geoms = self.geoms
            geoms_sol_params = np.array([geom.sol_params for geom in geoms], dtype=ezsim.np_float)
            ...
            kernel_init_geom_fields(
                geoms_pos=np.array([geom.init_pos for geom in geoms], dtype=ezsim.np_float),
                geoms_center=np.array(geoms_center, dtype=ezsim.np_float),
                geoms_quat=np.array([geom.init_quat for geom in geoms], dtype=ezsim.np_float),
                geoms_link_idx=np.array([geom.link.idx for geom in geoms], dtype=ezsim.np_int),
                geoms_type=np.array([geom.type for geom in geoms], dtype=ezsim.np_int),
                geoms_friction=np.array([geom.friction for geom in geoms], dtype=ezsim.np_float),
                geoms_sol_params=geoms_sol_params,
                geoms_vert_start=np.array([geom.vert_start for geom in geoms], dtype=ezsim.np_int),
                geoms_face_start=np.array([geom.face_start for geom in geoms], dtype=ezsim.np_int),
                geoms_edge_start=np.array([geom.edge_start for geom in geoms], dtype=ezsim.np_int),
                geoms_verts_state_start=np.array([geom.verts_state_start for geom in geoms], dtype=ezsim.np_int),
                geoms_vert_end=np.array([geom.vert_end for geom in geoms], dtype=ezsim.np_int),
                geoms_face_end=np.array([geom.face_end for geom in geoms], dtype=ezsim.np_int),
                geoms_edge_end=np.array([geom.edge_end for geom in geoms], dtype=ezsim.np_int),
                geoms_verts_state_end=np.array([geom.verts_state_end for geom in geoms], dtype=ezsim.np_int),
                geoms_data=np.array([geom.data for geom in geoms], dtype=ezsim.np_float),
                geoms_is_convex=np.array([geom.is_convex for geom in geoms], dtype=ezsim.np_int),
                geoms_needs_coup=np.array([geom.needs_coup for geom in geoms], dtype=ezsim.np_int),
                ...
                geoms_coup_softness=np.array([geom.coup_softness for geom in geoms], dtype=ezsim.np_float),
                geoms_coup_friction=np.array([geom.coup_friction for geom in geoms], dtype=ezsim.np_float),
                geoms_coup_restitution=np.array([geom.coup_restitution for geom in geoms], dtype=ezsim.np_float),
                geoms_is_free=np.array([geom.is_free for geom in geoms], dtype=ezsim.np_int),
                geoms_is_decomp=np.array([geom.metadata.get("decomposed", False) for geom in geoms], dtype=ezsim.np_int),
                # taichi variables
                geoms_info=self.geoms_info,
                geoms_state=self.geoms_state,
                verts_info=self.verts_info,
    ...
    def _init_vgeom_fields(self):
        ...
        if self.n_vgeoms > 0:
            ...
            kernel_init_vgeom_fields(
                vgeoms_pos=np.array([vgeom.init_pos for vgeom in vgeoms], dtype=ezsim.np_float),
                vgeoms_quat=np.array([vgeom.init_quat for vgeom in vgeoms], dtype=ezsim.np_float),
                vgeoms_link_idx=np.array([vgeom.link.idx for vgeom in vgeoms], dtype=ezsim.np_int),
                vgeoms_vvert_start=np.array([vgeom.vvert_start for vgeom in vgeoms], dtype=ezsim.np_int),
                vgeoms_vface_start=np.array([vgeom.vface_start for vgeom in vgeoms], dtype=ezsim.np_int),
                vgeoms_vvert_end=np.array([vgeom.vvert_end for vgeom in vgeoms], dtype=ezsim.np_int),
                vgeoms_vface_end=np.array([vgeom.vface_end for vgeom in vgeoms], dtype=ezsim.np_int),
                vgeoms_color=np.array([vgeom._color for vgeom in vgeoms], dtype=ezsim.np_float),
                # taichi variables
    ...
    def _init_entity_fields(self):
        # if self._use_hibernation:
        #     self.n_awake_entities = ti.field(dtype=ezsim.ti_int, shape=self._B)
        #     self.awake_entities = ti.field(dtype=ezsim.ti_int, shape=self._batch_shape(self.n_entities_))

        # struct_entity_info = ti.types.struct(
        #     dof_start=ezsim.ti_int,
        #     dof_end=ezsim.ti_int,
        #     n_dofs=ezsim.ti_int,
        #     link_start=ezsim.ti_int,
        #     link_end=ezsim.ti_int,
        #     n_links=ezsim.ti_int,
        #     geom_start=ezsim.ti_int,
        #     geom_end=ezsim.ti_int,
        #     n_geoms=ezsim.ti_int,
        #     gravity_compensation=ezsim.ti_float,
        # )

        # struct_entity_state = ti.types.struct(
        #     hibernated=ezsim.ti_int,
        # )
        ...
        kernel_init_entity_fields(
            entities_dof_start=np.array([entity.dof_start for entity in entities], dtype=ezsim.np_int),
            entities_dof_end=np.array([entity.dof_end for entity in entities], dtype=ezsim.np_int),
            entities_link_start=np.array([entity.link_start for entity in entities], dtype=ezsim.np_int),
            entities_link_end=np.array([entity.link_end for entity in entities], dtype=ezsim.np_int),
            entities_geom_start=np.array([entity.geom_start for entity in entities], dtype=ezsim.np_int),
    ...
    def _init_equality_fields(self):
        ...
        if self.n_equalities > 0:
            ...
            equalities_sol_params = np.array([equality.sol_params for equality in equalities], dtype=ezsim.np_float)
            ...
            kernel_init_equality_fields(
                equalities_type=np.array([equality.type for equality in equalities], dtype=ezsim.np_int),
                equalities_eq_obj1id=np.array([equality.eq_obj1id for equality in equalities], dtype=ezsim.np_int),
                equalities_eq_obj2id=np.array([equality.eq_obj2id for equality in equalities], dtype=ezsim.np_int),
                equalities_eq_data=np.array([equality.eq_data for equality in equalities], dtype=ezsim.np_float),
                equalities_eq_type=np.array([equality.type for equality in equalities], dtype=ezsim.np_int),
            ...
            if self._use_contact_island:
                ezsim.logger.warn("contact island is not supported for equality constraints yet")
    ...
    def _init_collider(self):
        ...
        if self.collider._collider_static_config.has_terrain:
            links_idx = self.geoms_info.link_idx.to_numpy()[self.geoms_info.type.to_numpy() == ezsim.GEOM_TYPE.TERRAIN]
            entity = self._entities[self.links_info.entity_idx.to_numpy()[links_idx[0]]]

            scale = entity.terrain_scale
            rc = np.array(entity.terrain_hf.shape, dtype=ezsim.np_int)
            hf = entity.terrain_hf.astype(ezsim.np_float, copy=False) * scale[1]
            xyz_maxmin = np.array(
                [rc[0] * scale[0], rc[1] * scale[0], hf.max(), 0, 0, hf.min() - 1.0],
                dtype=ezsim.np_float,
            )

            self.terrain_hf = ti.field(dtype=ezsim.ti_float, shape=hf.shape)
            self.terrain_rc = ti.field(dtype=ezsim.ti_int, shape=2)
            self.terrain_scale = ti.field(dtype=ezsim.ti_float, shape=2)
            self.terrain_xyz_maxmin = ti.field(dtype=ezsim.ti_float, shape=6)
    ...
    #             ti.loop_config(serialize=static_rigid_sim_config.para_level < ezsim.PARA_LEVEL.ALL)
    #             for i_b in range(_B):
    ...
    def _sanitize_1D_io_variables(
        ...
        *,
        ...
        if batched:
            ...
            envs_idx = torch.empty((0,), dtype=ezsim.tc_int, device=ezsim.device)
        ...
        if not is_preallocated and not skip_allocation:
            ...
            tensor = torch.empty(shape, dtype=ezsim.tc_float, device=ezsim.device)
        ...
        _inputs_idx = torch.as_tensor(inputs_idx, dtype=ezsim.tc_int, device=ezsim.device).contiguous()
        ...
        if _inputs_idx.ndim != 1:
            ezsim.raise_exception(f"Expecting 1D tensor for `{idx_name}`.")
        if len(inputs_idx):
            inputs_start, inputs_end = min(inputs_idx), max(inputs_idx)
            if inputs_start < 0 or input_size <= inputs_end:
                ezsim.raise_exception(f"`{idx_name}` is out-of-range.")

        if is_preallocated:
            _tensor = torch.as_tensor(tensor, dtype=ezsim.tc_float, device=ezsim.device).contiguous()
            ...
            if tensor.shape[-1] != len(inputs_idx):
                ezsim.raise_exception(f"Last dimension of the input tensor does not match length of `{idx_name}`.")
            ...
            if batched:
                ...
                if tensor.ndim != 1:
                    ezsim.raise_exception("Expecting 1D output tensor.")
    ...
    def _sanitize_2D_io_variables(
        ...
        *,
        ...
        if batched:
            ...
            envs_idx = torch.empty((), dtype=ezsim.tc_int, device=ezsim.device)
        ...
        if is_preallocated:
            ...
            if tensor.shape[-2] != len(inputs_idx):
                ezsim.raise_exception(f"Second last dimension of the input tensor does not match length of `{idx_name}`.")
            if tensor.shape[-1] != vec_size:
                ezsim.raise_exception(f"Last dimension of the input tensor must be {vec_size}.")
            ...
            if batched:
                ...
                if tensor.ndim != 2:
                    ezsim.raise_exception("Expecting 2D input tensor.")
        return tensor, _inputs_idx, envs_idx
    ...
    def set_base_links_pos(
        ...
        if not unsafe and not torch.isin(links_idx, self._base_links_idx).all():
            ezsim.raise_exception("`links_idx` contains at least one link that is not a base link.")
    ...
    def set_global_sol_params(self, sol_params, *, unsafe=False):
        ...
        if not unsafe:
            _sol_params = torch.as_tensor(sol_params, dtype=ezsim.tc_float, device=ezsim.device).contiguous()
    ...
    def set_sol_params(self, sol_params, geoms_idx=None, envs_idx=None, *, joints_idx=None, eqs_idx=None, unsafe=False):
        ...
        if sum(inputs_idx is not None for inputs_idx in (geoms_idx, joints_idx, eqs_idx)) > 1:
            raise ezsim.raise_exception("Cannot set more than one constraint type at once.")
    ...
    def _set_dofs_info(self, tensor_list, dofs_idx, name, envs_idx=None, *, unsafe=False):
        ...
        if name == "kp":
            ...
            ezsim.raise_exception(f"Invalid `name` {name}.")
    ...
    def get_links_inertial_mass(self, links_idx=None, envs_idx=None, *, unsafe=False):
        if not unsafe and self._options.batch_links_info and envs_idx is not None:
            ezsim.raise_exception("`envs_idx` cannot be specified for non-batched links info.")
    ...
    def get_dofs_kp(self, dofs_idx=None, envs_idx=None, *, unsafe=False):
        if not unsafe and not self._options.batch_dofs_info and envs_idx is not None:
            ezsim.raise_exception("`envs_idx` cannot be specified for non-batched dofs info.")
    ...
    #     if to_torch:
    #         buf = torch.full((out_size, 3), -1, dtype=ezsim.tc_int, device=ezsim.device)
    #     else:
    ...
    #     else:
    #         if to_torch:
    #             obj_a = torch.empty((0,), dtype=ezsim.tc_int, device=ezsim.device)
    #             obj_b = torch.empty((0,), dtype=ezsim.tc_int, device=ezsim.device)
    #         else:
    #             obj_a = []
    #             obj_b = []
    ...
    def links(self):
        if self.is_built:
            return self._links
        return ezsim.List(link for entity in self._entities for link in entity.links)

    @property
    def joints(self):
        if self.is_built:
            return self._joints
        return ezsim.List(joint for entity in self._entities for joint in entity.joints)

    @property
    def geoms(self):
        if self.is_built:
            return self._geoms
        return ezsim.List(geom for entity in self._entities for geom in entity.geoms)

    @property
    def vgeoms(self):
        if self.is_built:
            return self._vgeoms
        return ezsim.List(vgeom for entity in self._entities for vgeom in entity.vgeoms)
    ...
    def init_qpos(self):
        if len(self._entities) == 0:
            return np.array([])
        return np.concatenate([entity.init_qpos for entity in self._entities], dtype=ezsim.np_float)
    ...
    def equalities(self):
        ...
        return ezsim.List(equality for entity in self._entities for equality in entity.equalities)
...
def kernel_init_meaninertia(
    ...
    ti.loop_config(serialize=static_rigid_sim_config.para_level < ezsim.PARA_LEVEL.PARTIAL)
    for i_b in range(_B):
        if n_dofs > 0:
...
def kernel_init_dof_fields(
    ...
    for i, b in ti.ndrange(n_dofs, _B):
        dofs_state.ctrl_mode[i, b] = ezsim.CTRL_MODE.FORCE
...
def kernel_init_geom_fields(
    ...
    for i in range(n_geoms):
        ...
        geoms_init_AABB[i, 0] = ti.Vector([lower[0], lower[1], lower[2]], dt=ezsim.ti_float)
        geoms_init_AABB[i, 1] = ti.Vector([lower[0], lower[1], upper[2]], dt=ezsim.ti_float)
        geoms_init_AABB[i, 2] = ti.Vector([lower[0], upper[1], lower[2]], dt=ezsim.ti_float)
        geoms_init_AABB[i, 3] = ti.Vector([lower[0], upper[1], upper[2]], dt=ezsim.ti_float)
        geoms_init_AABB[i, 4] = ti.Vector([upper[0], lower[1], lower[2]], dt=ezsim.ti_float)
        geoms_init_AABB[i, 5] = ti.Vector([upper[0], lower[1], upper[2]], dt=ezsim.ti_float)
        geoms_init_AABB[i, 6] = ti.Vector([upper[0], upper[1], lower[2]], dt=ezsim.ti_float)
        geoms_init_AABB[i, 7] = ti.Vector([upper[0], upper[1], upper[2]], dt=ezsim.ti_float)
            ...
            for i_e_ in range(rigid_global_info.n_awake_entities[i_b]):
                ...
                if ti.static(implicit_damping):
                    for i_d in range(entities_info.dof_start[i_e], entities_info.dof_end[i_e]):
                        ...
                        if (dofs_state.ctrl_mode[i_d, i_b] == ezsim.CTRL_MODE.POSITION) or (
                            dofs_state.ctrl_mode[i_d, i_b] == ezsim.CTRL_MODE.VELOCITY
                        ):
                ...
                if rigid_global_info._mass_mat_mask[i_e, i_b] == 1:
                    ...
                    for i_d in range(entity_dof_start, entity_dof_end):
                        ...
                        if ti.static(implicit_damping):
                            ...
                            if ti.static(static_rigid_sim_config.integrator == ezsim.integrator.implicitfast):
                                if (dofs_state.ctrl_mode[i_d, i_b] == ezsim.CTRL_MODE.POSITION) or (
                                    dofs_state.ctrl_mode[i_d, i_b] == ezsim.CTRL_MODE.VELOCITY
                                ):
                                    rigid_global_info.mass_mat_L[i_d, i_d, i_b] += (
...
def kernel_rigid_entity_inverse_kinematics(
    ...
    pos_mask = ti.Vector([pos_mask_[0], pos_mask_[1], pos_mask_[2]], dt=ezsim.ti_float)
    rot_mask = ti.Vector([rot_mask_[0], rot_mask_[1], rot_mask_[2]], dt=ezsim.ti_float)
            ...
            for _ in range(max_solver_iters):
                ...
                for i_ee in range(n_links):
                    ...
                    rigid_entity._func_get_jacobian(
                        i_l_ee, i_b, ti.Vector.zero(ezsim.ti_float, 3), pos_mask, rot_mask
                ...
                if respect_joint_limit and i_sample < max_samples - 1:
                    for _i_l in range(n_links_by_dofs):
                        ...
                        for i_j in range(links_info.joint_start[I_l], links_info.joint_end[I_l]):
                            ...
                            if joints_info.type[I_j] == ezsim.JOINT_TYPE.FREE:
                                pass

                            elif (
                                joints_info.type[I_j] == ezsim.JOINT_TYPE.REVOLUTE
                                or joints_info.type[I_j] == ezsim.JOINT_TYPE.PRISMATIC
                            ):
...
def func_forward_dynamics(
    ...
    func_compute_mass_matrix(
        implicit_damping=ti.static(static_rigid_sim_config.integrator == ezsim.integrator.approximate_implicitfast),
        links_state=links_state,
        links_info=links_info,
        dofs_state=dofs_state,
        dofs_info=dofs_info,
...
def func_implicit_damping(
    ...
    if ti.static(
        ...
        or static_rigid_sim_config.integrator == ezsim.integrator.Euler
    ):
        for i_e, i_b in ti.ndrange(n_entities, _B):
        ...
        for i_e, i_b in ti.ndrange(n_entities, _B):
            ...
            for i_d in range(entity_dof_start, entity_dof_end):
                I_d = [i_d, i_b] if ti.static(static_rigid_sim_config.batch_dofs_info) else i_d
                if dofs_info.damping[I_d] > ezsim.EPS:
                    rigid_global_info._mass_mat_mask[i_e, i_b] = 1
                if ti.static(static_rigid_sim_config.integrator != ezsim.integrator.Euler):
                    if (
                        (dofs_state.ctrl_mode[i_d, i_b] == ezsim.CTRL_MODE.POSITION)
                        or (dofs_state.ctrl_mode[i_d, i_b] == ezsim.CTRL_MODE.VELOCITY)
                    ) and dofs_info.kv[I_d] > ezsim.EPS:
                        rigid_global_info._mass_mat_mask[i_e, i_b] = 1
...
def kernel_step_2(
    ...
    if ti.static(static_rigid_sim_config.integrator != ezsim.integrator.approximate_implicitfast):
...
def func_COM_links(
    ...
    if ti.static(static_rigid_sim_config.use_hibernation):
        ...
        for i_l_ in range(rigid_global_info.n_awake_links[i_b]):
            ...
            p_pos = ti.Vector.zero(ezsim.ti_float, 3)
            p_quat = gu.ti_identity_quat()
            if i_p != -1:
                p_pos = links_state.pos[i_p, i_b]
                p_quat = links_state.quat[i_p, i_b]

            if joint_type == ezsim.JOINT_TYPE.FREE or (links_info.is_fixed[I_l] and i_p == -1):
        ...
        for i_l_ in range(rigid_global_info.n_awake_links[i_b]):
            ...
            elif joint_type == ezsim.JOINT_TYPE.FIXED:
                pass
            else:
                for i_d in range(links_info.dof_start[I_l], links_info.dof_end[I_l]):
                    I_d = [i_d, i_b] if ti.static(static_rigid_sim_config.batch_dofs_info) else i_d
        ...
        for i_l in range(n_links):
            ...
            for i_j in range(links_info.joint_start[I_l], links_info.joint_end[I_l]):
                ...
                if joint_type == ezsim.JOINT_TYPE.REVOLUTE:
                ...
                elif joint_type == ezsim.JOINT_TYPE.PRISMATIC:
                    dofs_state.cdof_ang[dof_start, i_b] = ti.Vector.zero(ezsim.ti_float, 3)
                ...
                elif joint_type == ezsim.JOINT_TYPE.FREE:
                    for i in ti.static(range(3)):
                        dofs_state.cdof_ang[i + dof_start, i_b] = ti.Vector.zero(ezsim.ti_float, 3)
                        dofs_state.cdof_vel[i + dof_start, i_b] = ti.Vector.zero(ezsim.ti_float, 3)
    ...
    for i_l in range(entities_info.link_start[i_e], entities_info.link_end[i_e]):
        ...
        for i_j in range(links_info.joint_start[I_l], links_info.joint_end[I_l]):
            ...
            if joint_type == ezsim.JOINT_TYPE.FREE:
                ...
                axis = ti.Vector([0.0, 0.0, 1.0], dt=ezsim.ti_float)
                if joint_type == ezsim.JOINT_TYPE.REVOLUTE:
            ...
            if joint_type == ezsim.JOINT_TYPE.FREE:
            ...
            elif joint_type == ezsim.JOINT_TYPE.REVOLUTE:
                axis = dofs_info.motion_ang[I_d]
                ...
                quat = gu.ti_transform_quat_by_quat(qloc, quat)
                pos = joints_state.xanchor[i_j, i_b] - gu.ti_transform_by_quat(joints_info.pos[I_j], quat)
            else:  # joint_type == ezsim.JOINT_TYPE.PRISMATIC:
                dofs_state.pos[dof_start, i_b] = (
                    rigid_global_info.qpos[q_start, i_b] - rigid_global_info.qpos0[q_start, i_b]
                )
...
def func_forward_velocity_entity(
    ...
    for i_l in range(entities_info.link_start[i_e], entities_info.link_end[i_e]):
        ...
        cvel_vel = ti.Vector.zero(ezsim.ti_float, 3)
        cvel_ang = ti.Vector.zero(ezsim.ti_float, 3)
        if links_info.parent_idx[I_l] != -1:
        ...
        for i_j in range(links_info.joint_start[I_l], links_info.joint_end[I_l]):
            ...
            if joint_type == ezsim.JOINT_TYPE.FREE:
                ...
                for i_3 in ti.static(range(3)):
                    ...
                    ) = ti.Vector.zero(ezsim.ti_float, 3), ti.Vector.zero(ezsim.ti_float, 3)
...
def kernel_apply_links_external_force(
    ...
    for i_l_, i_b_ in ti.ndrange(links_idx.shape[0], envs_idx.shape[0]):
        force_i = ti.Vector([force[i_b_, i_l_, 0], force[i_b_, i_l_, 1], force[i_b_, i_l_, 2]], dt=ezsim.ti_float)
...
def kernel_apply_links_external_torque(
    ...
    ti.loop_config(serialize=static_rigid_sim_config.para_level < ezsim.PARA_LEVEL.PARTIAL)
    for i_l_, i_b_ in ti.ndrange(links_idx.shape[0], envs_idx.shape[0]):
        torque_i = ti.Vector([torque[i_b_, i_l_, 0], torque[i_b_, i_l_, 1], torque[i_b_, i_l_, 2]], dt=ezsim.ti_float)
...
def func_apply_link_external_force(
    ...
    torque = ti.Vector.zero(ezsim.ti_float, 3)
...
def func_clear_external_force(
    ...
    if ti.static(static_rigid_sim_config.use_hibernation):
        ...
        for i_b in range(_B):
            for i_l_ in range(rigid_global_info.n_awake_links[i_b]):
                i_l = rigid_global_info.awake_links[i_l_, i_b]
                links_state.cfrc_applied_ang[i_l, i_b] = ti.Vector.zero(ezsim.ti_float, 3)
                links_state.cfrc_applied_vel[i_l, i_b] = ti.Vector.zero(ezsim.ti_float, 3)
    else:
        ti.loop_config(serialize=static_rigid_sim_config.para_level < ezsim.PARA_LEVEL.PARTIAL)
        for i_l, i_b in ti.ndrange(n_links, _B):
            links_state.cfrc_applied_ang[i_l, i_b] = ti.Vector.zero(ezsim.ti_float, 3)
...
def func_torque_and_passive_force(
    ...
    for i_e, i_b in ti.ndrange(n_entities, _B):
        ...
        for i_l in range(entities_info.link_start[i_e], entities_info.link_end[i_e]):
            ...
            for i_d in range(links_info.dof_start[I_l], links_info.dof_end[I_l]):
                I_d = [i_d, i_b] if ti.static(static_rigid_sim_config.batch_dofs_info) else i_d
                force = ezsim.ti_float(0.0)
                if dofs_state.ctrl_mode[i_d, i_b] == ezsim.CTRL_MODE.FORCE:
                    force = dofs_state.ctrl_force[i_d, i_b]
                elif dofs_state.ctrl_mode[i_d, i_b] == ezsim.CTRL_MODE.VELOCITY:
                    force = dofs_info.kv[I_d] * (dofs_state.ctrl_vel[i_d, i_b] - dofs_state.vel[i_d, i_b])
                elif dofs_state.ctrl_mode[i_d, i_b] == ezsim.CTRL_MODE.POSITION and not (
                    joint_type == ezsim.JOINT_TYPE.FREE and i_d >= links_info.dof_start[I_l] + 3
                ):
                    force = (
                ...
                if ti.abs(force) > ezsim.EPS:
                    wakeup = True

            dof_start = links_info.dof_start[I_l]
            if joint_type == ezsim.JOINT_TYPE.FREE and (
                dofs_state.ctrl_mode[dof_start + 3, i_b] == ezsim.CTRL_MODE.POSITION
                or dofs_state.ctrl_mode[dof_start + 4, i_b] == ezsim.CTRL_MODE.POSITION
                or dofs_state.ctrl_mode[dof_start + 5, i_b] == ezsim.CTRL_MODE.POSITION
            ):
                xyz = ti.Vector(
                    [
    ...
    if ti.static(static_rigid_sim_config.use_hibernation):
        ...
        for i_b in range(_B):
            for i_l_ in range(rigid_global_info.n_awake_links[i_b]):
                ...
                if joint_type != ezsim.JOINT_TYPE.FREE and joint_type != ezsim.JOINT_TYPE.FIXED:
            ...
            for i_e_ in range(rigid_global_info.n_awake_entities[i_b]):
                ...
                for i_l in range(entities_info.link_start[i_e], entities_info.link_end[i_e]):
                    ...
                    if i_p == -1:
                        ...
                        links_state.cdd_ang[i_l, i_b] = ti.Vector.zero(ezsim.ti_float, 3)
                        if ti.static(update_cacc):
                            links_state.cacc_lin[i_l, i_b] = ti.Vector.zero(ezsim.ti_float, 3)
                            links_state.cacc_ang[i_l, i_b] = ti.Vector.zero(ezsim.ti_float, 3)
                    else:
    ...
    if ti.static(self._use_hibernation):
        ...
        for i_l, i_b in ti.ndrange(self.n_links, self._B):
            ...
            for i_j in range(self.links_info.joint_start[I_l], self.links_info.joint_end[I_l]):
                ...
                if joint_type == ezsim.JOINT_TYPE.REVOLUTE or joint_type == ezsim.JOINT_TYPE.PRISMATIC:
                    gear = -1  # TODO
    ...
    if ti.static(static_rigid_sim_config.use_hibernation):
        ...
        for i_l, i_b in ti.ndrange(n_links, _B):
            ...
            if joint_type == ezsim.JOINT_TYPE.FREE:
                ...
                for j in ti.static(range(3)):
                    rigid_global_info.qpos[q_start + j, i_b] = pos[j]
            if joint_type == ezsim.JOINT_TYPE.SPHERICAL or joint_type == ezsim.JOINT_TYPE.FREE:
                rot_offset = 3 if joint_type == ezsim.JOINT_TYPE.FREE else 0
                rot = ti.Vector(
                    [
...
def kernel_control_dofs_force(
    ...
    ti.loop_config(serialize=static_rigid_sim_config.para_level < ezsim.PARA_LEVEL.PARTIAL)
    for i_d_, i_b_ in ti.ndrange(dofs_idx.shape[0], envs_idx.shape[0]):
        dofs_state.ctrl_mode[dofs_idx[i_d_], envs_idx[i_b_]] = ezsim.CTRL_MODE.FORCE
...
def kernel_control_dofs_velocity(
    ...
    ti.loop_config(serialize=static_rigid_sim_config.para_level < ezsim.PARA_LEVEL.PARTIAL)
    for i_d_, i_b_ in ti.ndrange(dofs_idx.shape[0], envs_idx.shape[0]):
        dofs_state.ctrl_mode[dofs_idx[i_d_], envs_idx[i_b_]] = ezsim.CTRL_MODE.VELOCITY
...
def kernel_control_dofs_position(
    ...
    ti.loop_config(serialize=static_rigid_sim_config.para_level < ezsim.PARA_LEVEL.PARTIAL)
    for i_d_, i_b_ in ti.ndrange(dofs_idx.shape[0], envs_idx.shape[0]):
        dofs_state.ctrl_mode[dofs_idx[i_d_], envs_idx[i_b_]] = ezsim.CTRL_MODE.POSITION
...
def kernel_get_links_acc(
    ...
    for i_l_, i_b_ in ti.ndrange(links_idx.shape[0], envs_idx.shape[0]):
        ...
        cpos = links_state.pos[i_l, i_b] - links_state.COM[i_l, i_b]   # 𝐫_{P/C}
        acc_ang = links_state.cacc_ang[i_l, i_b]                       # 𝛚̇
        acc_lin = links_state.cacc_lin[i_l, i_b] + acc_ang.cross(cpos) # 𝐚_C + 𝛚̇ × 𝐫_{P/C}
...
def kernel_get_dofs_control_force(
    ...
    for i_d_, i_b_ in ti.ndrange(dofs_idx.shape[0], envs_idx.shape[0]):
        ...
        if dofs_state.ctrl_mode[i_d, i_b] == ezsim.CTRL_MODE.FORCE:
            ...
            force = dofs_info.kv[I_d] * (dofs_state.ctrl_vel[i_d, i_b] - dofs_state.vel[i_d, i_b])
        elif dofs_state.ctrl_mode[i_d, i_b] == ezsim.CTRL_MODE.POSITION:
            force = (
                dofs_info.kp[I_d] * (dofs_state.ctrl_pos[i_d, i_b] - dofs_state.pos[i_d, i_b])
                - dofs_info.kv[I_d] * dofs_state.vel[i_d, i_b]
            )
...
def kernel_set_drone_rpm(
    ...
    for i_b in range(_B):
        for i_prop in range(n_propellers):
            ...
            force = ti.Vector([0.0, 0.0, propellers_rpm[i_prop, i_b] ** 2 * KF], dt=ezsim.ti_float)
            torque = ti.Vector(
                [0.0, 0.0, propellers_rpm[i_prop, i_b] ** 2 * KM * propellers_spin[i_prop]], dt=ezsim.ti_float
            )
            if invert:
                torque = -torque
...
def kernel_update_drone_propeller_vgeoms(
    ...
    for i, b in ti.ndrange(n_propellers, _B):
        ...
        vgeoms_state.quat[propellers_vgeom_idxs[i], b] = gu.ti_transform_quat_by_quat(
            gu.ti_rotvec_to_quat(ti.Vector([0.0, 0.0, rad], dt=ezsim.ti_float)),
...
#             equalities_info.eq_type[i_e, i_b] = ezsim.ti_int(ezsim.EQUALITY_TYPE.WELD)
...
#             if (
#                 equalities_info.eq_type[i_e, i_b] == ezsim.EQUALITY_TYPE.WELD
...
#         n = constraint_state.ti_n_equalities[e]
#         for i in range(n):
#             if equalities_info.eq_type[i, e] == ezsim.EQUALITY_TYPE.WELD and out < max_eq:
#                 idx = base + out
#                 buf[idx, 0] = e
