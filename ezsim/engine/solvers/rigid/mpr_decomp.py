...
class MPR:
    ...
    def __init__(self, rigid_solver):
        ...
        self._mpr_static_config = MPR.MPRStaticConfig(
            ...
            CCD_EPS=1e-9 if ezsim.ti_float == ti.f32 else 1e-10,
            CCD_TOLERANCE=1e-6,
            CCD_ITERATIONS=50,
        )
        self.init_state()
...
def mpr_point_segment_dist2(mpr_static_config: ti.template(), P, A, B):
    ...
    if t < mpr_static_config.CCD_EPS:
        t = ezsim.ti_float(0.0)
    elif t > 1.0 - mpr_static_config.CCD_EPS:
        t = ezsim.ti_float(1.0)
    Q = A + AB * t
...
def mpr_point_tri_depth(mpr_static_config: ti.template(), P, x0, B, C):
    ...
    d = w * v - r * r
    dist = s = t = ezsim.ti_float(0.0)
    pdir = ezsim.ti_vec3([0.0, 0.0, 0.0])
...
def mpr_find_pos(
    ...
    b = ti.Vector([0.0, 0.0, 0.0, 0.0], dt=ezsim.ti_float)
    ...
    p1 = ezsim.ti_vec3([0.0, 0.0, 0.0])
    p2 = ezsim.ti_vec3([0.0, 0.0, 0.0])
    for i in range(4):
