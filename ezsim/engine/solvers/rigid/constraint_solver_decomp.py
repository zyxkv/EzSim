...
class ConstraintSolver:
    def __init__(self, rigid_solver: "RigidSolver"):
        ...
        # self.ti_n_equalities = ti.field(ezsim.ti_int, shape=self._solver._batch_shape())
        # self.ti_n_equalities.from_numpy(np.full((self._solver._B,), self._solver.n_equalities, dtype=ezsim.np_int))
        ...
        # self.n_constraints_equality = ti.field(ezsim.ti_int, shape=self._solver._batch_shape())
    ...
    def get_equality_constraints(self, as_tensor: bool = True, to_torch: bool = True):
        ...
        if to_torch:
            iout = torch.full((out_size, 3), -1, dtype=ezsim.tc_int, device=ezsim.device)
            fout = torch.zeros((out_size, 6), dtype=ezsim.tc_float, device=ezsim.device)
        else:
            iout = np.full((out_size, 3), -1, dtype=ezsim.np_int)
            fout = np.zeros((out_size, 6), dtype=ezsim.np_float)
    ...
    def get_weld_constraints(self, as_tensor: bool = True, to_torch: bool = True):
        ...
        weld_const_info = {}
        if as_tensor:
            weld_mask = eq_type == ezsim.EQUALITY_TYPE.WELD
            ...
            weld_mask_chunks = tuple(eq_type_i == ezsim.EQUALITY_TYPE.WELD for eq_type_i in eq_type)
            for key, value in eq_const_info.items():
            ...
            for i in range(4):
                ...
                if ti.static(static_rigid_sim_config.sparse_solve):
                    for i_d_ in range(constraint_state.jac_n_relevant_dofs[n_con, i_b]):
                        ...
                        constraint_state.jac[n_con, i_d, i_b] = ezsim.ti_float(0.0)
                else:
                    for i_d in range(n_dofs):
                        constraint_state.jac[n_con, i_d, i_b] = ezsim.ti_float(0.0)
...
def func_equality_connect(
    ...
    for i_3 in range(3):
        ...
        diag = ti.max(invweight * (1.0 - imp) / imp, ezsim.EPS)
...
def func_equality_joint(
    ...
    constraint_state.jac[n_con, i_dof1, i_b] = ezsim.ti_float(1.0)
...
def add_equality_constraints(
    ...
    for i_b in range(_B):
        for i_e in range(constraint_state.ti_n_equalities[i_b]):
            if equalities_info.eq_type[i_e, i_b] == ezsim.EQUALITY_TYPE.CONNECT:
            ...
            elif equalities_info.eq_type[i_e, i_b] == ezsim.EQUALITY_TYPE.WELD:
                func_equality_weld(
                    i_b,
                    i_e,
                    links_info=links_info,
            ...
            elif equalities_info.eq_type[i_e, i_b] == ezsim.EQUALITY_TYPE.JOINT:
                func_equality_joint(
                    i_b,
                    i_e,
...
def func_equality_weld(
    ...
    rot_error = ezsim.ti_vec3([error_quat[1], error_quat[2], error_quat[3]]) * torquescale

    all_error = ezsim.ti_vec6([pos_error[0], pos_error[1], pos_error[2], rot_error[0], rot_error[1], rot_error[2]])
    ...
    for i in range(3):
        ...
        jac_qvel = ezsim.ti_float(0.0)
        for i_ab in range(2):
            sign = ezsim.ti_float(1.0) if i_ab == 0 else ezsim.ti_float(-1.0)
        ...
        diag = ti.max(invweight[0] * (1 - imp) / imp, ezsim.EPS)
    ...
    for i_con in range(n_con, n_con + 3):
        for i_d in range(n_dofs):
            constraint_state.jac[i_con, i_d, i_b] = ezsim.ti_float(0.0)
    ...
    for i_con in range(n_con, n_con + 3):
        ...
        diag = ti.max(invweight[1] * (1.0 - imp) / imp, ezsim.EPS)
    ...
    for i_b in range(_B):
        for i_l in range(n_links):
            ...
            for i_j in range(links_info.joint_start[I_l], links_info.joint_end[I_l]):
                ...
                if joints_info.type[I_j] == ezsim.JOINT_TYPE.REVOLUTE or joints_info.type[I_j] == ezsim.JOINT_TYPE.PRISMATIC:
                    ...
                    if pos_delta < 0:
                        ...
                        diag = ti.max(dofs_info.invweight[I_d] * (1 - imp) / imp, ezsim.EPS)
                        ...
                        if ti.static(static_rigid_sim_config.sparse_solve):
                            for i_d2_ in range(constraint_state.jac_n_relevant_dofs[n_con, i_b]):
                                ...
                                constraint_state.jac[n_con, i_d2, i_b] = ezsim.ti_float(0.0)
                        else:
        ...
        for i_l in range(n_links):
            ...
            for i_j in range(links_info.joint_start[I_l], links_info.joint_end[I_l]):
                ...
                for i_d in range(joints_info.dof_start[I_j], joints_info.dof_end[I_j]):
                    ...
                    if dofs_info.frictionloss[I_d] > ezsim.EPS:
                        jac = 1.0
                        jac_qvel = jac * dofs_state.vel[i_d, i_b]
                        imp, aref = gu.imp_aref(joints_info.sol_params[I_j], 0.0, jac_qvel, 0.0)
                        diag = ti.max(dofs_info.invweight[I_d] * (1.0 - imp) / imp, ezsim.EPS)
                        ...
                        for i_d2 in range(n_dofs):
                            constraint_state.jac[i_con, i_d2, i_b] = ezsim.ti_float(0.0)
            ...
            if ti.static(static_rigid_sim_config.sparse_solve):
                ...
                if flag_update != -1:
                    ...
                    for k in range(n_dofs):
                        if ti.abs(constraint_state.nt_vec[k, i_b]) > ezsim.EPS:
                            Lkk = constraint_state.nt_H[k, k, i_b]
                            tmp = Lkk * Lkk + constraint_state.nt_vec[k, i_b] * constraint_state.nt_vec[k, i_b] * (
                                flag_update * 2 - 1
                            )
...
def func_nt_hessian_direct(
    ...
    for i_d1 in range(n_dofs):
        for i_d2 in range(n_dofs):
            constraint_state.nt_H[i_d1, i_d2, i_b] = ezsim.ti_float(0.0)
    ...
    if ti.static(static_rigid_sim_config.sparse_solve):
        for i_c in range(constraint_state.n_constraints[i_b]):
            ...
            for i_d1_ in range(jac_n_relevant_dofs):
                ...
                if ti.abs(constraint_state.jac[i_c, i_d1, i_b]) > ezsim.EPS:
...
def func_update_contact_force(
    ...
    for i_l, i_b in ti.ndrange(n_links, _B):
        links_state.contact_force[i_l, i_b] = ti.Vector.zero(ezsim.ti_float, 3)
...
def func_ls_point_fn(
    ...
    tmp_quad_total_0, tmp_quad_total_1, tmp_quad_total_2 = ezsim.ti_float(0.0), ezsim.ti_float(0.0), ezsim.ti_float(0.0)
    ...
    for i_c in range(constraint_state.n_constraints[i_b]):
        ...
        active = ezsim.ti_bool(True)  # Equality constraints
        if ne <= i_c and i_c < nef:  # Friction constraints
            f = constraint_state.efc_frictionloss[i_c, i_b]
            r = 1.0 / ti.max(constraint_state.efc_D[i_c, i_b], ezsim.EPS)
            rf = r * f
            linear_neg = x <= -rf
            linear_pos = x >= rf
    ...
    deriv_1 = 2 * tmp_quad_total_2 + ezsim.EPS * (ti.abs(tmp_quad_total_2) < ezsim.EPS)
    if deriv_1 <= 0.0:
        deriv_1 = ezsim.EPS
...
def func_linesearch(
    ...
    if snorm < ezsim.EPS:
        ...
        p0_alpha, p0_cost, p0_deriv_0, p0_deriv_1 = func_ls_point_fn(i_b, ezsim.ti_float(0.0), constraint_state)
...
def func_solve_body(
    ...
    if ti.abs(alpha) < ezsim.EPS:
        ...
        if ti.static(static_rigid_sim_config.solver_type == ezsim.constraint_solver.CG):
            for i_d in range(n_dofs):
        ...
        if ti.static(static_rigid_sim_config.solver_type == ezsim.constraint_solver.Newton):
        ...
        if gradient < tol_scaled or improvement < tol_scaled:
            ...
            if ti.static(static_rigid_sim_config.solver_type == ezsim.constraint_solver.Newton):
                ...
                constraint_state.cg_beta[i_b] = ezsim.ti_float(0.0)
                constraint_state.cg_pg_dot_pMg[i_b] = ezsim.ti_float(0.0)
                for i_d in range(n_dofs):
                ...
                constraint_state.cg_beta[i_b] = ti.max(
                    0.0, constraint_state.cg_beta[i_b] / ti.max(ezsim.EPS, constraint_state.cg_pg_dot_pMg[i_b])
                )
                for i_d in range(n_dofs):
        ...
        # elif ti.static(static_rigid_sim_config.solver_type == ezsim.constraint_solver.Newton):
        #     improvement = constraint_state.prev_cost[i_b] - constraint_state.cost[i_b]
        #     if improvement > 0:
        #         func_nt_hessian_incremental(
        #             i_b,
...
def func_update_constraint(
    ...
    constraint_state.gauss[i_b] = ezsim.ti_float(0.0)

    for i_c in range(constraint_state.n_constraints[i_b]):
        if ti.static(static_rigid_sim_config.solver_type == ezsim.constraint_solver.Newton):
            constraint_state.prev_active[i_c, i_b] = constraint_state.active[i_c, i_b]
        constraint_state.active[i_c, i_b] = True
        floss_force = ezsim.ti_float(0.0)
    ...
    if ti.static(static_rigid_sim_config.sparse_solve):
        for i_d in range(n_dofs):
            constraint_state.qfrc_constraint[i_d, i_b] = ezsim.ti_float(0.0)
        for i_c in range(constraint_state.n_constraints[i_b]):
            for i_d_ in range(constraint_state.jac_n_relevant_dofs[i_c, i_b]):
                i_d = constraint_state.jac_relevant_dofs[i_c, i_d_, i_b]
...
def kernel_get_equality_constraints(
    ...
    for i_b in range(_B):
        ...
        for i_e_ in range(constraint_state.ti_n_equalities[i_b]):
            ...
            if equalities_info.eq_type[i_e_, i_b] == ezsim.EQUALITY_TYPE.CONNECT:
                for i_c_ in ti.static(range(3)):
                    i_c = i_c_start + i_c_
                    fout[i_e, i_c_] = constraint_state.efc_force[i_c, i_b]
                i_c_start = i_c_start + 3
            elif equalities_info.eq_type[i_e_, i_b] == ezsim.EQUALITY_TYPE.WELD:
                for i_c_ in ti.static(range(6)):
                    i_c = i_c_start + i_c_
                    fout[i_e, i_c_] = constraint_state.efc_force[i_c, i_b]
                i_c_start = i_c_start + 6
            elif equalities_info.eq_type[i_e_, i_b] == ezsim.EQUALITY_TYPE.JOINT:
