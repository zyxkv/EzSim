...
class SupportField:
    ...
    def _compute_support(self) -> None:
        ...
        if self.solver.n_geoms > 0:
            ...
            support_vid = np.concatenate(support_vid, dtype=ezsim.np_int)
            support_cell_start = np.array(support_cell_start, dtype=ezsim.np_int)
        else:
            support_v = np.zeros([1, 3], dtype=ezsim.np_float)
            support_vid = np.zeros([1], dtype=ezsim.np_int)
            support_cell_start = np.zeros([1], dtype=ezsim.np_int)
    ...
    def _func_support_mesh(
        ...
        support_res = ezsim.ti_int(support_field_static_config.support_res)
        dot_max = ezsim.ti_float(-1e20)
        ...
        for i4 in range(4):
            i, j = ezsim.ti_int(0), ezsim.ti_int(0)
            if i4 % 2:
                i = ezsim.ti_int(ti.math.ceil(ii) % support_res)
            else:
                i = ezsim.ti_int(ti.math.floor(ii) % support_res)

            if i4 // 2 > 0:
                j = ezsim.ti_int(ti.math.clamp(ti.math.ceil(jj), 0, support_res - 1))
                if j == support_res - 1:
                    j = support_res - 2
            else:
                j = ezsim.ti_int(ti.math.clamp(ti.math.floor(jj), 0, support_res - 1))
                if j == 0:
                    j = 1

            support_idx = ezsim.ti_int(support_field_info.support_cell_start[i_g] + i * support_res + j)
    ...
    def _func_support_capsule(
        ...
        if shrink:
            ...
            capsule_axis = gu.ti_transform_by_quat(ti.Vector([0.0, 0.0, 1.0], dt=ezsim.ti_float), g_quat)
    ...
    def _func_count_supports_box(
        ...
        return 2 ** (d_box == 0.0).cast(ezsim.ti_int).sum()
