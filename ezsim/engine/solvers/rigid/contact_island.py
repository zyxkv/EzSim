...
if TYPE_CHECKING:
    from ezsim.engine.solvers.rigid.collider_decomp import Collider
...
class ContactIsland:
    def __init__(self, collider: "Collider"):
        ...
        #     dtype=ezsim.ti_int, shape=self.solver._batch_shape((self.collider._collider_info._max_contact_pairs[None], 2))
        # )
        ...
        # )
        self.ci_edges = ti.field(dtype=ezsim.ti_int, shape=self.solver._batch_shape((max_contact_pairs, 2)))

        # self.constraint_list = ti.field(
        #     dtype=ezsim.ti_int, shape=self.solver._batch_shape((self.collider._collider_info._max_contact_pairs[None]))
        # )
        # maps half-edges (half-edges are referenced by entity_edge range) to actual edge index
        # description: half_edge_ref_to_edge_idx
        self.edge_id = ti.field(dtype=ezsim.ti_int, shape=self.solver._batch_shape((max_contact_pairs * 2)))
        ...
        self.constraint_list = ti.field(dtype=ezsim.ti_int, shape=self.solver._batch_shape((max_contact_pairs)))

        # analogous to edge_id: maps island's constraint local-index to world's contact index
        self.constraint_id = ti.field(dtype=ezsim.ti_int, shape=self.solver._batch_shape((max_contact_pairs * 2)))
        ...
        self.island_hibernated = ti.field(dtype=ezsim.ti_int, shape=self.solver._batch_shape((self.solver.n_entities)))
        ...
        self.entity_id = ti.field(dtype=ezsim.ti_int, shape=self.solver._batch_shape((self.solver.n_entities)))
        # num all collision edges in world
        self.n_edges = ti.field(dtype=ezsim.ti_int, shape=self.solver._B)
        self.n_islands = ti.field(dtype=ezsim.ti_int, shape=self.solver._B)
        self.n_stack = ti.field(dtype=ezsim.ti_int, shape=self.solver._B)
        # description: entity_idx_to_island_idx
        self.entity_island = ti.field(dtype=ezsim.ti_int, shape=self.solver._batch_shape(self.solver.n_entities))
        self.stack = ti.field(dtype=ezsim.ti_int, shape=self.solver._batch_shape(self.solver.n_entities))
        # Used to make islands persist through hibernation:
        self.entity_idx_to_next_entity_idx_in_hibernated_island = ti.field(
            dtype=ezsim.ti_int, shape=self.solver._batch_shape(self.solver.n_entities)
        )
        self.entity_idx_to_next_entity_idx_in_hibernated_island.fill(INVALID_NEXT_HIBERNATED_ENTITY_IDX)
    
    @ti.kernel
    def clear(self):
        ti.loop_config(serialize=self.solver._para_level < ezsim.PARA_LEVEL.ALL)
