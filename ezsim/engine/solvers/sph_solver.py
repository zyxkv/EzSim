...
from ezsim.engine.entities import SPHEntity
from ezsim.engine.states.solvers import SPHSolverState
...
class SPHSolver(Solver):
    ...
    def init_particle_fields(self):
        ...
        struct_particle_state = ti.types.struct(
            ...
            acc=ezsim.ti_vec3,  # acceleration
            rho=ezsim.ti_float,  # density
            p=ezsim.ti_float,  # pressure
            dfsph_factor=ezsim.ti_float,  # DFSPH use: Factor for Divergence and density solver
            drho=ezsim.ti_float,  # density deritivate
        )
        ...
        struct_particle_info = ti.types.struct(
            rho=ezsim.ti_float,  # rest density
            mass=ezsim.ti_float,  # mass
            stiffness=ezsim.ti_float,
            exponent=ezsim.ti_float,
            mu=ezsim.ti_float,  # viscosity
            gamma=ezsim.ti_float,  # surface tension
        )
    ...
    def _kernel_compute_DFSPH_factor(self, f: ti.i32):
        for i_p, i_b in ti.ndrange(self._n_particles, self._B):
            if self.particles_ng_reordered[i_p, i_b].active:
                sum_grad_p_k = 0.0
                grad_p_i = ti.Vector.zero(ezsim.ti_float, 3)

                # `ret` concatenates `grad_p_i` and `sum_grad_p_k`
                ret = ti.Vector.zero(ezsim.ti_float, 4)
    ...
    def _kernel_divergence_solver_iteration(self):
        ...
        for i_p, i_b in ti.ndrange(self._n_particles, self._B):
            if self.particles_ng_reordered[i_p, i_b].active:
                ...
                ret = ti.Struct(dv=ti.Vector.zero(ezsim.ti_float, 3), k_i=k_i)
                # TODO: if warm start
                # get_kappa_V += k_i
                self.sh.for_all_neighbors(
    ...
    def _divergence_solve(self, f: ti.i32):
        ...
        ezsim.logger.debug(f"DFSPH - iteration V: {iteration} Avg divergence err: {avg_density_err / self._density0:.4f}")
    ...
    def _density_solve(self, f: ti.i32):
        ...
        ezsim.logger.debug(f"DFSPH - iterations: {iteration} Avg density err: {avg_density_err:.4f} kg/m^3")
    ...
    def cubic_kernel(self, r_norm):
        res = ti.cast(0.0, ezsim.ti_float)
    ...
    def cubic_kernel_derivative(self, r):
        ...
        r_norm = r.norm()
        q = r_norm / h
        res = ti.Vector.zero(ezsim.ti_float, 3)
    ...
    def _kernel_add_particles(
        ...
        for i_p, i_b in ti.ndrange(n_particles, self._B):
            ...
            self.particles_ng[i_global, i_b].active = ti.cast(active, ezsim.ti_bool)
            for j in ti.static(range(3)):
                self.particles[i_global, i_b].pos[j] = pos[i_p, j]
            self.particles[i_global, i_b].vel = ti.Vector.zero(ezsim.ti_float, 3)
    ...
    def _kernel_set_particles_vel(
        ...
        for i_p, i_b in ti.ndrange(n_particles, self._B):
            ...
            self.particles[i_global, i_b].acc = ti.Vector.zero(ezsim.ti_float, 3)
