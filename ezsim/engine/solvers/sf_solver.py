...
from ezsim.engine.entities import SFParticleEntity
...
class SFSolver(Solver):
    ...
    def setup_fields(self):
        cell_state = ti.types.struct(
            ...
            div=ezsim.ti_float,
            p=ezsim.ti_float,
            q=ti.types.vector(len(self.jets), ezsim.ti_float),
        )
        ...
        self.p_swap = TexPair(
            cur=ti.field(dtype=ezsim.ti_float, shape=self.res),
            nxt=ti.field(dtype=ezsim.ti_float, shape=self.res),
        )
    ...
    def advect_and_impulse(self, f: ti.i32, t: ti.f32):
        for i, j, k in ti.ndrange(*self.res):
            p = ti.Vector([i, j, k], dt=ezsim.ti_float) + 0.5
    ...
    def trilerp_scalar(self, qf, p, qf_idx):
        ...
        base_I = ti.floor(p - 0.5, ezsim.ti_int)
        p_I = p - 0.5
    ...
    def trilerp(self, qf, p):
        ...
        q = ti.Vector([0.0, 0.0, 0.0], dt=ezsim.ti_float)
        w_total = 0.0
        for offset in ti.static(ti.grouped(ti.ndrange(2, 2, 2))):
            grid_I = base_I + offset
            w_xyz = 1 - ti.abs(p_I - grid_I)
