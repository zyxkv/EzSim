...
class Mesh(RBC):
    """
    EzSim's own triangle mesh object.
    ...
    def __init__(
        ...
        self._color = np.array([1.0, 1.0, 1.0, 1.0], dtype=ezsim.np_float)  # default color is white
    ...
    def remesh(self, edge_len_abs=None, edge_len_ratio=0.01, fix=True):
        ...
        if os.path.exists(rm_file_path):
            ezsim.logger.debug("Remeshed file (`.rm`) found in cache.")
            try:
        ...
        if not is_cached_loaded:
            ezsim.logger.info("Remeshing for tetrahedralization...")
    ...
    def clear_visuals(self):
        """
        Clear the mesh's visual attributes by resetting the surface to ezsim.surfaces.Default().
        """
        self._surface = ezsim.surfaces.Default()
    ...
    def from_trimesh(
        ...
        """
        Create a ezsim.Mesh from a trimesh.Trimesh object.
        """
        if surface is None:
    ...
    def from_attrs(cls, verts, faces, normals=None, surface=None, uvs=None, scale=None):
        """
        Create a ezsim.Mesh from mesh attribtues including vertices, faces, and normals.
        """
        if surface is None:
            surface = ezsim.surfaces.Default()
    ...
    def from_morph_surface(cls, morph, surface=None):
        """
        Create a ezsim.Mesh from morph and surface options.
        ...
        if isinstance(morph, ezsim.options.morphs.Mesh):
            ...
            metadata = {"mesh_path": morph.file} if isinstance(morph, ezsim.options.morphs.FileMorph) else {}
    ...
    def set_color(self, color):
        ...
        """
        self._color = color 
        color_texture = ezsim.textures.ColorTexture(color=tuple(color))
        opacity_texture = color_texture.check_dim(3)
