...
from ezsim.options.solvers import (
    AvatarOptions,
    BaseCouplerOptions,
...
class Simulator(RBC):
    ...
    def __init__(
        ...
        self._gravity = np.array(options.gravity, dtype=ezsim.np_float)
        ...
        self._solvers: list[Solver] = ezsim.List(
            [
                self.tool_solver,
                self.rigid_solver,
                self.avatar_solver,
        ...
        self._active_solvers: list[Solver] = ezsim.List()
    ...
    def _add_entity(self, morph: Morph, material, surface, visualize_contact=False):
        ...
        elif isinstance(material, ezsim.materials.Hybrid):
            entity = HybridEntity(
                self.n_entities, self.scene, material, morph, surface
            )  # adding to solver is handled in the hybrid entity

        else:
            ezsim.raise_exception(f"Material not supported.: {material}")
    ...
    def build(self):
        ...
        if self.n_envs > 0 and self.sf_solver.is_active():
            ezsim.raise_exception("Batching is not supported for SF solver as of now.")
