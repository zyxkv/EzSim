...
class LBVH(RBC):
    ...
    def __init__(self, aabb: AABB, max_n_query_result_per_aabb: int = 8, n_radix_sort_groups: int = 256):
        ...
        self.aabb_centers = ti.field(ezsim.ti_vec3, shape=(self.n_batches, self.n_aabbs))
        self.aabb_min = ti.field(ezsim.ti_vec3, shape=(self.n_batches))
        self.aabb_max = ti.field(ezsim.ti_vec3, shape=(self.n_batches))
        self.scale = ti.field(ezsim.ti_vec3, shape=(self.n_batches))
        ...
        self.internal_node_active = ti.field(ezsim.ti_bool, shape=(self.n_batches, self.n_aabbs - 1))
        self.internal_node_ready = ti.field(ezsim.ti_bool, shape=(self.n_batches, self.n_aabbs - 1))

        # Query results, vec3 of batch id, self id, query id
        self.query_result = ti.field(ezsim.ti_ivec3, shape=(self.max_n_query_results))
    ...
    def compute_aabb_centers_and_scales(self):
        ...
        for i_b in ti.ndrange(self.n_batches):
            ...
            for i in ti.static(range(3)):
                self.scale[i_b][i] = ti.select(scale[i] > ezsim.EPS, 1.0 / scale[i], 1.0)
                ...
                if aabbs[i_b, i_q].intersects(node.bound):
                    ...
                    if node.left == -1 and node.right == -1:
                        ...
                        if idx < self.max_n_query_results:
                            self.query_result[idx] = ezsim.ti_ivec3(i_b, i_a, i_q)  # Store the AABB index
                    else:
