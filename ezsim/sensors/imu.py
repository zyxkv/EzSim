...
class IMUSharedMetadata(SharedSensorMetadata):
    ...
    offsets_pos: torch.Tensor = field(default_factory=lambda: torch.tensor([], dtype=ezsim.tc_float, device=ezsim.device))
    offsets_quat: torch.Tensor = field(default_factory=lambda: torch.tensor([], dtype=ezsim.tc_float, device=ezsim.device))
    acc_bias: torch.Tensor = field(default_factory=lambda: torch.tensor([], dtype=ezsim.tc_float, device=ezsim.device))
    ang_bias: torch.Tensor = field(default_factory=lambda: torch.tensor([], dtype=ezsim.tc_float, device=ezsim.device))
    ...
    def build(self):
        ...
        self._shared_metadata.offsets_pos = torch.cat(
            [
                ...
                torch.tensor([self._options.pos_offset], dtype=ezsim.tc_float, device=ezsim.device),
            ]
        )

        quat_tensor = torch.tensor(euler_to_quat([self._options.euler_offset]), dtype=ezsim.tc_float, device=ezsim.device)
        ...
        self._shared_metadata.acc_bias = torch.cat(
            [
                ...
                torch.tensor([self._options.accelerometer_bias], dtype=ezsim.tc_float, device=ezsim.device),
            ]
        )
        self._shared_metadata.ang_bias = torch.cat(
            [
                self._shared_metadata.ang_bias,
                torch.tensor([self._options.gyroscope_bias], dtype=ezsim.tc_float, device=ezsim.device),
            ]
        )
