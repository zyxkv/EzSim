...
import ezsim
from ezsim.utils.tools import animate
...
class VideoFileWriter(DataHandler):
    ...
    def cleanup(self):
        if self.frames:
            ...
            ezsim.logger.warning("VideoFileWriter: No frames to write to video file.")
...
class VideoFileStreamer(DataHandler):
    ...
    def _lazy_initialize(self, data):
        ...
        for i in self.streams_idx:
            ...
            if not video_writer.isOpened():
                ezsim.raise_exception(f"Failed to open video writer for {self.filenames[i]}")
    ...
    def cleanup(self):
        for i, video_writer in enumerate(self.video_writers):
            ...
            ezsim.logger.info(f'Video saved to "~<{self.filenames[i]}>~".')
...
class CSVFileWriter(DataHandler):
    ...
    def cleanup(self):
        if self.file_handle:
            ezsim.logger.info(f'Saving CSV file to ~<"{self.filename}">~...')
            self.file_handle.close()
            ezsim.logger.info("File saved.")
...
class NPZFileWriter(DataHandler):
    ...
    def cleanup(self):
        ezsim.logger.info(f'Saving data to "~<{self.filename}>~"...')
        try:
            np.savez_compressed(self.filename, np.array(self.all_data))
        except ValueError as error:
            ezsim.logger.warning(f"NPZFileWriter: saving as dtype=object due to ValueError: {error}")
            np.savez_compressed(self.filename, np.array(self.all_data, dtype=object))
        ezsim.logger.info("NPZ data saved.")
