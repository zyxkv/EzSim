...
class Visualizer(RBC):
    ...
    def __init__(self, scene, show_viewer, vis_options, viewer_options, renderer_options):
        ...
        try:
            ...
            ezsim.raise_exception_from("Rendering not working on this machine.", e)
        ...
        try:
            ...
            if show_viewer:
                ezsim.raise_exception_from("No display detected. Use `show_viewer=False` for headless mode.", e)
            # self._connected_to_display = False
            self._has_display = False

        if show_viewer:
            if ezsim.global_scene_list:
            ...
            if viewer_options.run_in_thread is None:
                ...
                elif ezsim.platform == "Windows":
                    viewer_options.run_in_thread = True
            if ezsim.platform == "macOS" and viewer_options.run_in_thread:
                ezsim.raise_exception("Running viewer in background thread is not supported on MacOS.")
        ...
        elif isinstance(renderer_options, ezsim.renderers.Rasterizer):
             self._renderer = self._rasterizer
        else:
            # self._renderer = self._rasterizer
            # self._raytracer = None
            ezsim.raise_exception("Unsupported renderer type.")

        self._cameras = ezsim.List()
    ...
    def update(self, force=True, auto=None):
        if force:  # force update
            ...
            if self._viewer.is_alive():
                ...
                ezsim.raise_exception("Viewer closed.")
