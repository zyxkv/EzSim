...
from ezsim.utils.tools import Rate
from ezsim.utils.misc import redirect_libc_stderr

if TYPE_CHECKING:
    from ezsim.options.vis import ViewerOptions
...
class Viewer(RBC):
    def __init__(self, options: "ViewerOptions", context):
        ...
        self._camera_init_pos = np.asarray(options.camera_pos, dtype=ezsim.np_float)
        self._camera_init_lookat = np.asarray(options.camera_lookat, dtype=ezsim.np_float)
        self._camera_up = np.asarray(options.camera_up, dtype=ezsim.np_float)
        self._camera_fov = options.camera_fov
        self._enable_interaction = options.enable_interaction

        if options.enable_interaction and ezsim.backend != ezsim.cpu:
            ezsim.logger.warning("Interaction code is slow on GPU. Switch to CPU backend or disable interaction.")
    ...
    def build(self, scene):
        ...
        if opengl_platform_orig is None:
            if ezsim.platform == "Windows":
                all_opengl_platforms = ("wgl",)  # same as "native"
            elif ezsim.platform == "Linux":
                # "native" is platform-specific ("egl" or "glx")
                all_opengl_platforms = ("native", "egl", "glx", "osmesa")
            else:
                all_opengl_platforms = ("native",)
        else:
            if ezsim.platform == "Windows" and opengl_platform_orig == "osmesa":
                ezsim.raise_exception("PYOPENGL_PLATFORM='osmesa' is not supported on Windows OS.")
        ...
        for i, platform in enumerate(all_opengl_platforms):
            ...
            try:
                ezsim.logger.debug(f"Trying to create OpenGL Context for PYOPENGL_PLATFORM='{platform}'...")
                ...
                ezsim.logger.debug(f"Invalid OpenGL context.")
        ...
        ezsim.logger.info(f"Viewer created. Resolution: ~<{self._res[0]}×{self._res[1]}>~, max_FPS: ~<{self._max_FPS}>~.")

    def run(self):
        if self._pyrender_viewer is None:
            ezsim.raise_exception("Viewer must be built successfully before calling this method.")
    ...
    def set_camera_pose(self, pose=None, pos=None, lookat=None):
        ...
        if pose is None:
            ...
            if np.array(pose).shape != (4, 4):
                ezsim.raise_exception("pose should be a 4x4 matrix.")
