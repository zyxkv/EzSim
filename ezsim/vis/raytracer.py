...
import ezsim.utils.misc as miscu
import ezsim.utils.particle as pu
from ezsim.engine import entities
import trimesh
...
try:
    ...
    ezsim.raise_exception(f"Failed to import LuisaRenderer. {e.__class__.__name__}: {e}")
...
class EnvironmentSphere:
    ...
    def __init__(self, radius, pos, quat, surface=None, name=None, file_path="meshes/env_sphere/env_sphere.obj"):
        ...
        self.name = str(ezsim.UID()) if name is None else name
        self.file_path = os.path.join(ezsim.utils.get_assets_dir(), file_path)
...
class SphereLight(ShapeLight):
    ...
    def add_to_render(self, renderer):
        ...
        if sphere_light_as_mesh:
            ...
            renderer.update_rigid(self.name, ezsim.trans_to_T(self.pos))
        else:
...
class Raytracer:
    ...
    def add_mesh_light(self, mesh, color, intensity, pos, quat, revert_dir=False, double_sided=False, beam_angle=180.0):
        ...
        if color.ndim != 1 or (color.shape[0] != 3 and color.shape[0] != 4):
            ezsim.raise_exception("Light color should have shape (3,) or (4,).")

        self.lights.append(
            MeshLight(
                mesh=mesh,
                transform=gu.trans_quat_to_T(np.array(pos), np.array(quat)),
                surface=ezsim.surfaces.Plastic(
    ...
    def build(self, scene):
        ...
        ezsim.exit_callbacks.append(self.destroy)
    ...
    def get_texture(self, texture):
        ...
        if isinstance(texture, ezsim.textures.ColorTexture):
            return LuisaRenderPy.ColorTexture(color=texture.color)
        elif isinstance(texture, ezsim.textures.ImageTexture):
            ...
            if image_array is not None:
                ...
                ezsim.raise_exception("Texture type error. Both 'image_path' and 'image_array' are None")
        else:
            ezsim.raise_exception(f"Texture type error: {type(texture)}")
    ...
    def add_surface(self, shape_name, surface):
        ...
        if isinstance(surface, ezsim.surfaces.Plastic):
            surface_luisa = LuisaRenderPy.PlasticSurface(
                ...
                eta=self.get_texture(ezsim.textures.ColorTexture(color=(surface.ior,))),
            )
        elif isinstance(surface, ezsim.surfaces.BSDF):
            surface_luisa = LuisaRenderPy.DisneySurface(
                ...
                specular_trans=self.get_texture(ezsim.textures.ColorTexture(color=(surface.specular_trans,))),
                diffuse_trans=(
                    None
                    if surface.diffuse_trans == 0.0
                    else self.get_texture(ezsim.textures.ColorTexture(color=(surface.diffuse_trans,)))
                ),
            )
        elif isinstance(surface, ezsim.surfaces.Metal):
        ...
        elif isinstance(surface, ezsim.surfaces.Glass):
        ...
        elif isinstance(surface, ezsim.surfaces.Emission):
            surface_luisa = None
        else:
            ezsim.raise_exception(f"Suface type error: {type(surface)}")
        ...
        if surface.generate_foam:
            ...
            self.add_surface(f"{shape_name}_foams", ezsim.surfaces.Rough(color=surface.foam_options.color))
        else:
    ...
    def update_particles(self, name, particles, radius=None, particles_vel=None, particles_radii=None):
        ...
        if self.shape_foamgens[name] is not None:
            if particles_vel is None:
                ezsim.raise_exception("Velocities not passed when generating foams.")
    ...
    def add_camera(self, camera):
        ...
        if camera_model == "pinhole":
            ...
            ezsim.raise_exception("Invalid camera model.")
