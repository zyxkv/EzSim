...
class Options(BaseModel, RBC):
    """
    This is the base class for all `ezsim.options.*` classes. An `Options` object is a group of parameters for setting a specific component in the scene.
    ...
    def __init__(self, **data):
        ...
        for key in data.keys():
            if key not in allowed_params:
                ezsim.raise_exception(f"Unrecognized attribute: {key}")
        ...
        try:
            ...
            errors = e.errors()[0]
            ezsim.raise_exception(f"Invalid '{errors['loc'][0]}': {errors['msg'].lower()}.")
    ...
    @classmethod
    def _repr_type(cls):
        return f"<{cls.__module__}.{cls.__qualname__}>".replace("ezsim", "gs")
