...
class Morph(Options):
    """
    This is the base class for all ezsim morphs.
    A morph in ezsim is a hybrid concept, encapsulating both the geometry and pose information of an entity. This includes shape primitives, meshes, URDF, MJCF, Terrain, and soft robot description files.
    ...
    def __init__(self, **data):
        ...
        if self.pos is not None:
            if not isinstance(self.pos, tuple) or len(self.pos) != 3:
                ezsim.raise_exception("`pos` should be a 3-tuple.")

        if self.euler is not None:
            if not isinstance(self.euler, tuple) or len(self.euler) != 3:
                ezsim.raise_exception("`euler` should be a 3-tuple.")

        if self.quat is not None:
            if not isinstance(self.quat, tuple) or len(self.quat) != 4:
                ezsim.raise_exception("`quat` should be a 4-tuple.")

        if (self.quat is not None) and (self.euler is not None):
            ezsim.raise_exception("`euler` and `quat` cannot be jointly specified.")

        if self.euler is not None:
            self.quat = tuple(ezsim.utils.geom.xyz_to_quat(np.array(self.euler), rpy=True, degrees=True))
        elif self.quat is None:
            self.quat = (1.0, 0.0, 0.0, 0.0)

        if not self.visualization and not self.collision:
            ezsim.raise_exception("`visualization` and `collision` cannot both be False.")

    def _repr_type(self):
        return f"<ezsim.morphs.{self.__class__.__name__}>"
...
class Nowhere(Morph):
    ...
    def __init__(self, **data):
        ...
        if self.n_particles <= 0:
            ezsim.raise_exception("`n_particles` should be greater than 0.")
    ...
    def __init__(self, **data):
        ...
        if self.lower is None or self.upper is None:
            if self.pos is None or self.size is None:
                ezsim.raise_exception("Either [`pos` and `size`] or [`lower` and `upper`] should be specified.")
            ...
            if not (np.array(self.upper) >= np.array(self.lower)).all():
                ezsim.raise_exception("Invalid lower and upper corner.")
...
class Plane(Primitive):
    ...
    normal : tuple, shape (3,), optional
        The normal of the plane in its local frame. Defaults to (0, 0, 1).
    visualization : bool, optional
        Whether the entity needs to be visualized. Set it to False if you need a invisible object only for collision purposes. Defaults to True. `visualization` and `collision` cannot both be False. **This is only used for RigidEntity.**
    collision : bool, optional
        Whether the entity needs to be considered for collision checking. Defaults to True. `visualization` and `collision` cannot both be False. **This is only used for RigidEntity.**
    ...
    def __init__(self, **data):
        ...
        if not isinstance(self.normal, tuple) or len(self.normal) != 3:
            ezsim.raise_exception("`normal` should be a 3-tuple.")

        if not self.fixed:
            ezsim.raise_exception("`fixed` must be True for `Plane`.")

        if self.requires_jac_and_IK:
            ezsim.raise_exception("`requires_jac_and_IK` must be False for `Plane`.")
...
class FileMorph(Morph):
    ...
    scale : float or tuple, optional
        The scaling factor for the size of the entity. If a float, it scales uniformly. If a 3-tuple, it scales along each axis. Defaults to 1.0. Note that 3-tuple scaling is only supported for `ezsim.morphs.Mesh`.
    pos : tuple, shape (3,), optional
        The position of the entity in meters. Defaults to (0.0, 0.0, 0.0).
    euler : tuple, shape (3,), optional
        The euler angle of the entity in degrees. This follows scipy's extrinsic x-y-z rotation convention. Defaults to (0.0, 0.0, 0.0).
    ...
    coacd_options : CoacdOptions, optional
        Options for configuring coacd convex decomposition. Needs to be a `ezsim.options.CoacdOptions` object.
    visualization : bool, optional
        Whether the entity needs to be visualized. Set it to False if you need a invisible object only for collision purposes. Defaults to True. `visualization` and `collision` cannot both be False. **This is only used for RigidEntity.**
    collision : bool, optional
        Whether the entity needs to be considered for collision checking. Defaults to True. `visualization` and `collision` cannot both be False. **This is only used for RigidEntity.**
    ...
    def __init__(self, **data):
        ...
        self.decompose_object_error_threshold = max(self.decompose_object_error_threshold, ezsim.EPS)
        self.decompose_robot_error_threshold = max(self.decompose_robot_error_threshold, ezsim.EPS)
        ...
        if isinstance(self.file, str):
            ...
            if not os.path.exists(file):
                file = os.path.join(ezsim.utils.get_assets_dir(), self.file)

            if not os.path.exists(file):
                ezsim.raise_exception(f"File not found in either current directory or assets directory: '{self.file}'.")

            self.file = file

        if isinstance(self, Mesh):
            if isinstance(self.scale, tuple) and len(self.scale) != 3:
                ezsim.raise_exception("`scale` should be a float or a 3-tuple.")
        else:
            if not isinstance(self.scale, float):
                ezsim.raise_exception("`scale` should be a float.")

    def _repr_type(self):
        return f"<ezsim.morphs.{self.__class__.__name__}(file='{self.file}')>"
...
class MJCF(FileMorph):
    ...
    Note
    ----
    EzSim currently processes MJCF as if it describing a single entity instead of an actual scene. This means that
    ...
    but must be set at the scene-level in EzSim are completely ignored at the moment, e.g. the simulation timestep,
    ...
    def __init__(self, **data):
        ...
        # if not self.file.endswith(".xml"):
            # ezsim.raise_exception(f"Expected `.xml` extension for MJCF file: {self.file}")
        if not self.is_format(MJCF_FORMAT):
            ezsim.raise_exception(f"Expected `{MJCF_FORMAT}` extension for MJCF file: {self.file}")
        ...
        if isinstance(self.scale, np.ndarray):
            if self.scale.std() > ezsim.EPS:
                ezsim.raise_exception("Anisotropic scaling is not supported by MJCF morph.")
            self.scale = self.scale.mean()


class URDF(FileMorph):
    """
    Morph loaded from a URDF file. This morph only supports `RigidEntity`. If you need to create a `Drone` entity, use `ezsim.morphs.Drone` instead.
    ...
    def __init__(self, **data):
        ...
        #     ezsim.raise_exception(f"Expected `.urdf` extension for URDF file: {self.file}")
        if isinstance(self.file, str) and not self.is_format(URDF_FORMAT):
            ezsim.raise_exception(f"Expected `{URDF_FORMAT}` extension for URDF file: {self.file}")

        # Anisotropic scaling is ill-defined for poly-articulated robots. See related MJCF about this for details.
        if isinstance(self.scale, np.ndarray) and self.scale.std() > ezsim.EPS:
            if self.scale.std() > ezsim.EPS:
                ezsim.raise_exception("Anisotropic scaling is not supported by MJCF morph.")
            self.scale = self.scale.mean()
...
class Drone(FileMorph):
    ...
    scale : float or tuple, optional
        ...
        each axis. Defaults to 1.0. Note that 3-tuple scaling is only supported for `ezsim.morphs.Mesh`.
    pos : tuple, shape (3,), optional
        The position of the entity in meters. Defaults to (0.0, 0.0, 0.0).
    euler : tuple, shape (3,), optional
    ...
    def __init__(self, **data):
        ...
        if self.COM_link_name is not None:
            ezsim.logger.warning("Drone option 'COM_link_name' is deprecated and will be ignored.")
        ...
        # if isinstance(self.file, str) and not self.file.endswith(".urdf"):
        #     ezsim.raise_exception(f"Drone only supports `.urdf` extension: {self.file}")

        if isinstance(self.file, str) and not self.is_format(URDF_FORMAT):
            ezsim.raise_exception(f"Drone only supports `{URDF_FORMAT}` extension: {self.file}")

        if self.model not in ("CF2X", "CF2P", "RACE"):
            ezsim.raise_exception(f"Unsupported `model`: {self.model}.")
...
class Terrain(Morph):
    ...
    Note
    ----
    Rigid terrain will also be represented as SDF for collision checking, but its resolution is auto-computed and ignores the value specified in `ezsim.materials.Rigid()`.
    ...
    def __init__(self, **data):
        ...
        if self.height_field is not None:
            try:
                if np.array(self.height_field).ndim != 2:
                    ezsim.raise_exception("`height_field` should be a 2D array.")
            except:
                ezsim.raise_exception("`height_field` should be array-like to be converted to np.ndarray.")
        ...
        if not mu.is_approx_multiple(self.subterrain_size[0], self.horizontal_scale) or not mu.is_approx_multiple(
            ...
            ezsim.raise_exception("`subterrain_size` should be divisible by `horizontal_scale`.")
