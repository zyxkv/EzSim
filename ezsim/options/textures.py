...
class ColorTexture(Texture):
    ...
    def is_black(self):
        return all(c < ezsim.EPS for c in self.color)
...
class ImageTexture(Texture):
    ...
    def __init__(self, **data):
        ...
        if not (self.image_path is None) ^ (self.image_array is None):
            ezsim.raise_exception("Please set either `image_path` or `image_array`.")

        if self.image_path is not None:
            input_image_path = self.image_path
            if not os.path.exists(self.image_path):
                self.image_path = os.path.join(ezsim.utils.get_assets_dir(), self.image_path)
            ...
            if not isinstance(self.image_array, np.ndarray):
                ezsim.raise_exception("`image_array` needs to be a numpy array.")
        ...
        if self.encoding not in ["srgb", "linear"]:
            ezsim.raise_exception(f"Invalid image encoding: {self.encoding}.")
    ...
    def is_black(self):
        return all(c < ezsim.EPS for c in self.image_color) or np.max(self.image_array) == 0