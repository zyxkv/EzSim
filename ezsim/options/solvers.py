    ...
    def __init__(self, **data):
        ...
        if self.requires_grad:
            if self.substeps_local % self.substeps != 0:
                ezsim.raise_exception("`substeps_local` must be divisible by `substeps` when `requires_grad` is True.")
            else:
...
class RigidOptions(Options):
    ...
    integrator : ezsim.integrator, optional
        Integrator type. Current supported integrators are 'ezsim.integrator.Euler', 'ezsim.integrator.implicitfast' and
        'ezsim.integrator.approximate_implicitfast'. 'Euler' and 'implicitfast' are consistent with their Mujoco
    ...
    IK_max_targets : int, optional
        ...
        Defaults to 6.
    constraint_solver : ezsim.constraint_solver, optional
        Constraint solver type. Current supported constraint solvers are 'ezsim.constraint_solver.CG' (conjugate gradient)
        and 'ezsim.constraint_solver.Newton' (Newton's method). Defaults to 'Newton'.
    ...
    integrator: ezsim.integrator = ezsim.integrator.approximate_implicitfast
    ...
    # constraint solver
    constraint_solver: ezsim.constraint_solver = ezsim.constraint_solver.Newton
    iterations: int = 50
...
class MPMOptions(Options):
    ...
    def __init__(self, **data):
        ...
        if not np.all(np.array(self.upper_bound) > np.array(self.lower_bound)):
            ezsim.raise_exception("Invalid pair of upper_bound and lower_bound.")
    ...
    def __init__(self, **data):
        ...
        if self.hash_grid_cell_size is None:
            ...
            if self.hash_grid_cell_size < self._support_radius:
                ezsim.raise_exception("`hash_grid_cell_size` should not be smaller than 2 * `particle_size`.")
        ...
        if self.hash_grid_res is None:
            ...
            default_hash_grid_res = np.array([150, 150, 150], dtype=ezsim.np_int)
            self._hash_grid_res = np.minimum(max_hash_grid_res, default_hash_grid_res)
        else:
            self._hash_grid_res = np.ceil(np.array(self.hash_grid_res) / self.hash_grid_cell_size).astype(ezsim.np_int)
    ...
    def __init__(self, **data):
        ...
        if self.hash_grid_cell_size is None:
            ...
            if self.hash_grid_cell_size < 1.25 * self.particle_size:
                ezsim.raise_exception("`hash_grid_cell_size` should not be smaller than 1.25 * `particle_size`.")
