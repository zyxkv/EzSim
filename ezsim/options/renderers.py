...
class Renderer(Options):
    """
    This is the base class for all `ezsim.renderers.*` classes.
...
class Rasterizer(Renderer):
    ...
    Note
    ----
    You can set which renderer to use for cameras, but the interactive viewer always uses the rasterizer rendering backend. If you want to configure properties like shadow, lights, etc., you should use `ezsim.options.VisOptions` instead.
    """
...
class RayTracer(Renderer):
    ...
    def __init__(self, **data):
        ...
        if self.logging_level not in ["debug", "info", "warning"]:
            ezsim.raise_exception("Invalid logging level.")

        if self.env_euler is not None:
            if self.env_quat is None:
                self.env_quat = ezsim.utils.geom.xyz_to_quat(np.array(self.env_euler), rpy=True, degrees=True)
            else:
                ezsim.logger.warning("`env_euler` is ignored when `env_quat` is specified.")
