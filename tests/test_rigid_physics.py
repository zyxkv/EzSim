...
def test_rope_ball(ezsim_sim, mj_sim, ezsim_solver, tol):
    ...
    simulate_and_check_mujoco_consistency(ezsim_sim, mj_sim, num_steps=300, tol=1e-8)
...
def test_pendulum_links_acc(ezsim_sim, tol):
    ...
    for _ in range(100):
        ...
        acc_classical_lin_world = tensor_to_array(ezsim_sim.rigid_solver.get_links_acc())
    ...
    for _ in range(400):
        ezsim_sim.scene.step()
    acc_classical_lin_world = ezsim_sim.rigid_solver.get_links_acc()
...
def test_double_pendulum_links_acc(ezsim_sim, tol):
    ...
    for _ in range(100):
        ...
        # Linear true acceleration
        acc_classical_lin_world = tensor_to_array(ezsim_sim.rigid_solver.get_links_acc()[[0, 2, 4]])
        assert_allclose(acc_classical_lin_world[0], 0, tol=tol)
...
def test_pd_control(show_viewer):
    ...
    with pytest.raises(ezsim.EzSimException):
...
def test_path_planning_avoidance(n_envs, show_viewer, tol):
    ...
    hand_pos_ref = torch.tensor([0.3, 0.1, 0.1], dtype=ezsim.tc_float, device=ezsim.device)
...
def test_frictionloss_advanced(show_viewer, tol):
    ...
    scene.reset()
    box.set_pos(torch.tensor((0.1, 0.0, 1.0), dtype=ezsim.tc_float, device=ezsim.device))
    for _ in range(200):
        scene.step()
...
def test_get_constraints_api(show_viewer, tol):
    scene = ezsim.Scene(
        sim_options=ezsim.options.SimOptions(gravity=(0.0, 0.0, 0.0)),
        show_viewer=show_viewer,
    )
    robot = scene.add_entity(
        ezsim.morphs.MJCF(
    ...
    cube = scene.add_entity(ezsim.morphs.Box(size=(0.05, 0.05, 0.05), pos=(0.2, 0.0, 0.05)))
    scene.build(n_envs=2)
