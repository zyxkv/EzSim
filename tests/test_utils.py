...
def test_utils_geom_numpy_vs_tensor_consistency(batch_shape):
    for py_func, shapes_in, shapes_out in (
        ...
        np_args, tc_args = [], []
        for i in range(len(shape_args)):
            np_arg = np.random.randn(*batch_shape, *shape_args[i]).clip(-1.0, 1.0).astype(ezsim.np_float)
            tc_arg = torch.as_tensor(np_arg, dtype=ezsim.tc_float, device=ezsim.device)
        ...
        for np_out, tc_out in zip(np_outs, tc_outs):
            np.testing.assert_allclose(np_out, tc_out, atol=ezsim.EPS)
    ...
    for ti_func, ti_func_inv, shapes_value_args, shapes_transform_args in (
        ...
        for i, shape_arg in enumerate(map(tuple, (*shapes_in, *shapes_value_args, *shapes_value_args))):
            if shape_arg in ((4, 4), (3, 3)):
                R = gu.rotvec_to_R(np.random.randn(*batch_shape, 3).clip(-1.0, 1.0).astype(ezsim.np_float))
                if shape_arg == (4, 4):
                    trans = np.random.randn(*batch_shape, 3).astype(ezsim.np_float)
                    np_arg = gu.trans_R_to_T(trans, R)
                else:
                    np_arg = R
            else:
                np_arg = np.random.randn(*batch_shape, *shape_arg).clip(-1.0, 1.0).astype(ezsim.np_float)
...
def test_utils_geom_taichi_identity(batch_shape):
    ...
    for ti_funcs, shape_args in (
        ...
        for shape_arg in (*shape_args, shape_args[0]):
            ti_type = ti.Vector if len(shape_arg) == 1 else ti.Matrix
            ti_arg = ti_type.field(*shape_arg, dtype=ezsim.ti_float, shape=batch_shape)
            ti_arg.from_numpy(np.random.randn(*batch_shape, *shape_arg).clip(-1.0, 1.0).astype(ezsim.np_float))
            ti_args.append(ti_arg)
    ...
    for py_funcs, shape_args in (
        ...
        for shape_arg in (*shape_args, shape_args[0]):
            if tuple(shape_arg) == (3, 3):
                np_arg = gu.rotvec_to_R(np.random.randn(*batch_shape, 3).clip(-1.0, 1.0).astype(ezsim.np_float))
            else:
                np_arg = np.random.randn(*batch_shape, *shape_arg).clip(-1.0, 1.0).astype(ezsim.np_float)
            tc_arg = torch.as_tensor(np_arg, dtype=ezsim.tc_float, device=ezsim.device)
            np_args.append(np_arg)
