...
class Go2Env:
    def __init__(self, num_envs, env_cfg, obs_cfg, reward_cfg, command_cfg, show_viewer=False):
        ...
        self.scene = ezsim.Scene(
            ...
            vis_options=ezsim.options.VisOptions(rendered_envs_idx=list(range(1))),
        ...
        # add plain
        self.scene.add_entity(ezsim.morphs.URDF(file="urdf/plane/plane.urdf", fixed=True))
        ...
        self.projected_gravity = torch.zeros((self.num_envs, 3), device=ezsim.device, dtype=ezsim.tc_float)
        self.global_gravity = torch.tensor([0.0, 0.0, -1.0], device=ezsim.device, dtype=ezsim.tc_float).repeat(
            self.num_envs, 1
        )
    ...
    def _resample_commands(self, envs_idx):
        self.commands[envs_idx, 0] = ezsim_rand_float(*self.command_cfg["lin_vel_x_range"], (len(envs_idx),), ezsim.device)
        self.commands[envs_idx, 1] = ezsim_rand_float(*self.command_cfg["lin_vel_y_range"], (len(envs_idx),), ezsim.device)
        self.commands[envs_idx, 2] = ezsim_rand_float(*self.command_cfg["ang_vel_range"], (len(envs_idx),), ezsim.device)
